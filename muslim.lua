...
if not DevAbs:get(Server.."Idmuslim") then 
io.write('\27[1;35m\nالان ارسل ايدي المطور الاساسي -› ⤈\n\27[0;33;49m') 
local DevId = io.read():gsub(' ','') 
if tostring(DevId):match('%d+') then 
io.write('\27[1;36mتم حفظ ايدي المطور الاساسي\n27[0;39;49m') 
DevAbs:set(Server.."Idmuslim",DevId) 
else 
print('\27[1;31m------------ Y●L ------------\nلم يتم حفظ ايدي المطور الاساسي ارسله مره اخرى\n------------ Y●L ------------') 
end 
os.execute('lua muslim.lua') 
end 
if not DevAbs:get(Server.."Tokenmuslim") then 
...
DevAbs:set(Server.."Tokenmuslim",TokenBot) 
end  
else 
...
Config = {
DevId = DevAbs:get(Server.."Idmuslim"),
TokenBot = DevAbs:get(Server.."Tokenmuslim"),
muslim = DevAbs:get(Server.."Tokenmuslim"):match("(%d+)"),
SudoIds = {DevAbs:get(Server.."Idmuslim")},
}
Create(Config, "./config.lua") 
file = io.open("muslim.sh", "w")  
file:write([[
#!/usr/bin/env bash
cd $HOME/muslim
token="]]..DevAbs:get(Server.."Tokenmuslim")..[["
while(true) do
rm -fr ../.telegram-cli
if [ ! -f ./tg ]; then
echo "------------ Y●L ------------ ≈ ------------ Y●L ------------ ≈ ┉ ≈ ┉"
...
./tg -s ./muslim.lua -p PROFILE --bot=$token
done
]])  
file:close()  
file = io.open("Run", "w")  
...
while(true) do
rm -fr ../.telegram-cli
screen -S muslim -X kill
screen -S muslim ./muslim.sh
done
]]) 
file:close() 
...
local Load_muslim = function() 
local f = io.open("./config.lua", "r") 
if not f then 
AutoSet() 
else 
f:close() 
DevAbs:del(Server.."Idmuslim");DevAbs:del(Server.."Tokenmuslim")
end 
...
muslim = Config.muslim
TokenBot = Config.TokenBot
NameBot = (DevAbs:get(muslim..'Abs:NameBot') or 'dev-yousef 🇸🇦')
--     Source dev yousef     --
...
local Status = DevAbs:sismember(muslim..'Abs:SecondSudo:',msg.sender_user_id_) 
...
function Bot(msg) 
local var = false  
if msg.sender_user_id_ == tonumber(muslim) then  
var = true  
end  
return var  
end 
...
local Status = DevAbs:sismember(muslim..'Abs:SudoBot:',msg.sender_user_id_) 
...
local Status = DevAbs:sismember(muslim..'Abs:ManagerAll:',msg.sender_user_id_) 
...
local Status = DevAbs:sismember(muslim..'Abs:AdminAll:',msg.sender_user_id_) 
...
------ Vip Member All ------
function VipAll(msg) 
local Status = DevAbs:sismember(muslim..'Abs:VipAll:',msg.sender_user_id_) 
...
local Status = DevAbs:sismember(muslim..'Abs:AbsConstructor:'..msg.chat_id_,msg.sender_user_id_) 
if Status or SudoBot(msg) or Sudo(msg) or SecondSudo(msg) or Bot(msg) then  
return true  
else  
return false  
...
local Status = DevAbs:sismember(muslim..'Abs:BasicConstructor:'..msg.chat_id_,msg.sender_user_id_) 
...
local Status = DevAbs:sismember(muslim..'Abs:Constructor:'..msg.chat_id_,msg.sender_user_id_) 
...
local Status = DevAbs:sismember(muslim..'Abs:Managers:'..msg.chat_id_,msg.sender_user_id_) 
...
----------  Admin  ---------
function Admin(msg) 
local Status = DevAbs:sismember(muslim..'Abs:Admins:'..msg.chat_id_,msg.sender_user_id_) 
...
---------Vip Member---------
function VipMem(msg) 
local Status = DevAbs:sismember(muslim..'Abs:VipMem:'..msg.chat_id_,msg.sender_user_id_) 
...
local Status = DevAbs:sismember(muslim..'Abs:Cleaner:'..msg.chat_id_,msg.sender_user_id_) 
...
if DevAbs:sismember(muslim..'Abs:Ban:'..chat_id, user_id) then
var = true
else
var = false
end
...
if DevAbs:sismember(muslim..'Abs:BanAll:', user_id) then
var = true
else
var = false
end
...
if DevAbs:sismember(muslim..'Abs:Muted:'..chat_id, user_id) then
var = true
else
var = false
end
...
if DevAbs:sismember(muslim..'Abs:MuteAll:', user_id) then
var = true
else
var = false
end
...
--     Source dev yousef     --
function muslimFiles(msg)
for v in io.popen('ls Files'):lines() do
if v:match(".lua$") then
plugin = dofile("Files/"..v)
if plugin.muslim and msg then
FilesText = plugin.muslim(msg)
end
end
end
...
if File_Name:lower():match('(%d+)') ~= muslim:lower() then 
...
local info_file = io.open('./'..muslim..'.json', "r"):read('*a')
local JsonInfo = JSON.decode(info_file)
vardump(JsonInfo)
DevAbs:set(muslim.."Abs:NameBot",JsonInfo.BotName) 
for IdGps,v in pairs(JsonInfo.GroupsList) do
DevAbs:sadd(muslim.."Abs:Groups",IdGps) 
DevAbs:set(muslim.."Abs:Lock:Bots"..IdGps,"del") DevAbs:hset(muslim.."Abs:Spam:Group:User"..IdGps ,"Spam:User","keed") 
...
DevAbs:set(muslim..Lock..IdGps,true)
end
if v.AbsConstructors then
for k,IdAbsConstructors in pairs(v.AbsConstructors) do
DevAbs:sadd(muslim..'Abs:AbsConstructor:'..IdGps,IdAbsConstructors)  
print('تم رفع منشئين المجموعات')
end
end
if v.BasicConstructors then
for k,IdBasicConstructors in pairs(v.BasicConstructors) do
DevAbs:sadd(muslim..'Abs:BasicConstructor:'..IdGps,IdBasicConstructors)  
print('تم رفع ( '..k..' ) منشئين اساسيين')
end
end
if v.Constructors then
for k,IdConstructors in pairs(v.Constructors) do
DevAbs:sadd(muslim..'Abs:Constructor:'..IdGps,IdConstructors)  
print('تم رفع ( '..k..' ) منشئين')
end
end
if v.Managers then
for k,IdManagers in pairs(v.Managers) do
DevAbs:sadd(muslim..'Abs:Managers:'..IdGps,IdManagers)  
...
vardump(IdAdmins)
DevAbs:sadd(muslim..'Abs:Admins:'..IdGps,IdAdmins)  
print('تم رفع ( '..k..' ) ادامن')
end
end
if v.Vips then
for k,IdVips in pairs(v.Vips) do
DevAbs:sadd(muslim..'Abs:VipMem:'..IdGps,IdVips)  
print('تم رفع ( '..k..' ) مميزين')
end
end
if v.LinkGroups then
if v.LinkGroups ~= "" then
DevAbs:set(muslim.."Abs:Groups:Links"..IdGps,v.LinkGroups)   
print('( تم وضع روابط المجموعات )')
end
end
if v.Welcomes then
if v.Welcomes ~= "" then
DevAbs:set(muslim.."Abs:Groups:Welcomes"..IdGps,v.Welcomes)   
...
function IdRank(user_id,chat_id) if tonumber(user_id) == tonumber(1432584666) then devyousef = 'مبرمج السورس' elseif tonumber(user_id) == tonumber(muslim) then devyousef = 'البوت' elseif SudoId(user_id) then devyousef = 'المطور الاساسي' elseif DevAbs:sismember(muslim..'Abs:SecondSudo:', user_id) then devyousef = 'المطور الثانوي' elseif DevAbs:sismember(muslim..'Abs:SudoBot:', user_id) then devyousef = DevAbs:get(muslim.."Abs:SudoBot:Rd"..chat_id) or 'المطور' elseif DevAbs:sismember(muslim..'Abs:ManagerAll:', user_id) then devyousef = DevAbs:get(muslim.."Abs:Managers:Rd"..chat_id) or 'المدير العام' elseif DevAbs:sismember(muslim..'Abs:AdminAll:', user_id) then devyousef = DevAbs:get(muslim.."Abs:Admins:Rd"..chat_id) or 'الادمن العام' elseif DevAbs:sismember(muslim..'Abs:VipAll:', user_id) then devyousef = DevAbs:get(muslim.."Abs:VipMem:Rd"..chat_id) or 'المميز العام' elseif DevAbs:sismember(muslim..'Abs:AbsConstructor:'..chat_id, user_id) then devyousef = 'المالك' elseif DevAbs:sismember(muslim..'Abs:BasicConstructor:'..chat_id, user_id) then devyousef = DevAbs:get(muslim.."Abs:BasicConstructor:Rd"..chat_id) or 'المنشئ الاساسي' elseif DevAbs:sismember(muslim..'Abs:Constructor:'..chat_id, user_id) then devyousef = DevAbs:get(muslim.."Abs:Constructor:Rd"..chat_id) or 'المنشئ' elseif DevAbs:sismember(muslim..'Abs:Managers:'..chat_id, user_id) then devyousef = DevAbs:get(muslim.."Abs:Managers:Rd"..chat_id) or 'المدير' elseif DevAbs:sismember(muslim..'Abs:Admins:'..chat_id, user_id) then devyousef = DevAbs:get(muslim.."Abs:Admins:Rd"..chat_id) or 'الادمن' elseif DevAbs:sismember(muslim..'Abs:VipMem:'..chat_id, user_id) then  devyousef = DevAbs:get(muslim.."Abs:VipMem:Rd"..chat_id) or 'المميز' elseif DevAbs:sismember(muslim..'Abs:Cleaner:'..chat_id, user_id) then  devyousef = DevAbs:get(muslim.."Abs:Cleaner:Rd"..chat_id) or 'المنظف' else devyousef = DevAbs:get(muslim.."Abs:mem:Rd"..chat_id) or 'العضو' end return devyousef end
--     Source dev yousef     --
function RankChecking(user_id,chat_id)
if SudoId(user_id) then
var = true  
elseif tonumber(user_id) == tonumber(muslim) then  
var = true  
elseif DevAbs:sismember(muslim..'Abs:SecondSudo:', user_id) then
var = true  
elseif DevAbs:sismember(muslim..'Abs:SudoBot:', user_id) then
var = true  
elseif DevAbs:sismember(muslim..'Abs:ManagerAll:', user_id) then
var = true  
elseif DevAbs:sismember(muslim..'Abs:AdminAll:', user_id) then
var = true  
elseif DevAbs:sismember(muslim..'Abs:VipAll:', user_id) then
var = true  
elseif DevAbs:sismember(muslim..'Abs:AbsConstructor:'..chat_id, user_id) then
var = true
elseif DevAbs:sismember(muslim..'Abs:BasicConstructor:'..chat_id, user_id) then
var = true
elseif DevAbs:sismember(muslim..'Abs:Constructor:'..chat_id, user_id) then
var = true  
elseif DevAbs:sismember(muslim..'Abs:Managers:'..chat_id, user_id) then
var = true  
elseif DevAbs:sismember(muslim..'Abs:Admins:'..chat_id, user_id) then
var = true  
elseif DevAbs:sismember(muslim..'Abs:VipMem:'..chat_id, user_id) then  
var = true 
else  
var = false
end  
...
local abs = (muslim..'Abs:Filters:'..msg.chat_id_)
if abs then
...
local var = true 
if DevAbs:get(muslim.."Abs:ChId") then
local url , res = https.request('https://api.telegram.org/bot'..TokenBot..'/getchatmember?chat_id='..DevAbs:get(muslim..'Abs:ChId')..'&user_id='..msg.sender_user_id_)
local data = json:decode(url)
if res ~= 200 or data.result.status == "left" or data.result.status == "kicked" then
var = false 
if DevAbs:get(muslim..'Abs:ChText') then
local ChText = DevAbs:get(muslim..'Abs:ChText')
send(msg.chat_id_,msg.id_,'['..ChText..']')
else
local Check = https.request('https://api.telegram.org/bot'..TokenBot..'/getChat?chat_id='..DevAbs:get(muslim.."Abs:ChId"))
local GetInfo = JSON.decode(Check)
...
if DataText == '/delyes' and DevAbs:get(muslim..'yes'..data.sender_user_id_) == 'delyes' then
DevAbs:del(muslim..'yes'..data.sender_user_id_, 'delyes')
DevAbs:del(muslim..'no'..data.sender_user_id_, 'delno')
...
if DataText == '/delno' and DevAbs:get(muslim..'no'..data.sender_user_id_) == 'delno' then
DevAbs:del(muslim..'yes'..data.sender_user_id_, 'delyes')
DevAbs:del(muslim..'no'..data.sender_user_id_, 'delno')
EditMsg(Chat_Id2, Msg_Id2, "🇸🇦︙تم الغاء امر اطردني") 
end
--     Source dev yousef     --
if DataText == '/yesdel' and DevAbs:get(muslim..'yesdel'..data.sender_user_id_) == 'delyes' then
DevAbs:del(muslim..'yesdel'..data.sender_user_id_, 'delyes')
DevAbs:del(muslim..'nodel'..data.sender_user_id_, 'delno')
if DevAbs:sismember(muslim..'Abs:Constructor:'..data.chat_id_, data.sender_user_id_) then
constructor = 'المنشئين • ' else constructor = '' end 
if DevAbs:sismember(muslim..'Abs:Managers:'..data.chat_id_, data.sender_user_id_) then
Managers = 'المدراء • ' else Managers = '' end
if DevAbs:sismember(muslim..'Abs:Admins:'..data.chat_id_, data.sender_user_id_) then
admins = 'الادامن • ' else admins = '' end
if DevAbs:sismember(muslim..'Abs:VipMem:'..data.chat_id_, data.sender_user_id_) then
vipmem = 'المميزين • ' else vipmem = '' end
if DevAbs:sismember(muslim..'Abs:Cleaner:'..data.chat_id_, data.sender_user_id_) then
cleaner = 'المنظفين • ' else cleaner = '' end
if DevAbs:sismember(muslim..'User:Donky:'..data.chat_id_, data.sender_user_id_) then
donky = 'المطايا • ' else donky = '' end
if DevAbs:sismember(muslim..'Abs:Constructor:'..data.chat_id_, data.sender_user_id_) or DevAbs:sismember(muslim..'Abs:Managers:'..data.chat_id_, data.sender_user_id_) or DevAbs:sismember(muslim..'Abs:Admins:'..data.chat_id_, data.sender_user_id_) or DevAbs:sismember(muslim..'Abs:VipMem:'..data.chat_id_, data.sender_user_id_) or DevAbs:sismember(muslim..'Abs:Cleaner:'..data.chat_id_, data.sender_user_id_) or DevAbs:sismember(muslim..'User:Donky:'..data.chat_id_, data.sender_user_id_) then
DevAbs:srem(muslim..'Abs:Constructor:'..data.chat_id_,data.sender_user_id_)
DevAbs:srem(muslim..'Abs:Managers:'..data.chat_id_,data.sender_user_id_)
DevAbs:srem(muslim..'Abs:Admins:'..data.chat_id_,data.sender_user_id_)
DevAbs:srem(muslim..'Abs:VipMem:'..data.chat_id_,data.sender_user_id_)
DevAbs:srem(muslim..'Abs:Cleaner:'..data.chat_id_,data.sender_user_id_)
DevAbs:srem(muslim..'User:Donky:'..data.chat_id_,data.sender_user_id_)
...
if DevAbs:get(muslim.."Abs:NewDev"..data.sender_user_id_) then
if DataText == '/setno' then
EditMsg(Chat_Id2, Msg_Id2, "🇸🇦︙تم الغاء امر تغير المطور الاساسي") 
DevAbs:del(muslim.."Abs:NewDev"..data.sender_user_id_)
return false
end
if DataText == '/setyes' then
local NewDev = DevAbs:get(muslim.."Abs:NewDev"..data.sender_user_id_)
...
if DataText == '/nodel' and DevAbs:get(muslim..'nodel'..data.sender_user_id_) == 'delno' then
DevAbs:del(muslim..'yesdel'..data.sender_user_id_, 'delyes')
DevAbs:del(muslim..'nodel'..data.sender_user_id_, 'delno')
EditMsg(Chat_Id2, Msg_Id2, "🇸🇦︙تم الغاء امر نزلني") 
end
if DataText == '/YesRolet' and DevAbs:get(muslim.."Abs:WittingStartRolet"..data.chat_id_..data.sender_user_id_) then
local List = DevAbs:smembers(muslim..'Abs:ListRolet'..data.chat_id_) 
local UserName = List[math.random(#List)]
tdcli_function ({ID="SearchPublicChat",username_ = UserName},function(arg,dp) 
DevAbs:incrby(muslim..'Abs:GamesNumber'..data.chat_id_..dp.id_, 5) 
end,nil) 
DevAbs:del(muslim..'Abs:ListRolet'..data.chat_id_) 
DevAbs:del(muslim.."Abs:WittingStartRolet"..data.chat_id_..data.sender_user_id_)
EditMsg(Chat_Id2, Msg_Id2, "🇸🇦︙*صاحب الحظ* -› ["..UserName.."]\n🇸🇦︙*مبروك لقد ربحت وحصلت على 5 نقاط يمكنك استبدالها بالرسائل*")
end
if DataText == '/NoRolet' then
DevAbs:del(muslim..'Abs:ListRolet'..data.chat_id_) 
DevAbs:del(muslim.."Abs:NumRolet"..data.chat_id_..data.sender_user_id_) 
DevAbs:del(muslim.."Abs:WittingStartRolet"..data.chat_id_..data.sender_user_id_)
...
if DevAbs:sismember(muslim..'Abs:Tkeed:'..Chat_Id2, data.sender_user_id_) then
HTTPS.request("https://api.telegram.org/bot"..TokenBot.."/restrictChatMember?chat_id="..Chat_Id2.."&user_id="..data.sender_user_id_.."&can_send_messages=True&can_send_media_messages=True&can_send_other_messages=True&can_add_web_page_previews=True")
DevAbs:srem(muslim..'Abs:Tkeed:'..Chat_Id2, data.sender_user_id_)
...
DevAbs:sadd(muslim..'Abs:VipMem:'..data.chat_id_,dp.id_)
...
tdcli_function ({ID = "GetUser",user_id_ = AbsId},function(arg,dp) 
DevAbs:sadd(muslim..'Abs:Cleaner:'..data.chat_id_,dp.id_)
...
tdcli_function ({ID = "GetUser",user_id_ = AbsId},function(arg,dp) 
DevAbs:sadd(muslim..'Abs:Admins:'..data.chat_id_,dp.id_)
...
tdcli_function ({ID = "GetUser",user_id_ = AbsId},function(arg,dp) 
DevAbs:sadd(muslim..'Abs:Managers:'..data.chat_id_,dp.id_)
...
tdcli_function ({ID = "GetUser",user_id_ = AbsId},function(arg,dp) 
DevAbs:sadd(muslim..'Abs:Constructor:'..data.chat_id_,dp.id_)
...
tdcli_function ({ID = "GetUser",user_id_ = AbsId},function(arg,dp) 
DevAbs:sadd(muslim..'Abs:BasicConstructor:'..data.chat_id_,dp.id_)
...
tdcli_function ({ID = "GetUser",user_id_ = AbsId},function(arg,dp) 
DevAbs:sadd(muslim..'Abs:AbsConstructor:'..data.chat_id_,dp.id_)
...
tdcli_function ({ID = "GetUser",user_id_ = AbsId},function(arg,dp) 
DevAbs:sadd(muslim..'Abs:SudoBot:'..data.chat_id_,dp.id_)
...
tdcli_function ({ID = "GetUser",user_id_ = AbsId},function(arg,dp) 
DevAbs:sadd(muslim..'Abs:SecondSudo:'..data.chat_id_,dp.id_)
...
tdcli_function ({ID = "GetUser",user_id_ = AbsId},function(arg,dp) 
DevAbs:srem(muslim..'Abs:VipMem:'..data.chat_id_,dp.id_)
...
tdcli_function ({ID = "GetUser",user_id_ = AbsId},function(arg,dp) 
DevAbs:srem(muslim..'Abs:Cleaner:'..data.chat_id_,dp.id_)
...
tdcli_function ({ID = "GetUser",user_id_ = AbsId},function(arg,dp) 
DevAbs:srem(muslim..'Abs:Admins:'..data.chat_id_,dp.id_)
...
tdcli_function ({ID = "GetUser",user_id_ = AbsId},function(arg,dp) 
DevAbs:srem(muslim..'Abs:Managers:'..data.chat_id_,dp.id_)
...
tdcli_function ({ID = "GetUser",user_id_ = AbsId},function(arg,dp) 
DevAbs:srem(muslim..'Abs:Constructor:'..data.chat_id_,dp.id_)
...
tdcli_function ({ID = "GetUser",user_id_ = AbsId},function(arg,dp) 
DevAbs:srem(muslim..'Abs:BasicConstructor:'..data.chat_id_,dp.id_)
...
tdcli_function ({ID = "GetUser",user_id_ = AbsId},function(arg,dp) 
DevAbs:srem(muslim..'Abs:AbsConstructor:'..data.chat_id_,dp.id_)
...
tdcli_function ({ID = "GetUser",user_id_ = AbsId},function(arg,dp) 
DevAbs:srem(muslim..'Abs:SudoBot:'..data.chat_id_,dp.id_)
...
tdcli_function ({ID = "GetUser",user_id_ = AbsId},function(arg,dp) 
DevAbs:srem(muslim..'Abs:SecondSudo:'..data.chat_id_,dp.id_)
...
DevAbs:sadd(muslim..'Abs:Muted:'..data.chat_id_,dp.id_)
EditMsg(Chat_Id2, Msg_Id2,'🇸🇦︙العضو -› ['..dp.first_name_..'](t.me/'..(dp.username_ or 'yousef_labban1')..')\n🇸🇦︙تم كتمه من المجموعة')
end
end,nil)
...
tdcli_function ({ID = "GetUser",user_id_ = AbsId},function(arg,dp) 
DevAbs:srem(muslim..'Abs:Muted:'..data.chat_id_,dp.id_)
...
DevAbs:sadd(muslim..'Abs:Ban:'..data.chat_id_,dp.id_)
EditMsg(Chat_Id2, Msg_Id2,'🇸🇦︙العضو -› ['..dp.first_name_..'](t.me/'..(dp.username_ or 'yousef_labban1')..')\n🇸🇦︙تم حظره من المجموعة')
end
end,nil)
...
tdcli_function ({ID = "GetUser",user_id_ = AbsId},function(arg,dp) 
DevAbs:srem(muslim..'Abs:Ban:'..data.chat_id_,dp.id_)
...
DevAbs:sadd(muslim..'Abs:Tkeed:'..data.chat_id_,dp.id_)
...
DevAbs:srem(muslim..'Abs:Tkeed:'..data.chat_id_,dp.id_)
...
DevAbs:del(muslim..'Abs:Text:GpTexts'..Abbs..data.chat_id_)
DevAbs:srem(muslim..'Abs:Manager:GpRedod'..data.chat_id_,Abbs)
end
if DataText and DataText:match('/EndRedod:'..tonumber(data.sender_user_id_)..'(.*)') then
local Abbs = DataText:match('/EndRedod:'..tonumber(data.sender_user_id_)..'(.*)')
local List = DevAbs:smembers(muslim..'Abs:Text:GpTexts'..Abbs..data.chat_id_)
if DevAbs:get(muslim..'Abs:Add:GpRedod'..data.sender_user_id_..data.chat_id_) then
EditMsg(Chat_Id2, Msg_Id2, "🇸🇦︙تم انهاء وحفظ -› "..#List.." من الردود المتعددة للامر -› "..Abbs) 
DevAbs:del(muslim..'Abs:Add:GpRedod'..data.sender_user_id_..data.chat_id_)
else
...
DevAbs:del(muslim..'Abs:Add:GpRedod'..data.sender_user_id_..data.chat_id_)
DevAbs:del(muslim..'Abs:Text:GpTexts'..Abbs..data.chat_id_)
DevAbs:del(muslim..'Abs:Add:GpTexts'..data.sender_user_id_..data.chat_id_)
DevAbs:srem(muslim..'Abs:Manager:GpRedod'..data.chat_id_,Abbs)
else
EditMsg(Chat_Id2, Msg_Id2, "🇸🇦︙عذرا صلاحية الامر منتهيه !") 
end
...
if tonumber(Abbs) == tonumber(data.sender_user_id_) then
local Help = DevAbs:get(muslim..'Abs:Help')
local Text = [[
...
local Help = DevAbs:get(muslim..'Abs:Help1')
local Text = [[
...
local Help = DevAbs:get(muslim..'Abs:Help2')
local Text = [[
🇸🇦︙اوامر الادامن -› ⤈
------------ Y●L ------------
🇸🇦︙الاعدادت
...
local Help = DevAbs:get(muslim..'Abs:Help3')
local Text = [[
🇸🇦︙اوامر المدراء -› ⤈
------------ Y●L ------------
🇸🇦︙فحص البوت
...
local Help = DevAbs:get(muslim..'Abs:Help4')
local Text = [[
🇸🇦︙اوامر المنشئين -› ⤈
------------ Y●L ------------
🇸🇦︙تنزيل الكل
...
local Help = DevAbs:get(muslim..'Abs:Help5')
local Text = [[
🇸🇦︙اوامر المطورين -› ⤈
------------ Y●L ------------
🇸🇦︙القروبات
...
if tonumber(Abbs) == tonumber(data.sender_user_id_) then
local Help = DevAbs:get(muslim..'Abs:Help6')
local Text = [[
🇸🇦︙اوامر الاعضاء -› ⤈
------------ Y●L ------------
...
if text and DevAbs:get(muslim.."Del:Cmd:Group"..msg.chat_id_..":"..msg.sender_user_id_) == "true" then
local NewCmmd = DevAbs:get(muslim.."Set:Cmd:Group:New1"..msg.chat_id_..":"..text)
if NewCmmd then
DevAbs:del(muslim.."Set:Cmd:Group:New1"..msg.chat_id_..":"..text)
DevAbs:del(muslim.."Set:Cmd:Group:New"..msg.chat_id_)
DevAbs:srem(muslim.."List:Cmd:Group:New"..msg.chat_id_,text)
...
DevAbs:del(muslim.."Del:Cmd:Group"..msg.chat_id_..":"..msg.sender_user_id_)
return false
end
if text and text:match('^'..(DevAbs:get(muslim..'Abs:NameBot') or "dev-yousef 🇸🇦")..' ') then
data.message_.content_.text_ = data.message_.content_.text_:gsub('^'..(DevAbs:get(muslim..'Abs:NameBot') or "dev-yousef 🇸🇦")..' ','')
end
if data.message_.content_.text_ then
local NewCmmd = DevAbs:get(muslim.."Set:Cmd:Group:New1"..msg.chat_id_..":"..data.message_.content_.text_)
if NewCmmd then
data.message_.content_.text_ = (NewCmmd or data.message_.content_.text_)
end
end
if text and DevAbs:get(muslim.."Set:Cmd:Group"..msg.chat_id_..":"..msg.sender_user_id_) == "true" then
DevAbs:set(muslim.."Set:Cmd:Group:New"..msg.chat_id_,text)
Dev_Abs(msg.chat_id_, msg.id_, 1, "🇸🇦︙ارسل الامر الجديد", 1, 'html')
DevAbs:del(muslim.."Set:Cmd:Group"..msg.chat_id_..":"..msg.sender_user_id_)
DevAbs:set(muslim.."Set:Cmd:Group1"..msg.chat_id_..":"..msg.sender_user_id_,"true1") 
return false
end
if text and DevAbs:get(muslim.."Set:Cmd:Group1"..msg.chat_id_..":"..msg.sender_user_id_) == "true1" then
local NewCmd = DevAbs:get(muslim.."Set:Cmd:Group:New"..msg.chat_id_)
DevAbs:set(muslim.."Set:Cmd:Group:New1"..msg.chat_id_..":"..text,NewCmd)
DevAbs:sadd(muslim.."List:Cmd:Group:New"..msg.chat_id_,text)
Dev_Abs(msg.chat_id_, msg.id_, 1, "🇸🇦︙تم حفظ الامر", 1, 'html')
DevAbs:del(muslim.."Set:Cmd:Group1"..msg.chat_id_..":"..msg.sender_user_id_)
return false
end
if Constructor(msg) then
if text == "الاوامر المضافة" and ChCheck(msg) then
local List = DevAbs:smembers(muslim.."List:Cmd:Group:New"..msg.chat_id_.."") 
t = "🇸🇦︙قائمة الاوامر المضافة -› ⤈ \n------------ Y●L ------------\n"
for k,v in pairs(List) do
Cmds = DevAbs:get(muslim.."Set:Cmd:Group:New1"..msg.chat_id_..":"..v)
if Cmds then 
t = t..k.."~ ("..v..") • {"..Cmds.."}\n"
else
t = t..k.."~ ("..v..") \n"
...
local List = DevAbs:smembers(muslim.."List:Cmd:Group:New"..msg.chat_id_)
for k,v in pairs(List) do
DevAbs:del(muslim.."Set:Cmd:Group:New1"..msg.chat_id_..":"..v)
DevAbs:del(muslim.."List:Cmd:Group:New"..msg.chat_id_)
end
Dev_Abs(msg.chat_id_, msg.id_, 1, "🇸🇦︙تم حذف الاوامر المضافة في المجموعة", 1, 'html')
end
if text == "ترتيب الاوامر" then
DevAbs:set(muslim.."Set:Cmd:Group:New1"..msg.chat_id_..":ا","ايدي")
DevAbs:sadd(muslim.."List:Cmd:Group:New"..msg.chat_id_,"ا")
DevAbs:set(muslim.."Set:Cmd:Group:New1"..msg.chat_id_..":تك","تنزيل الكل")
DevAbs:sadd(muslim.."List:Cmd:Group:New"..msg.chat_id_,"تك")
DevAbs:set(muslim.."Set:Cmd:Group:New1"..msg.chat_id_..":م","رفع مميز")
DevAbs:sadd(muslim.."List:Cmd:Group:New"..msg.chat_id_,"م")
DevAbs:set(muslim.."Set:Cmd:Group:New1"..msg.chat_id_..":اد","رفع ادمن")
DevAbs:sadd(muslim.."List:Cmd:Group:New"..msg.chat_id_,"اد")
DevAbs:set(muslim.."Set:Cmd:Group:New1"..msg.chat_id_..":مد","رفع مدير")
DevAbs:sadd(muslim.."List:Cmd:Group:New"..msg.chat_id_,"مد")
DevAbs:set(muslim.."Set:Cmd:Group:New1"..msg.chat_id_..":من","رفع منشئ")
DevAbs:sadd(muslim.."List:Cmd:Group:New"..msg.chat_id_,"من")
DevAbs:set(muslim.."Set:Cmd:Group:New1"..msg.chat_id_..":اس","رفع منشئ اساسي")
DevAbs:sadd(muslim.."List:Cmd:Group:New"..msg.chat_id_,"اس")
DevAbs:set(muslim.."Set:Cmd:Group:New1"..msg.chat_id_..":تعط","تعطيل الايدي بالصورة")
DevAbs:sadd(muslim.."List:Cmd:Group:New"..msg.chat_id_,"تعط")
DevAbs:set(muslim.."Set:Cmd:Group:New1"..msg.chat_id_..":تفع","تفعيل الايدي بالصورة")
DevAbs:sadd(muslim.."List:Cmd:Group:New"..msg.chat_id_,"تفع")
...
DevAbs:set(muslim.."Set:Cmd:Group"..msg.chat_id_..":"..msg.sender_user_id_,"true") 
Dev_Abs(msg.chat_id_, msg.id_, 1, "🇸🇦︙ارسل الامر القديم", 1, 'html')
return false
end
if text == "حذف امر" or text == "مسح امر" and ChCheck(msg) then 
DevAbs:set(muslim.."Del:Cmd:Group"..msg.chat_id_..":"..msg.sender_user_id_,"true") 
...
local List = DevAbs:smembers(muslim.."Coomds"..msg.chat_id_)
if #List == 0 then
...
for k,v in pairs(List) do
var = DevAbs:get(muslim.."Comd:New:rt:Abs:"..v..msg.chat_id_)
if var then
t = t..k.."~ "..v.." • ("..var..")\n"
else
t = t..k.."~ "..v.."\n"
...
local List = DevAbs:smembers(muslim.."Coomds"..msg.chat_id_)
for k,v in pairs(List) do
DevAbs:del(muslim.."Comd:New:rt:Abs:"..v..msg.chat_id_)
DevAbs:del(muslim.."Coomds"..msg.chat_id_)
end
...
DevAbs:set(muslim.."Comd:New:rt"..msg.chat_id_..msg.sender_user_id_,ComdNew)  
DevAbs:sadd(muslim.."Coomds"..msg.chat_id_,ComdNew)  
DevAbs:setex(muslim.."Comd:New"..msg.chat_id_..msg.sender_user_id_,200,true)  
...
DevAbs:del(muslim.."Comd:New:rt:Abs:"..ComdNew..msg.chat_id_)
Dev_Abs(msg.chat_id_, msg.id_, 1, "🇸🇦︙تم حذف الصلاحية", 1, 'html')
end
if DevAbs:get(muslim.."Comd:New"..msg.chat_id_..msg.sender_user_id_) then 
if text and text:match("^الغاء$") then 
Dev_Abs(msg.chat_id_, msg.id_, 1, "🇸🇦︙تم الغاء الامر", 1, 'html')
DevAbs:del(muslim.."Comd:New"..msg.chat_id_..msg.sender_user_id_) 
return false  
end 
if text == "مدير" then
...
local textn = DevAbs:get(muslim.."Comd:New:rt"..msg.chat_id_..msg.sender_user_id_)  
DevAbs:set(muslim.."Comd:New:rt:Abs:"..textn..msg.chat_id_,text)
...
if DevAbs:sismember(muslim.."Coomds"..msg.chat_id_,DEV_ABBAS) then
function by_reply(extra, result, success)   
tdcli_function ({ID = "GetUser",user_id_ = result.sender_user_id_},function(arg,data) 
local mrabs = DevAbs:get(muslim.."Comd:New:rt:Abs:"..DEV_ABBAS..msg.chat_id_)
...
DevAbs:set(muslim.."Comd:New:rt:User:"..msg.chat_id_..result.sender_user_id_,DEV_ABBAS) 
DevAbs:sadd(muslim..'Abs:VipMem:'..msg.chat_id_, result.sender_user_id_)
elseif mrabs == "ادمن" and Admin(msg) then 
Dev_Abs(msg.chat_id_, msg.id_, 1, '🇸🇦︙العضو -› ❨ ['..data.first_name_..'](t.me/'..(data.username_ or 'yousef_labban1')..')'..' ❩\n🇸🇦︙تم رفعه ❨ '..DEV_ABBAS..' ❩ بنجاح', 1, 'md')
DevAbs:set(muslim.."Comd:New:rt:User:"..msg.chat_id_..result.sender_user_id_,DEV_ABBAS)
DevAbs:sadd(muslim..'Abs:Admins:'..msg.chat_id_, result.sender_user_id_)
elseif mrabs == "مدير" and Manager(msg) then
Dev_Abs(msg.chat_id_, msg.id_, 1, '🇸🇦︙العضو -› ❨ ['..data.first_name_..'](t.me/'..(data.username_ or 'yousef_labban1')..')'..' ❩\n🇸🇦︙تم رفعه ❨ '..DEV_ABBAS..' ❩ بنجاح', 1, 'md')
DevAbs:set(muslim.."Comd:New:rt:User:"..msg.chat_id_..result.sender_user_id_,DEV_ABBAS)  
DevAbs:sadd(muslim..'Abs:Managers:'..msg.chat_id_, result.sender_user_id_)
...
DevAbs:srem(muslim..'Abs:VipMem:'..msg.chat_id_, result.sender_user_id_)
DevAbs:del(muslim.."Comd:New:rt:User:"..msg.chat_id_..result.sender_user_id_)
elseif mrabs == "ادمن" and Admin(msg) then 
Dev_Abs(msg.chat_id_, msg.id_, 1, '🇸🇦︙العضو -› ❨ ['..data.first_name_..'](t.me/'..(data.username_ or 'yousef_labban1')..')'..' ❩\n🇸🇦︙تم تنزيله ❨ '..DEV_ABBAS..' ❩ بنجاح', 1, 'md')
DevAbs:srem(muslim..'Abs:Admins:'..msg.chat_id_, result.sender_user_id_)
DevAbs:del(muslim.."Comd:New:rt:User:"..msg.chat_id_..result.sender_user_id_)
elseif mrabs == "مدير" and Manager(msg) then
Dev_Abs(msg.chat_id_, msg.id_, 1, '🇸🇦︙العضو -› ❨ ['..data.first_name_..'](t.me/'..(data.username_ or 'yousef_labban1')..')'..' ❩\n🇸🇦︙تم تنزيله ❨ '..DEV_ABBAS..' ❩ بنجاح', 1, 'md')
DevAbs:srem(muslim..'Abs:Managers:'..msg.chat_id_, result.sender_user_id_)
DevAbs:del(muslim.."Comd:New:rt:User:"..msg.chat_id_..result.sender_user_id_)
elseif mrabs == "عضو" then
Dev_Abs(msg.chat_id_, msg.id_, 1, '🇸🇦︙العضو -› ❨ ['..data.first_name_..'](t.me/'..(data.username_ or 'yousef_labban1')..')'..' ❩\n🇸🇦︙تم تنزيله ❨ '..DEV_ABBAS..' ❩ بنجاح', 1, 'md')
end
...
if DevAbs:sismember(muslim.."Coomds"..msg.chat_id_,text1[2]) then
function py_username(extra, result, success)   
if result.id_ then
local mrabs = DevAbs:get(muslim.."Comd:New:rt:Abs:"..text1[2]..msg.chat_id_)
if mrabs == "مميز" and VipMem(msg) then
Dev_Abs(msg.chat_id_, msg.id_, 1, '🇸🇦︙العضو -› ❨ ['..result.title_..'](t.me/'..(text1[3] or 'yousef_labban1')..')'..' ❩\n🇸🇦︙تم رفعه ❨ '..text1[2]..' ❩ بنجاح', 1, 'md')
DevAbs:sadd(muslim..'Abs:VipMem:'..msg.chat_id_, result.id_)
DevAbs:set(muslim.."Comd:New:rt:User:"..msg.chat_id_..result.id_,text1[2])
elseif mrabs == "ادمن" and Admin(msg) then 
Dev_Abs(msg.chat_id_, msg.id_, 1, '🇸🇦︙العضو -› ❨ ['..result.title_..'](t.me/'..(text1[3] or 'yousef_labban1')..')'..' ❩\n🇸🇦︙تم رفعه ❨ '..text1[2]..' ❩ بنجاح', 1, 'md')
DevAbs:sadd(muslim..'Abs:Admins:'..msg.chat_id_, result.id_)
DevAbs:set(muslim.."Comd:New:rt:User:"..msg.chat_id_..result.id_,text1[2])
elseif mrabs == "مدير" and Manager(msg) then
Dev_Abs(msg.chat_id_, msg.id_, 1, '🇸🇦︙العضو -› ❨ ['..result.title_..'](t.me/'..(text1[3] or 'yousef_labban1')..')'..' ❩\n🇸🇦︙تم رفعه ❨ '..text1[2]..' ❩ بنجاح', 1, 'md')
DevAbs:sadd(muslim..'Abs:Managers:'..msg.chat_id_, result.id_)
DevAbs:set(muslim.."Comd:New:rt:User:"..msg.chat_id_..result.id_,text1[2])
elseif mrabs == "عضو" then
Dev_Abs(msg.chat_id_, msg.id_, 1, '🇸🇦︙العضو -› ❨ ['..result.title_..'](t.me/'..(text1[3] or 'yousef_labban1')..')'..' ❩\n🇸🇦︙تم رفعه ❨ '..text1[2]..' ❩ بنجاح', 1, 'md')
end
...
DevAbs:srem(muslim..'Abs:VipMem:'..msg.chat_id_, result.id_)
DevAbs:del(muslim.."Comd:New:rt:User:"..msg.chat_id_..result.id_)
elseif mrabs == "ادمن" and Admin(msg) then 
Dev_Abs(msg.chat_id_, msg.id_, 1, '🇸🇦︙العضو -› ❨ ['..result.title_..'](t.me/'..(text1[3] or 'yousef_labban1')..')'..' ❩\n🇸🇦︙تم تنزيله ❨ '..text1[2]..' ❩ بنجاح', 1, 'md')
DevAbs:srem(muslim..'Abs:Admins:'..msg.chat_id_, result.id_)
DevAbs:del(muslim.."Comd:New:rt:User:"..msg.chat_id_..result.id_)
elseif mrabs == "مدير" and Manager(msg) then
Dev_Abs(msg.chat_id_, msg.id_, 1, '🇸🇦︙العضو -› ❨ ['..result.title_..'](t.me/'..(text1[3] or 'yousef_labban1')..')'..' ❩\n🇸🇦︙تم تنزيله ❨ '..text1[2]..' ❩ بنجاح', 1, 'md')
DevAbs:srem(muslim..'Abs:Managers:'..msg.chat_id_, result.id_)
DevAbs:del(muslim.."Comd:New:rt:User:"..msg.chat_id_..result.id_)
elseif mrabs == "عضو" then
Dev_Abs(msg.chat_id_, msg.id_, 1, '🇸🇦︙العضو -› ❨ ['..result.title_..'](t.me/'..(text1[3] or 'yousef_labban1')..')'..' ❩\n🇸🇦︙تم تنزيله ❨ '..text1[2]..' ❩ بنجاح', 1, 'md')
end
...
DevAbs:incr(muslim..'Abs:UsersMsgs'..muslim..os.date('%d')..':'..msg.chat_id_..':'..msg.sender_user_id_)
DevAbs:incr(muslim..'Abs:UsersMsgs'..msg.chat_id_..':'..msg.sender_user_id_)
DevAbs:incr(muslim..'Abs:MsgNumberDay'..msg.chat_id_..':'..os.date('%d'))  
ChatType = 'sp' 
elseif id:match("^(%d+)") then
if not DevAbs:sismember(muslim.."Abs:Users",msg.chat_id_) then
DevAbs:sadd(muslim.."Abs:Users",msg.chat_id_)
end
ChatType = 'pv' 
else
ChatType = 'gp' 
...
if not DevAbs:get(muslim..'Abs:Start:Time'..msg.sender_user_id_) then
tdcli_function({ID="GetUser",user_id_=muslim},function(arg,dp) 
local inline = {{{text="⌁ اضفني في مجموعتك ⌁",url="t.me/"..dp.username_.."?startgroup=botstart"}}}
local start = DevAbs:get(muslim.."Abs:Start:Bot")
if start then 
Start_Source = start
else
...
DevAbs:setex(muslim..'Abs:Start:Time'..msg.sender_user_id_,300,true)
return false
end 
--     Source dev yousef     --
if not SecondSudo(msg) and not DevAbs:sismember(muslim..'Abs:Ban:Pv',msg.sender_user_id_) and not DevAbs:get(muslim..'Abs:Texting:Pv') then
...
DevAbs:sadd(muslim..'Abs:Ban:Pv',data.id_)  
return false  
end 
if text == 'الغاء الحظر' or text == 'الغاء حظر' then
local Text = '🇸🇦︙العضو -› ['..string.sub(data.first_name_,0, 40)..'](tg://user?id='..data.id_..')'..'\n🇸🇦︙تم الغاء حظره من التواصل'
SendText(DevId,Text,msg.id_/2097152/0.5,'md') 
DevAbs:srem(muslim..'Abs:Ban:Pv',data.id_)  
return false  
end 
...
--     Source dev yousef     --
if text and DevAbs:get(muslim..'Abs:Start:Bots'..msg.sender_user_id_) then
if text == 'الغاء' then   
Dev_Abs(msg.chat_id_, msg.id_, 1, '🇸🇦︙تم الغاء حفظ كليشة الستارت', 1, 'md')
DevAbs:del(muslim..'Abs:Start:Bots'..msg.sender_user_id_) 
return false
end
DevAbs:set(muslim.."Abs:Start:Bot",text)  
...
DevAbs:set(muslim..'Abs:Start:Bots'..msg.sender_user_id_,true) 
...
DevAbs:del(muslim..'Abs:Texting:Pv') 
end
if text == 'تعطيل التواصل' or text == '-› تعطيل التواصل ⌁' then  
local devyousef = '🇸🇦︙اهلا عزيزي -› '..AbsRank(msg)..' \n🇸🇦︙تم تعطيل التواصل بنجاح'
absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, devyousef, 14, string.len(msg.sender_user_id_))
DevAbs:set(muslim..'Abs:Texting:Pv',true) 
end
end
--     Source dev yousef     --
...
DevAbs:incr(muslim..'Abs:ContactNumber'..msg.chat_id_..':'..msg.sender_user_id_)
DevAbs:set(muslim.."Who:Added:Me"..msg.chat_id_..':'..msg.content_.members_[0].id_,msg.sender_user_id_)
local mem_id = msg.content_.members_  
local Bots = DevAbs:get(muslim.."Abs:Lock:Bots"..msg.chat_id_) 
for i=0,#mem_id do  
...
DevAbs:sadd(muslim..'Abs:Tkeed:'..msg.chat_id_, msg.sender_user_id_)
GetInfo = https.request("https://api.telegram.org/bot"..TokenBot.."/kickChatMember?chat_id="..msg.chat_id_.."&user_id="..mem_id[i].id_)
local JsonInfo = JSON.decode(GetInfo)
if JsonInfo.ok == true and #mem_id == i then
local Msgs = {}
...
if msg.content_.ID == "MessageChatDeleteMember" and tonumber(msg.content_.user_.id_) == tonumber(muslim) then 
DevAbs:srem(muslim.."Abs:Groups", msg.chat_id_) 
...
if DevAbs:get(muslim..'Abs:Lock:TagServr'..msg.chat_id_) then
DeleteMessage(msg.chat_id_,{[0] = msg.id_})    
end   
end
if msg.content_.ID == "MessageChatJoinByLink" or msg.content_.ID == "MessageChatAddMembers" then   
DevAbs:incr(muslim..'Abs:EntryNumber'..msg.chat_id_..':'..os.date('%d'))  
elseif msg.content_.ID == "MessageChatDeleteMember" then   
DevAbs:incr(muslim..'Abs:ExitNumber'..msg.chat_id_..':'..os.date('%d'))  
end
--     Source dev yousef     --
if text ==('تفعيل') and not SudoBot(msg) and ChCheck(msg) then
if not DevAbs:get(muslim..'Abs:Lock:FreeBot'..muslim) then
if ChatType == 'pv' then
...
DevAbs:sadd(muslim..'Abs:Admins:'..msg.chat_id_, admins[i].user_id_)
tdcli_function ({ID = "GetUser",user_id_ = admins[i].user_id_},function(arg,ba) 
if ba.first_name_ == false then
DevAbs:srem(muslim..'Abs:Admins:'..msg.chat_id_, admins[i].user_id_)
end
end,nil)   
else
DevAbs:sadd(muslim..'Abs:Admins:'..msg.chat_id_, admins[i].user_id_)
...
DevAbs:sadd(muslim.."Abs:BasicConstructor:"..msg.chat_id_,admins[i].user_id_)
DevAbs:sadd(muslim.."Abs:AbsConstructor:"..msg.chat_id_,admins[i].user_id_)
tdcli_function ({ID = "GetUser",user_id_ = admins[i].user_id_},function(arg,ba) 
if ba.first_name_ == false then
DevAbs:srem(muslim.."Abs:BasicConstructor:"..msg.chat_id_,admins[i].user_id_)
DevAbs:srem(muslim.."Abs:AbsConstructor:"..msg.chat_id_,admins[i].user_id_)
end
end,nil)  
end 
end
end,nil)
if DevAbs:sismember(muslim..'Abs:Groups',msg.chat_id_) then
Dev_Abs(msg.chat_id_, msg.id_, 1, '🇸🇦︙المجموعة بالتاكيد مفعله', 1, 'md')
else
if tonumber(data.member_count_) < tonumber(DevAbs:get(muslim..'Abs:Num:Add:Bot') or 0) and not SecondSudo(msg) then
Dev_Abs(msg.chat_id_, msg.id_, 1, '🇸🇦︙عدد اعضاء المجموعة اقل من -› *'..(DevAbs:get(muslim..'Abs:Num:Add:Bot') or 0)..'* عضو', 1, 'md')
return false
end
ReplyStatus(msg,result.id_,"ReplyBy","🇸🇦︙تم تفعيل المجموعة "..dp.title_)  
DevAbs:sadd(muslim.."Abs:Groups",msg.chat_id_)
DevAbs:sadd(muslim..'Abs:BasicConstructor:'..msg.chat_id_,msg.sender_user_id_)
local Name1 = result.first_name_
local Name1 = Name1:gsub('"',"") 
local Name1 = Name1:gsub("'","") 
local Name1 = Name1:gsub("`","") 
...
DevAbs:set(muslim.."Abs:Groups:Links"..msg.chat_id_,LinkGroup) 
...
DevAbs:set(muslim..'Save:UserName'..msg.sender_user_id_,data.username_)
end;end,nil) 
--     Source dev yousef     --
local ReFalse = tostring(msg.chat_id_)
if not DevAbs:sismember(muslim.."Abs:Groups",msg.chat_id_) and not ReFalse:match("^(%d+)") and not SudoBot(msg) then
...
if DevAbs:get(muslim..'Abs:Lock:Robot'..msg.chat_id_) then
tdcli_function({ID="GetUser",user_id_=msg.sender_user_id_},function(arg,dp) 
HTTPS.request("https://api.telegram.org/bot"..TokenBot.."/restrictChatMember?chat_id="..msg.chat_id_.."&user_id="..dp.id_)
DevAbs:sadd(muslim..'Abs:Tkeed:'..msg.chat_id_, dp.id_)
...
if DevAbs:get(muslim.."Abs:Lock:Join"..msg.chat_id_) then
...
if not Manager(msg) then 
local filter = DevAbs:smembers(muslim.."Abs:FilterPhoto"..msg.chat_id_)
...
if not Manager(msg) then 
local filter = DevAbs:smembers(muslim.."Abs:FilterAnimation"..msg.chat_id_)
for k,v in pairs(filter) do
...
if not Manager(msg) then 
local filter = DevAbs:smembers(muslim.."Abs:FilterSteckr"..msg.chat_id_)
for k,v in pairs(filter) do
...
local DelGpRedRedods = DevAbs:get(muslim..'Abs:Add:GpRedod'..msg.sender_user_id_..msg.chat_id_)
local GetGpTexts = DevAbs:get(muslim..'Abs:Add:GpTexts'..msg.sender_user_id_..msg.chat_id_)
if DelGpRedRedods == 'DelGpRedRedods' then
Dev_Abs(msg.chat_id_, msg.id_, 1,'🇸🇦︙الرد -› '..msg.content_.text_..' للكلمه -› '..GetGpTexts..' تم حذفها',  1, "html")
DevAbs:del(muslim..'Abs:Add:GpRedod'..msg.sender_user_id_..msg.chat_id_)
DevAbs:srem(muslim..'Abs:Text:GpTexts'..GetGpTexts..msg.chat_id_,msg.content_.text_)
return false
end
end
if text and text:match("^(.*)$") then
local DelGpRed = DevAbs:get(muslim..'Abs:Add:GpRedod'..msg.sender_user_id_..msg.chat_id_)
if DelGpRed == 'DelGpRedod' then
Dev_Abs(msg.chat_id_, msg.id_, 1,'🇸🇦︙الكلمه -› '..msg.content_.text_..' تم حذفها',  1, "html")
DevAbs:del(muslim..'Abs:Add:GpRedod'..msg.sender_user_id_..msg.chat_id_)
DevAbs:del(muslim..'Abs:Text:GpTexts'..msg.content_.text_..msg.chat_id_)
DevAbs:srem(muslim..'Abs:Manager:GpRedod'..msg.chat_id_,msg.content_.text_)
return false
end
end
if text and text:match("^(.*)$") then
local DelGpRed = DevAbs:get(muslim..'Abs:Add:GpRed'..msg.sender_user_id_..msg.chat_id_)
if DelGpRed == 'DelGpRed' then
Dev_Abs(msg.chat_id_, msg.id_, 1,'🇸🇦︙الكلمه -› '..msg.content_.text_..' تم حذفها',  1, "html")
DevAbs:del(muslim..'Abs:Add:GpRed'..msg.sender_user_id_..msg.chat_id_)
DevAbs:del(muslim..'Abs:Gif:GpRed'..msg.content_.text_..msg.chat_id_)
DevAbs:del(muslim..'Abs:Voice:GpRed'..msg.content_.text_..msg.chat_id_)
DevAbs:del(muslim..'Abs:Audio:GpRed'..msg.content_.text_..msg.chat_id_)
DevAbs:del(muslim..'Abs:Photo:GpRed'..msg.content_.text_..msg.chat_id_)
DevAbs:del(muslim..'Abs:Stecker:GpRed'..msg.content_.text_..msg.chat_id_)
DevAbs:del(muslim..'Abs:Video:GpRed'..msg.content_.text_..msg.chat_id_)
DevAbs:del(muslim..'Abs:File:GpRed'..msg.content_.text_..msg.chat_id_)
DevAbs:del(muslim..'Abs:Text:GpRed'..msg.content_.text_..msg.chat_id_)
DevAbs:srem(muslim..'Abs:Manager:GpRed'..msg.chat_id_,msg.content_.text_)
return false
end
end
if text and text:match("^(.*)$") then
local DelAllRed = DevAbs:get(muslim.."Abs:Add:AllRed"..msg.sender_user_id_)
if DelAllRed == 'DelAllRed' then
Dev_Abs(msg.chat_id_, msg.id_, 1,'🇸🇦︙الكلمه -› '..msg.content_.text_..' تم حذفها',  1, "html")
DevAbs:del(muslim.."Abs:Add:AllRed"..msg.sender_user_id_)
DevAbs:del(muslim.."Abs:Gif:AllRed"..msg.content_.text_)
DevAbs:del(muslim.."Abs:Voice:AllRed"..msg.content_.text_)
DevAbs:del(muslim.."Abs:Audio:AllRed"..msg.content_.text_)
DevAbs:del(muslim.."Abs:Photo:AllRed"..msg.content_.text_)
DevAbs:del(muslim.."Abs:Stecker:AllRed"..msg.content_.text_)
DevAbs:del(muslim.."Abs:Video:AllRed"..msg.content_.text_)
DevAbs:del(muslim.."Abs:File:AllRed"..msg.content_.text_)
DevAbs:del(muslim.."Abs:Text:AllRed"..msg.content_.text_)
DevAbs:del(muslim.."Abs:Sudo:AllRed",msg.content_.text_)
return false
end
end
--     Source dev yousef     --
if text and text:match("^(.*)$") then
local SaveGpRedod = DevAbs:get(muslim..'Abs:Add:GpRedod'..msg.sender_user_id_..msg.chat_id_)
if SaveGpRedod == 'SaveGpRedod' then
local GetGpTexts = DevAbs:get(muslim..'Abs:Add:GpTexts'..msg.sender_user_id_..msg.chat_id_)
local List = DevAbs:smembers(muslim..'Abs:Text:GpTexts'..GetGpTexts..msg.chat_id_)
if text == "الغاء" then 
Dev_Abs(msg.chat_id_, msg.id_, 1, "🇸🇦︙🇸🇦︙تم الغاء عملية حفظ الردود المتعددة للامر -› "..GetGpTexts ,  1, "md")
DevAbs:del(muslim..'Abs:Add:GpRedod'..msg.sender_user_id_..msg.chat_id_)
DevAbs:del(muslim..'Abs:Text:GpTexts'..GetGpTexts..msg.chat_id_)
DevAbs:del(muslim..'Abs:Add:GpTexts'..msg.sender_user_id_..msg.chat_id_)
DevAbs:srem(muslim..'Abs:Manager:GpRedod'..msg.chat_id_,GetGpTexts)
return false
end
Text = text:gsub('"',""):gsub('"',""):gsub("`",""):gsub("*","")
DevAbs:sadd(muslim..'Abs:Text:GpTexts'..GetGpTexts..msg.chat_id_,Text)
if #List == 4 then 
...
if text and not DevAbs:get(muslim..'Abs:Add:GpRedod'..msg.sender_user_id_..msg.chat_id_) then
if DevAbs:sismember(muslim..'Abs:Manager:GpRedod'..msg.chat_id_,text) then
local devyousef =  DevAbs:smembers(muslim..'Abs:Text:GpTexts'..text..msg.chat_id_)
Dev_Abs(msg.chat_id_, msg.id_, 1, '['..devyousef[math.random(#devyousef)]..']' , 1, 'md')  
end
end
--     Source dev yousef     --
if msg.content_.text_ or msg.content_.video_ or msg.content_.document_ or msg.content_.sticker_ or msg.content_.voice_ or msg.content_.audio_ or msg.content_.photo_ or msg.content_.animation_ then 
local SaveGpRed = DevAbs:get(muslim..'Abs:Add:GpRed'..msg.sender_user_id_..msg.chat_id_)
if SaveGpRed == 'SaveGpRed' then 
if text == 'الغاء' then
local DelManagerRep = DevAbs:get(muslim..'DelManagerRep'..msg.chat_id_)
DevAbs:srem(muslim..'Abs:Manager:GpRed'..msg.chat_id_,DelManagerRep)
Dev_Abs(msg.chat_id_, msg.id_, 1, '🇸🇦︙تم الغاء حفظ الرد', 1, 'md')
DevAbs:del(muslim..'Abs:Add:GpText'..msg.sender_user_id_..msg.chat_id_)
DevAbs:del(muslim..'Abs:Add:GpRed'..msg.sender_user_id_..msg.chat_id_)
DevAbs:del(muslim..'DelManagerRep'..msg.chat_id_)
return false
end
DevAbs:del(muslim..'Abs:Add:GpRed'..msg.sender_user_id_..msg.chat_id_)
local SaveGpRed = DevAbs:get(muslim..'Abs:Add:GpText'..msg.sender_user_id_..msg.chat_id_)
if msg.content_.video_ then DevAbs:set(muslim..'Abs:Video:GpRed'..SaveGpRed..msg.chat_id_, msg.content_.video_.video_.persistent_id_)
end
if msg.content_.document_ then DevAbs:set(muslim..'Abs:File:GpRed'..SaveGpRed..msg.chat_id_, msg.content_.document_.document_.persistent_id_)
end
if msg.content_.sticker_ then DevAbs:set(muslim..'Abs:Stecker:GpRed'..SaveGpRed..msg.chat_id_, msg.content_.sticker_.sticker_.persistent_id_) 
end 
if msg.content_.voice_ then DevAbs:set(muslim..'Abs:Voice:GpRed'..SaveGpRed..msg.chat_id_, msg.content_.voice_.voice_.persistent_id_) 
end
if msg.content_.audio_ then DevAbs:set(muslim..'Abs:Audio:GpRed'..SaveGpRed..msg.chat_id_, msg.content_.audio_.audio_.persistent_id_) 
end
...
DevAbs:set(muslim..'Abs:Photo:GpRed'..SaveGpRed..msg.chat_id_, photo_in_group) 
end
if msg.content_.animation_ then DevAbs:set(muslim..'Abs:Gif:GpRed'..SaveGpRed..msg.chat_id_, msg.content_.animation_.animation_.persistent_id_) 
end 
if msg.content_.text_ then
DevAbs:set(muslim..'Abs:Text:GpRed'..SaveGpRed..msg.chat_id_, msg.content_.text_)
end 
...
if msg.content_.text_ and not DevAbs:get(muslim..'Abs:Lock:GpRed'..msg.chat_id_) then 
if DevAbs:get(muslim..'Abs:Video:GpRed'..msg.content_.text_..msg.chat_id_) then 
sendVideo(msg.chat_id_, msg.id_, 0, 1,nil, DevAbs:get(muslim..'Abs:Video:GpRed'..msg.content_.text_..msg.chat_id_)) 
end 
if DevAbs:get(muslim..'Abs:File:GpRed'..msg.content_.text_..msg.chat_id_) then 
sendDocument(msg.chat_id_, msg.id_, 0, 1,nil, DevAbs:get(muslim..'Abs:File:GpRed'..msg.content_.text_..msg.chat_id_)) 
end 
if DevAbs:get(muslim..'Abs:Voice:GpRed'..msg.content_.text_..msg.chat_id_) then 
sendVoice(msg.chat_id_, msg.id_, 0, 1, nil, DevAbs:get(muslim..'Abs:Voice:GpRed'..msg.content_.text_..msg.chat_id_)) 
end
if DevAbs:get(muslim..'Abs:Audio:GpRed'..msg.content_.text_..msg.chat_id_) then 
sendAudio(msg.chat_id_, msg.id_, 0, 1, nil, DevAbs:get(muslim..'Abs:Audio:GpRed'..msg.content_.text_..msg.chat_id_)) 
end
if DevAbs:get(muslim..'Abs:Photo:GpRed'..msg.content_.text_..msg.chat_id_) then 
sendPhoto(msg.chat_id_, msg.id_, 0, 1, nil, DevAbs:get(muslim..'Abs:Photo:GpRed'..msg.content_.text_..msg.chat_id_)) 
end
if DevAbs:get(muslim..'Abs:Gif:GpRed'..msg.content_.text_..msg.chat_id_) then 
sendDocument(msg.chat_id_, msg.id_, 0, 1, nil, DevAbs:get(muslim..'Abs:Gif:GpRed'..msg.content_.text_..msg.chat_id_)) 
end 
if DevAbs:get(muslim..'Abs:Stecker:GpRed'..msg.content_.text_..msg.chat_id_) then 
sendSticker(msg.chat_id_, msg.id_, 0, 1,nil, DevAbs:get(muslim..'Abs:Stecker:GpRed'..msg.content_.text_..msg.chat_id_))
end
if DevAbs:get(muslim..'Abs:Text:GpRed'..msg.content_.text_..msg.chat_id_) then
function devyousef(extra,result,success)
if result.username_ then username = '[@'..result.username_..']' else username = 'لا يوجد' end
local edit_msg = DevAbs:get(muslim..'Abs:EditMsg'..msg.chat_id_..msg.sender_user_id_) or 0
local user_msgs = DevAbs:get(muslim..'Abs:UsersMsgs'..msg.chat_id_..':'..msg.sender_user_id_)
local Text = DevAbs:get(muslim..'Abs:Text:GpRed'..msg.content_.text_..msg.chat_id_)
...
text = msg.content_.text_
if msg.content_.text_ or msg.content_.video_ or msg.content_.document_ or msg.content_.sticker_ or msg.content_.voice_ or msg.content_.audio_ or msg.content_.photo_ or msg.content_.animation_ then
local SaveAllRed = DevAbs:get(muslim.."Abs:Add:AllRed"..msg.sender_user_id_)
if SaveAllRed == 'SaveAllRed' then
if text == 'الغاء' then
local DelSudoRep = DevAbs:get(muslim..'DelSudoRep')
DevAbs:del(muslim.."Abs:Sudo:AllRed",DelSudoRep)
Dev_Abs(msg.chat_id_, msg.id_, 1, '🇸🇦︙تم الغاء حفظ الرد', 1, 'md')
DevAbs:del(muslim.."Abs:Add:AllText"..msg.sender_user_id_)
DevAbs:del(muslim.."Abs:Add:AllRed"..msg.sender_user_id_)
DevAbs:del(muslim.."DelSudoRep")
return false
end
DevAbs:del(muslim.."Abs:Add:AllRed"..msg.sender_user_id_)
local SaveAllRed = DevAbs:get(muslim.."Abs:Add:AllText"..msg.sender_user_id_)
if msg.content_.video_ then
DevAbs:set(muslim.."Abs:Video:AllRed"..SaveAllRed, msg.content_.video_.video_.persistent_id_)
end
if msg.content_.document_ then
DevAbs:set(muslim.."Abs:File:AllRed"..SaveAllRed, msg.content_.document_.document_.persistent_id_)
end
if msg.content_.sticker_ then
DevAbs:set(muslim.."Abs:Stecker:AllRed"..SaveAllRed, msg.content_.sticker_.sticker_.persistent_id_)
end
if msg.content_.voice_ then
DevAbs:set(muslim.."Abs:Voice:AllRed"..SaveAllRed, msg.content_.voice_.voice_.persistent_id_)
end
if msg.content_.audio_ then
DevAbs:set(muslim.."Abs:Audio:AllRed"..SaveAllRed, msg.content_.audio_.audio_.persistent_id_)
end
if msg.content_.photo_ then
if msg.content_.photo_.sizes_[0] then
...
DevAbs:set(muslim.."Abs:Photo:AllRed"..SaveAllRed, photo_in_all_groups)
end
if msg.content_.animation_ then
DevAbs:set(muslim.."Abs:Gif:AllRed"..SaveAllRed, msg.content_.animation_.animation_.persistent_id_)
end
if msg.content_.text_ then
DevAbs:set(muslim.."Abs:Text:AllRed"..SaveAllRed, msg.content_.text_)
end 
Dev_Abs(msg.chat_id_, msg.id_, 1, '🇸🇦︙تم حفظ الرد الجديد', 1, 'md') 
DevAbs:del(muslim.."Abs:Add:AllText"..msg.sender_user_id_)
DevAbs:del(muslim..'DelSudoRep')
return false end end
if msg.content_.text_ and not DevAbs:get(muslim..'Abs:Lock:AllRed'..msg.chat_id_) then
if DevAbs:get(muslim.."Abs:Video:AllRed"..msg.content_.text_) then
sendVideo(msg.chat_id_, msg.id_, 0, 1,nil, DevAbs:get(muslim.."Abs:Video:AllRed"..msg.content_.text_))
end
if DevAbs:get(muslim.."Abs:File:AllRed"..msg.content_.text_) then
sendDocument(msg.chat_id_, msg.id_, 0, 1,nil, DevAbs:get(muslim.."Abs:File:AllRed"..msg.content_.text_))
end
if DevAbs:get(muslim.."Abs:Voice:AllRed"..msg.content_.text_)  then
sendVoice(msg.chat_id_, msg.id_, 0, 1, nil, DevAbs:get(muslim.."Abs:Voice:AllRed"..msg.content_.text_))
end
if DevAbs:get(muslim.."Abs:Audio:AllRed"..msg.content_.text_)  then
sendAudio(msg.chat_id_, msg.id_, 0, 1, nil, DevAbs:get(muslim.."Abs:Audio:AllRed"..msg.content_.text_))
end
if DevAbs:get(muslim.."Abs:Photo:AllRed"..msg.content_.text_)  then
sendPhoto(msg.chat_id_, msg.id_, 0, 1, nil, DevAbs:get(muslim.."Abs:Photo:AllRed"..msg.content_.text_))
end
if  DevAbs:get(muslim.."Abs:Gif:AllRed"..msg.content_.text_) then
sendDocument(msg.chat_id_, msg.id_, 0, 1, nil, DevAbs:get(muslim.."Abs:Gif:AllRed"..msg.content_.text_))
end
if DevAbs:get(muslim.."Abs:Stecker:AllRed"..msg.content_.text_) then
sendSticker(msg.chat_id_, msg.id_, 0, 1,nil, DevAbs:get(muslim.."Abs:Stecker:AllRed"..msg.content_.text_))
end
if DevAbs:get(muslim.."Abs:Text:AllRed"..msg.content_.text_) then
function devyousef(extra,result,success)
if result.username_ then username = '[@'..result.username_..']' else username = 'لا يوجد' end
local edit_msg = DevAbs:get(muslim..'Abs:EditMsg'..msg.chat_id_..msg.sender_user_id_) or 0
local user_msgs = DevAbs:get(muslim..'Abs:UsersMsgs'..msg.chat_id_..':'..msg.sender_user_id_)
local Text = DevAbs:get(muslim.."Abs:Text:AllRed"..msg.content_.text_)
local Text = Text:gsub('#username',(username or 'لا يوجد')) 
local Text = Text:gsub('#name','['..result.first_name_..']')
local Text = Text:gsub('#id',msg.sender_user_id_)
local Text = Text:gsub('#edit',edit_msg)
...
if Type == "keed" and not DevAbs:sismember(muslim..'Abs:Tkeed:'..msg.chat_id_, msg.sender_user_id_) then
...
if Type == "mute" and not DevAbs:sismember(muslim..'Abs:Muted:'..msg.chat_id_, msg.sender_user_id_) then
...
--  end functions muslim --
--     Source dev yousef     --
--       Spam Check       --
if not VipMem(msg) and msg.content_.ID ~= "MessageChatAddMembers" and DevAbs:hget(muslim.."Abs:Spam:Group:User"..msg.chat_id_,"Spam:User") then 
if msg.sender_user_id_ ~= muslim then
floods = DevAbs:hget(muslim.."Abs:Spam:Group:User"..msg.chat_id_,"Spam:User") or "nil"
Num_Msg_Max = DevAbs:hget(muslim.."Abs:Spam:Group:User"..msg.chat_id_,"Num:Spam") or 5
Time_Spam = DevAbs:hget(muslim.."Abs:Spam:Group:User"..msg.chat_id_,"Num:Spam:Time") or 5
local post_count = tonumber(DevAbs:get(muslim.."Abs:Spam:Cont"..msg.sender_user_id_..":"..msg.chat_id_) or 0)
if post_count > tonumber(DevAbs:hget(muslim.."Abs:Spam:Group:User"..msg.chat_id_,"Num:Spam") or 5) then 
local ch = msg.chat_id_
local type = DevAbs:hget(muslim.."Abs:Spam:Group:User"..msg.chat_id_,"Spam:User") 
NotSpam(msg,type)  
end
DevAbs:setex(muslim.."Abs:Spam:Cont"..msg.sender_user_id_..":"..msg.chat_id_, tonumber(DevAbs:hget(muslim.."Abs:Spam:Group:User"..msg.chat_id_,"Num:Spam:Time") or 3), post_count+1) 
local edit_id = data.text_ or "nil"  
Num_Msg_Max = 5
if DevAbs:hget(muslim.."Abs:Spam:Group:User"..msg.chat_id_,"Num:Spam") then
Num_Msg_Max = DevAbs:hget(muslim.."Abs:Spam:Group:User"..msg.chat_id_,"Num:Spam") 
end
if DevAbs:hget(muslim.."Abs:Spam:Group:User"..msg.chat_id_,"Num:Spam:Time") then
Time_Spam = DevAbs:hget(muslim.."Abs:Spam:Group:User"..msg.chat_id_,"Num:Spam:Time") 
end 
end
end 
--     Source dev yousef     --
...
if Constructor(msg) or tonumber(msg.sender_user_id_) == tonumber(muslim) then
DevAbs:set(muslim..'Abs:PinnedMsg'..msg.chat_id_,msg.content_.message_id_)
else
local pin_id = DevAbs:get(muslim..'Abs:PinnedMsg'..msg.chat_id_)
if pin_id and DevAbs:get(muslim..'Abs:Lock:Pin'..msg.chat_id_) then
pinmsg(msg.chat_id_,pin_id,0)
end
end
end
if DevAbs:get(muslim..'Abs:viewget'..msg.sender_user_id_) then
if not msg.forward_info_ then
DevAbs:del(muslim..'Abs:viewget'..msg.sender_user_id_)
else
...
if DevAbs:get(muslim..'Abs:Lock:Forwards'..msg.chat_id_) then
if msg.forward_info_.ID == "MessageForwardedFromUser" or msg.forward_info_.ID == "MessageForwardedPost" then
DeleteMessage(msg.chat_id_,{[0] = msg.id_})
end
end
end
if DevAbs:get(muslim..'Abs:Lock:Photo'..msg.chat_id_) then
DeleteMessage(msg.chat_id_,{[0] = msg.id_})
end
if msg.content_.caption_ then
Filters(msg, msg.content_.caption_)
if DevAbs:get(muslim..'Abs:Lock:Links'..msg.chat_id_) then
if msg.content_.caption_:match("[Tt][Ee][Ll][Ee][Gg][Rr][Aa][Mm].[Mm][Ee]") or msg.content_.caption_:match("[Tt][Ll][Gg][Rr][Mm].[Mm][Ee]") or msg.content_.caption_:match("[Tt].[Mm][Ee]") or msg.content_.caption_:match("[Tt][Ee][Ll][Ee][Gg][Rr][Aa][Mm].[Dd][Oo][Gg]") then
DeleteMessage(msg.chat_id_,{[0] = msg.id_})
end
end
if DevAbs:get(muslim..'Abs:Lock:Tags'..msg.chat_id_) then
if msg.content_.caption_:match("@") then
DeleteMessage(msg.chat_id_,{[0] = msg.id_})
end
end
if msg.content_.caption_:match("#") then
if DevAbs:get(muslim..'Abs:Lock:Hashtak'..msg.chat_id_) then
DeleteMessage(msg.chat_id_,{[0] = msg.id_})
end
end
if msg.content_.caption_:match("[Hh][Tt][Tt][Pp][Ss]://") or msg.content_.caption_:match("[Hh][Tt][Tt][Pp]://") or msg.content_.caption_:match(".[Ii][Rr]") or msg.content_.caption_:match(".[Cc][Oo][Mm]") or msg.content_.caption_:match(".[Oo][Rr][Gg]") or msg.content_.caption_:match(".[Ii][Nn][Ff][Oo]") or msg.content_.caption_:match("[Ww][Ww][Ww].") or msg.content_.caption_:match(".[Xx][Yy][Zz]") or msg.content_.caption_:match(".[Tt][Kk]") or msg.content_.ID == "MessageEntityTextUrl" or msg.content_.ID == "MessageEntityUrl" then
if DevAbs:get(muslim..'Abs:Lock:WebLinks'..msg.chat_id_) then
DeleteMessage(msg.chat_id_,{[0] = msg.id_})
end
end
if msg.content_.caption_:match("[\216-\219][\128-\191]") then
if DevAbs:get(muslim..'Abs:Lock:Arabic'..msg.chat_id_) then
DeleteMessage(msg.chat_id_,{[0] = msg.id_})
end
end
if msg.content_.caption_:match("[A-Z]") or msg.content_.caption_:match("[a-z]") then
if DevAbs:get(muslim..'Abs:Lock:English'..msg.chat_id_) then
DeleteMessage(msg.chat_id_,{[0] = msg.id_})
end
end
end
...
if DevAbs:get(muslim..'Abs:Lock:Markdown'..msg.chat_id_) then
if not VipMem(msg) then
DeleteMessage(msg.chat_id_,{[0] = msg.id_})
end
end
...
if DevAbs:get(muslim..'Abs:Lock:Document'..msg.chat_id_) then
DeleteMessage(msg.chat_id_,{[0] = msg.id_})
end
if msg.content_.caption_ then
Filters(msg, msg.content_.caption_)
...
if not VipMem(msg) then
if DevAbs:get(muslim..'Abs:Lock:Inline'..msg.chat_id_) then
DeleteMessage(msg.chat_id_,{[0] = msg.id_})
end
end
--     Source dev yousef     --
...
elseif msg.content_.ID == "MessageSticker" then
if not VipMem(msg) then
if DevAbs:get(muslim..'Abs:Lock:Stickers'..msg.chat_id_) then
DeleteMessage(msg.chat_id_,{[0] = msg.id_})
end
end
...
if DevAbs:get(muslim..'Abs:Groups:Welcomes'..msg.chat_id_) then
Welcomes = DevAbs:get(muslim..'Abs:Groups:Welcomes'..msg.chat_id_)
else
Welcomes = '• نورت حبي \n• firstname \n• username'
end
...
if DevAbs:get(muslim.."Abs:Lock:Welcome"..msg.chat_id_) then
getUser(msg.sender_user_id_,get_welcome)
end
--     Source dev yousef     --
--      New User Add      --
elseif msg.content_.ID == "MessageChatAddMembers" then
if not DevAbs:get(muslim..'Abs:Lock:BotWelcome') then 
tdcli_function ({ID = "GetUserProfilePhotos",user_id_ = muslim,offset_ = 0,limit_ = 1},function(extra,abbas,success) 
for i=0,#msg.content_.members_ do    
BotWelcome = msg.content_.members_[i].id_    
if BotWelcome and BotWelcome == tonumber(muslim) then 
if DevAbs:sismember(muslim..'Abs:Groups',msg.chat_id_) then BotText = "مفعله في السابق\n🇸🇦︙ارسل -› الاوامر واستمتع بالمميزيات" else BotText = "معطله يجب رفعي مشرف\n🇸🇦︙بعد ذلك يرجى ارسال امر -› تفعيل\n🇸🇦︙سيتم رفع الادامن والمنشئ تلقائيا" end 
if DevAbs:get(muslim.."Abs:Text:BotWelcome") then AbsText = DevAbs:get(muslim.."Abs:Text:BotWelcome") else AbsText = "🇸🇦︙مرحبا انا بوت اسمي "..NameBot.."\n🇸🇦︙حالة المجموعة -› "..BotText.."\n------------ Y●L ------------" end 
if DevAbs:get(muslim.."Abs:Photo:BotWelcome") then AbsPhoto = DevAbs:get(muslim.."Abs:Photo:BotWelcome") elseif abbas.photos_[0] then AbsPhoto = abbas.photos_[0].sizes_[1].photo_.persistent_id_ else AbsPhoto = nil end 
...
if DevAbs:get(muslim..'Abs:Lock:Contact'..msg.chat_id_) then
DeleteMessage(msg.chat_id_,{[0] = msg.id_})
end
end
--     Source dev yousef     --
...
if DevAbs:get(muslim..'Abs:Lock:Music'..msg.chat_id_) then
DeleteMessage(msg.chat_id_,{[0] = msg.id_})
end
if msg.content_.caption_ then
Filters(msg, msg.content_.caption_)
...
if DevAbs:get(muslim..'Abs:Lock:Voice'..msg.chat_id_) then
DeleteMessage(msg.chat_id_,{[0] = msg.id_})
end
if msg.content_.caption_ then
Filters(msg, msg.content_.caption_)
...
if DevAbs:get(muslim..'Abs:Lock:Location'..msg.chat_id_) then
DeleteMessage(msg.chat_id_,{[0] = msg.id_})
return
end
if msg.content_.caption_ then
...
if DevAbs:get(muslim..'Abs:Lock:Videos'..msg.chat_id_) then
DeleteMessage(msg.chat_id_,{[0] = msg.id_})
end
if msg.content_.caption_ then
Filters(msg, msg.content_.caption_)
...
if DevAbs:get(muslim..'Abs:Lock:Gifs'..msg.chat_id_) then
DeleteMessage(msg.chat_id_,{[0] = msg.id_})
end
if msg.content_.caption_ then
Filters(msg, msg.content_.caption_)
...
if DevAbs:get(muslim..'Abs:Lock:Text'..msg.chat_id_) then
DeleteMessage(msg.chat_id_,{[0] = msg.id_})
end
...
if not DevAbs:get(muslim..'Abs:Spam:Text'..msg.chat_id_) then
sens = 400
else
sens = tonumber(DevAbs:get(muslim..'Abs:Spam:Text'..msg.chat_id_))
end
if DevAbs:get(muslim..'Abs:Lock:Spam'..msg.chat_id_) and string.len(msg.content_.text_) > (sens) or ctrl_chars > (sens) or real_digits > (sens) then
DeleteMessage(msg.chat_id_,{[0] = msg.id_})
end
end
...
--     Source dev yousef     --
if DevAbs:get(muslim.."Abs:Set:Groups:Links"..msg.chat_id_..msg.sender_user_id_) then
if text == "الغاء" then
send(msg.chat_id_,msg.id_,"🇸🇦︙تم الغاء حفظ الرابط")       
DevAbs:del(muslim.."Abs:Set:Groups:Links"..msg.chat_id_..msg.sender_user_id_) 
return false
end
if msg.content_.text_:match("(https://telegram.me/joinchat/%S+)") or msg.content_.text_:match("(https://t.me/joinchat/%S+)") then
local Link = msg.content_.text_:match("(https://telegram.me/joinchat/%S+)") or msg.content_.text_:match("(https://t.me/joinchat/%S+)")
DevAbs:set(muslim.."Abs:Groups:Links"..msg.chat_id_,Link)
...
if DevAbs:get('devyousef:'..muslim.."numadd:user"..msg.chat_id_.."" .. msg.sender_user_id_) then 
if text and text:match("^الغاء$") then 
DevAbs:del('devyousef:'..muslim..'id:user'..msg.chat_id_)  
Dev_Abs(msg.chat_id_, msg.id_, 1, '🇸🇦︙تم الغاء الامر', 1, 'md')
DevAbs:del('devyousef:'..muslim.."numadd:user"..msg.chat_id_.."" .. msg.sender_user_id_)  
return false  end 
DevAbs:del('devyousef:'..muslim.."numadd:user"..msg.chat_id_.."" .. msg.sender_user_id_)  
local numadded = string.match(text, "(%d+)") 
local iduserr = DevAbs:get('devyousef:'..muslim..'id:user'..msg.chat_id_)  
DevAbs:incrby(muslim..'Abs:UsersMsgs'..msg.chat_id_..':'..iduserr,numadded)
Dev_Abs(msg.chat_id_, msg.id_,  1, "🇸🇦︙تم اضافة "..numadded..' رساله', 1, 'md')
DevAbs:del('devyousef:'..muslim..'id:user'..msg.chat_id_) 
end
end
if text and Constructor(msg) then 
if DevAbs:get('devyousef:'..muslim.."nmadd:user"..msg.chat_id_.."" .. msg.sender_user_id_) then 
if text and text:match("^الغاء$") then 
DevAbs:del('devyousef:'..muslim..'ids:user'..msg.chat_id_)  
Dev_Abs(msg.chat_id_, msg.id_, 1, '🇸🇦︙تم الغاء الامر', 1, 'md')
DevAbs:del('devyousef:'..muslim.."nmadd:user"..msg.chat_id_.."" .. msg.sender_user_id_)  
return false  end 
DevAbs:del('devyousef:'..muslim.."nmadd:user"..msg.chat_id_.."" .. msg.sender_user_id_)  
local numadded = string.match(text, "(%d+)") 
local iduserr = DevAbs:get('devyousef:'..muslim..'ids:user'..msg.chat_id_)  
DevAbs:incrby(muslim..'Abs:GamesNumber'..msg.chat_id_..iduserr,numadded)  
...
if not DevAbs:get(muslim.."Abs:Lock:Fshar"..msg.chat_id_) and not VipMem(msg) then
DeleteMessage(msg.chat_id_,{[0] = msg.id_})
...
if DevAbs:get(muslim.."Abs:Lock:Farsi"..msg.chat_id_) and not VipMem(msg) then
DeleteMessage(msg.chat_id_,{[0] = msg.id_})
ReplyStatus(msg,msg.sender_user_id_,"WrongWay","🇸🇦︙ممنوع التكلم بالغه الفارسية هنا")  
end end
if text and (text:match("ڬ") or text:match("ٺ") or text:match("چ") or text:match("ڇ") or text:match("ڿ") or text:match("ڀ") or text:match("ڎ") or text:match("ݫ") or text:match("ژ") or text:match("ڟ") or text:match("ݜ") or text:match("ڸ") or text:match("پ") or text:match("۴") or text:match("مک") or text:match("زدن") or text:match("دخترا") or text:match("دیوث") or text:match("کلیپشن") or text:match("خوششون") or text:match("میدا") or text:match("که") or text:match("بدانیم") or text:match("باید") or text:match("زناشویی") or text:match("آموزش") or text:match("راحتی") or text:match("خسته") or text:match("بیام") or text:match("بپوشم") or text:match("كرمه")) then
if DevAbs:get(muslim.."Abs:Lock:FarsiBan"..msg.chat_id_) and not VipMem(msg) then
DeleteMessage(msg.chat_id_,{[0] = msg.id_})
ChatKick(msg.chat_id_, msg.sender_user_id_)
end end 
if text and (text:match("خره بالله") or text:match("خبربك") or text:match("كسدينربك") or text:match("خرب بالله") or text:match("خرب الله") or text:match("خره بربك") or text:match("الله الكواد") or text:match("خره بمحمد") or text:match("كسم الله") or text:match("كسم ربك") or text:match("كسربك") or text:match("كسختالله") or text:match("كسخت الله") or text:match("خره بدينك") or text:match("خرهبدينك") or text:match("كسالله") or text:match("خربالله")) then
if not DevAbs:get(muslim.."Abs:Lock:Kfr"..msg.chat_id_) and not VipMem(msg) then
DeleteMessage(msg.chat_id_,{[0] = msg.id_})
...
if not DevAbs:get(muslim.."Abs:Lock:Taf"..msg.chat_id_) and not VipMem(msg) then
DeleteMessage(msg.chat_id_,{[0] = msg.id_})
...
local BotName = (DevAbs:get(muslim.."Abs:NameBot") or 'dev-yousef 🇸🇦')
...
Constructors = DevAbs:smembers(muslim..'Abs:BasicConstructor:'..v)
BasicConstructors = DevAbs:smembers(muslim..'Abs:Constructor:'..v)
...
File:write(GetJson)
File:close()
sendDocument(msg.chat_id_, msg.id_, 0, 1, nil, './'..muslim..'.json', '🇸🇦︙يحتوي الملف على -› '..#List..' مجموعة',dl_cb, nil)
io.popen('rm -rf ./'..muslim..'.json')
end
...
--     Source dev yousef     --
if DevAbs:get(muslim.."SET:GAME"..msg.chat_id_) then  
if text and text:match("^(%d+)$") then
local NUM = text:match("^(%d+)$")
if tonumber(NUM) > 6 then
Dev_Abs( msg.chat_id_, msg.id_, 1,"🇸🇦︙يوجد فقط ( 6 ) اختيارات\n🇸🇦︙ارسل اختيارك مره اخرى", 1, "md")    
return false  end 
local GETNUM = DevAbs:get(muslim.."GAMES"..msg.chat_id_)
if tonumber(NUM) == tonumber(GETNUM) then
DevAbs:del(muslim.."SET:GAME"..msg.chat_id_)   
Dev_Abs( msg.chat_id_, msg.id_, 1,'🇸🇦︙*المحيبس باليد رقم* -› '..NUM..'\n🇸🇦︙*مبروك لقد ربحت وحصلت على 5 نقاط يمكنك استبدالها بالرسائل*', 1, "md") 
DevAbs:incrby(muslim..'Abs:GamesNumber'..msg.chat_id_..msg.sender_user_id_,5)  
...
if DevAbs:get(muslim..'DevAbs4'..msg.sender_user_id_) then
if text and text:match("^الغاء$") then 
send(msg.chat_id_, msg.id_, "🇸🇦︙تم الغاء الامر")
DevAbs:del(muslim..'DevAbs4'..msg.sender_user_id_)
return false  end 
DevAbs:del(muslim..'DevAbs4'..msg.sender_user_id_)
...
DevAbs:set(muslim..'Abs:ChId',data.id_)
else
...
--     Source dev yousef     --
if DevAbs:get(muslim.."Abs:DevText"..msg.chat_id_..":" .. msg.sender_user_id_) then
if text and text:match("^الغاء$") then 
DevAbs:del(muslim.."Abs:DevText"..msg.chat_id_..":" .. msg.sender_user_id_)
Dev_Abs(msg.chat_id_, msg.id_, 1, '🇸🇦︙تم الغاء الامر', 1, 'md')
return false 
end 
DevAbs:del(muslim.."Abs:DevText"..msg.chat_id_..":" .. msg.sender_user_id_)
local DevText = msg.content_.text_:match("(.*)")
DevAbs:set(muslim.."DevText", DevText)
Dev_Abs(msg.chat_id_, msg.id_, 1, "🇸🇦︙تم حفظ كليشة المطور", 1, "md")
end
if DevAbs:get(muslim..'Abs:NameBot'..msg.sender_user_id_) == 'msg' then
if text and text:match("^الغاء$") then 
DevAbs:del(muslim..'Abs:NameBot'..msg.sender_user_id_)
Dev_Abs(msg.chat_id_, msg.id_, 1, '🇸🇦︙تم الغاء الامر', 1, 'md')
return false 
end 
Dev_Abs(msg.chat_id_, msg.id_, 1, '🇸🇦︙تم حفظ اسم البوت ', 1, 'html')
DevAbs:del(muslim..'Abs:NameBot'..msg.sender_user_id_)
DevAbs:set(muslim..'Abs:NameBot', text)
return false 
end
--     Source dev yousef     --
if text == "الرابط" then
if not DevAbs:get(muslim.."Abs:Lock:GpLinksinline"..msg.chat_id_) then 
tdcli_function({ID ="GetChat",chat_id_=msg.chat_id_},function(arg,ta) 
local linkgpp = json:decode(https.request('https://api.telegram.org/bot'..TokenBot..'/exportChatInviteLink?chat_id='..msg.chat_id_)) or DevAbs:get(muslim.."Private:Group:Link"..msg.chat_id_) 
...
if text and text == (DevAbs:get(muslim..'Abs:NameBot') or 'dev-yousef 🇸🇦') then 
NameBot = (DevAbs:get(muslim..'Abs:NameBot') or 'dev-yousef 🇸🇦')
...
if tonumber((DevAbs:get(muslim..'Abs:GamesNumber'..msg.chat_id_..msg.sender_user_id_) or 0)) == 0 then
Dev_Abs(msg.chat_id_, msg.id_, 1,'🇸🇦︙لم تربح اي نقطه\n🇸🇦︙ارسل -› الالعاب للعب', 1, 'md')
else 
Dev_Abs(msg.chat_id_, msg.id_, 1,'🇸🇦︙عدد النقاط التي ربحتها -› '..(DevAbs:get(muslim..'Abs:GamesNumber'..msg.chat_id_..msg.sender_user_id_)), 1, 'md')
end
end
if text ==  'حذف رسائلي' and ChCheck(msg) or text ==  'مسح رسائلي' and ChCheck(msg) then DevAbs:del(muslim..'Abs:UsersMsgs'..msg.chat_id_..':'..msg.sender_user_id_) Dev_Abs(msg.chat_id_, msg.id_, 1, '🇸🇦︙تم حذف جميع رسائلك', 1, 'md') end
if text ==  'حذف نقاطي' and ChCheck(msg) or text ==  'مسح نقاطي' and ChCheck(msg) then DevAbs:del(muslim..'Abs:GamesNumber'..msg.chat_id_..msg.sender_user_id_) Dev_Abs(msg.chat_id_, msg.id_, 1, '🇸🇦︙تم حذف جميع نقاطك', 1, 'md') end
--     Source dev yousef     --
if text == 'سمايلات' and ChCheck(msg) or text == 'السمايلات' and ChCheck(msg) then
if not DevAbs:get(muslim..'Abs:Lock:Games'..msg.chat_id_) then
DevAbs2 = {'🍏','🍎','🍐','🍊','🍋','🍌','🍉','🍇','🍓','🍈','🍒','🍑','🍍','🥥','🥝','🍅','🍆','🥑','🥦','🥒','🌶','🌽','🥕','🥔','🍠','🥐','🍞','🥖','🥨','🧀','🥚','🍳','🥞','🥓','🥩','🍗','🍖','🌭','🍔','🍟','🍕','🥪','🥙','🍼','☕️','🍵','🥤','🍶','🍺','🍻','🏀','⚽️','🏈','⚾️','🎾','🏐','🏉','🎱','🏓','🏸','🥅','🎰','🎮','🎳','🎯','🏆','🎻','🎸','🎺','🥁','🎹','🎼','🎧','🎤','🎬','🎨','🎭','🎪','🛎','📤','🎗','🏵','🎖','🏆','🥌','🛷','🚕','🚗','🚙','🚌','🚎','🏎','🚓','🚑','🚚','🚛','🚜','🇮🇶','⚔️','🛡','🔮','🌡','💣','⏱','🛢','📓','📗','📂','📅','📪','📫','📬','📭','⏰','📺','🎚','☎️','📡'}
name = DevAbs2[math.random(#DevAbs2)]
DevAbs:set(muslim..'Abs:GameNum'..msg.chat_id_,name)
DevAbs:del(muslim..'Abs:Games:Ids'..msg.chat_id_)
...
if text == DevAbs:get(muslim..'Abs:GameNum'..msg.chat_id_) and not DevAbs:get(muslim..'Abs:Games:Ids'..msg.chat_id_) then
if not DevAbs:get(muslim..'Abs:Games:Ids'..msg.chat_id_) then 
devyousef = '🇸🇦︙مبروك لقد ربحت في اللعبه \n🇸🇦︙ارسل -› سمايلات للعب مره اخرى'
Dev_Abs(msg.chat_id_, msg.id_, 1,devyousef, 1, 'md')
DevAbs:incrby(muslim..'Abs:GamesNumber'..msg.chat_id_..msg.sender_user_id_, 1)  
end
DevAbs:set(muslim..'Abs:Games:Ids'..msg.chat_id_,true)
end
...
if not DevAbs:get(muslim..'Abs:Lock:Games'..msg.chat_id_) then
Num = math.random(1,6)
DevAbs:set(muslim.."GAMES"..msg.chat_id_,Num) 
TEST = [[
➀     ➁     ➂     ➃     ➄     ➅
↓     ↓     ↓     ↓     ↓     ↓
👊 ‹› 👊🏻 ‹› 👊🏼 ‹› 👊🏽 ‹› 👊🏾 ‹› 👊🏿
...
DevAbs:setex(muslim.."SET:GAME"..msg.chat_id_, 100, true)  
return false  
end end
...
name = DevAbs2[math.random(#DevAbs2)]
DevAbs:set(muslim..'Abs:GameNum2'..msg.chat_id_,name)
DevAbs:del(muslim..'Abs:Games:Ids'..msg.chat_id_)
...
if text == DevAbs:get(muslim..'Abs:GameNum2'..msg.chat_id_) and not DevAbs:get(muslim..'Abs:Games:Ids'..msg.chat_id_) then
if not DevAbs:get(muslim..'Abs:Games:Ids'..msg.chat_id_) then 
...
name = DevAbs2[math.random(#DevAbs2)]
DevAbs:set(muslim..'Abs:GameNum3'..msg.chat_id_,name)
DevAbs:del(muslim..'Abs:Games:Ids'..msg.chat_id_)
...
if text == DevAbs:get(muslim..'Abs:GameNum3'..msg.chat_id_) and not DevAbs:get(muslim..'Abs:Games:Ids'..msg.chat_id_) then
if not DevAbs:get(muslim..'Abs:Games:Ids'..msg.chat_id_) then 
...
DevAbs2 = {'😸','☠','🐼','🐇','🌑','🌚','⭐️','📥','⛈','🌥','⛄️','👨‍🔬','👨‍💻','👨‍🔧','👩‍🍳','🧚‍♀','🧚‍♂️','🧝‍♂','🙍‍♂','🧖‍♂','👬','👨‍👨‍👧','🕓','🕤','⌛️','📅','👩‍⚖️','👨‍🎨'};
name = DevAbs2[math.random(#DevAbs2)]
DevAbs:set(muslim..'Abs:GameNum4'..msg.chat_id_,name)
DevAbs:del(muslim..'Abs:Games:Ids'..msg.chat_id_)
name = string.gsub(name,'😸','😹😹😹😸😹😹😹😹')
name = string.gsub(name,'☠️','💀💀💀☠️💀💀💀💀')
name = string.gsub(name,'🐼','👻👻👻👻👻👻👻🐼')
...
if text == DevAbs:get(muslim..'Abs:GameNum4'..msg.chat_id_) and not DevAbs:get(muslim..'Abs:Games:Ids'..msg.chat_id_) then
if not DevAbs:get(muslim..'Abs:Games:Ids'..msg.chat_id_) then 
...
name = DevAbs2[math.random(#DevAbs2)]
DevAbs:set(muslim..'Abs:GameNum5'..msg.chat_id_,name)
DevAbs:del(muslim..'Abs:Games:Ids'..msg.chat_id_)
...
if text == DevAbs:get(muslim..'Abs:GameNum5'..msg.chat_id_) then
if not DevAbs:get(muslim..'Abs:Games:Ids'..msg.chat_id_) then 
DevAbs:incrby(muslim..'Abs:GamesNumber'..msg.chat_id_..msg.sender_user_id_, 1)  
DevAbs:del(muslim..'Abs:GameNum5'..msg.chat_id_)
...
name = DevAbs2[math.random(#DevAbs2)]
DevAbs:set(muslim..'Abs:GameNum6'..msg.chat_id_,name)
DevAbs:del(muslim..'Abs:Games:Ids'..msg.chat_id_)
...
if text == DevAbs:get(muslim..'Abs:GameNum6'..msg.chat_id_) then
if not DevAbs:get(muslim..'Abs:Games:Ids'..msg.chat_id_) then 
DevAbs:incrby(muslim..'Abs:GamesNumber'..msg.chat_id_..msg.sender_user_id_, 1)  
DevAbs:del(muslim..'Abs:GameNum6'..msg.chat_id_)
...
name = DevAbs2[math.random(#DevAbs2)]
DevAbs:set(muslim..'Abs:GameNum7'..msg.chat_id_,name)
DevAbs:del(muslim..'Abs:Games:Ids'..msg.chat_id_)
...
if text == DevAbs:get(muslim..'Abs:GameNum7'..msg.chat_id_) then
if not DevAbs:get(muslim..'Abs:Games:Ids'..msg.chat_id_) then 
DevAbs:incrby(muslim..'Abs:GamesNumber'..msg.chat_id_..msg.sender_user_id_, 1)  
DevAbs:del(muslim..'Abs:GameNum7'..msg.chat_id_)
...
name = DevAbs2[math.random(#DevAbs2)]
DevAbs:set(muslim..'Abs:GameNum8'..msg.chat_id_,name)
DevAbs:del(muslim..'Abs:Games:Ids'..msg.chat_id_)
...
if text == DevAbs:get(muslim..'Abs:GameNum8'..msg.chat_id_) then
if not DevAbs:get(muslim..'Abs:Games:Ids'..msg.chat_id_) then 
DevAbs:incrby(muslim..'Abs:GamesNumber'..msg.chat_id_..msg.sender_user_id_, 1)  
DevAbs:del(muslim..'Abs:GameNum8'..msg.chat_id_)
...
--     Source dev yousef     --
if DevAbs:get(muslim.."GAME:TKMEN"..msg.chat_id_.."" .. msg.sender_user_id_) then  
if text and text:match("^(%d+)$") then
local NUM = text:match("^(%d+)$")
if tonumber(NUM) > 20 then
Dev_Abs(msg.chat_id_, msg.id_, 1,"🇸🇦︙عذرا لا يمكنك تخمين عدد اكبر من الـ20 خمن رقم ما بين الـ1 والـ20", 1, 'md')
return false  end 
local GETNUM = DevAbs:get(muslim.."GAMES:NUM"..msg.chat_id_)
if tonumber(NUM) == tonumber(GETNUM) then
DevAbs:del(muslim..'Set:Num'..msg.chat_id_..msg.sender_user_id_)
DevAbs:del(muslim.."GAME:TKMEN"..msg.chat_id_.."" .. msg.sender_user_id_)   
DevAbs:incrby(muslim..'Abs:GamesNumber'..msg.chat_id_..msg.sender_user_id_,5)  
Dev_Abs(msg.chat_id_, msg.id_, 1,'🇸🇦︙*التخمين الصحيح هو* -› '..NUM..'\n🇸🇦︙*مبروك لقد ربحت وحصلت على 5 نقاط يمكنك استبدالها بالرسائل*', 1, 'md')
elseif tonumber(NUM) ~= tonumber(GETNUM) then
DevAbs:incrby(muslim..'Set:Num'..msg.chat_id_..msg.sender_user_id_,1)
if tonumber(DevAbs:get(muslim..'Set:Num'..msg.chat_id_..msg.sender_user_id_)) >= 3 then
DevAbs:del(muslim..'Set:Num'..msg.chat_id_..msg.sender_user_id_)
DevAbs:del(muslim.."GAME:TKMEN"..msg.chat_id_.."" .. msg.sender_user_id_)   
Dev_Abs(msg.chat_id_, msg.id_, 1,'🇸🇦︙*التخمين الصحيح هو* -› '..GETNUM..'\n🇸🇦︙*للاسف لقد خسرت حاول مره اخرى لتخمين الرقم الصحيح*', 1, 'md')
else
if tonumber(DevAbs:get(muslim..'Set:Num'..msg.chat_id_..msg.sender_user_id_)) == 1 then
SetNum = 'محاولتان فقط'
elseif tonumber(DevAbs:get(muslim..'Set:Num'..msg.chat_id_..msg.sender_user_id_)) == 2 then
...
DevAbs:set(muslim.."GAMES:NUM"..msg.chat_id_,Num) 
Dev_Abs(msg.chat_id_, msg.id_, 1,'🇸🇦︙اهلا بك عزيزي في لعبة التخمين -› ⤈\n ------------ Y●L ------------\n🇸🇦︙سيتم تخمين عدد ما بين الـ1 والـ20 اذا تعتقد انك تستطيع الفوز جرب واللعب الان .\n🇸🇦︙ملاحظه لديك ثلاث محاولات فقط فكر قبل ارسال تخمينك !', 1, 'md')
DevAbs:setex(muslim.."GAME:TKMEN"..msg.chat_id_.."" .. msg.sender_user_id_, 100, true)  
return false  
end
end
--     Source dev yousef     --
if text == 'روليت' then
if not DevAbs:get(muslim..'Abs:Lock:Games'..msg.chat_id_) then
DevAbs:del(muslim.."Abs:NumRolet"..msg.chat_id_..msg.sender_user_id_) 
DevAbs:del(muslim..'Abs:ListRolet'..msg.chat_id_)  
DevAbs:setex(muslim.."Abs:StartRolet"..msg.chat_id_..msg.sender_user_id_,3600,true)  
Dev_Abs(msg.chat_id_, msg.id_, 1, '🇸🇦︙حسنا لنلعب , ارسل عدد اللاعبين للروليت .', 1, 'md')
return false  
end
end
if text and text:match("^(%d+)$") and DevAbs:get(muslim.."Abs:StartRolet"..msg.chat_id_..msg.sender_user_id_) then
if text == "1" then
Text = "🇸🇦︙لا استطيع بدء اللعبه بلاعب واحد فقط"
else
DevAbs:set(muslim.."Abs:NumRolet"..msg.chat_id_..msg.sender_user_id_,text)  
Text = '🇸🇦︙تم بدء تسجيل اللسته يرجى ارسال المعرفات \n🇸🇦︙الفائز يحصل على 5 نقاط عدد المطلوبين -› '..text..' لاعب'
end
DevAbs:del(muslim.."Abs:StartRolet"..msg.chat_id_..msg.sender_user_id_)
send(msg.chat_id_,msg.id_,Text)
return false
end
if text and text:match('^(@[%a%d_]+)$') and DevAbs:get(muslim.."Abs:NumRolet"..msg.chat_id_..msg.sender_user_id_) then 
if DevAbs:sismember(muslim..'Abs:ListRolet'..msg.chat_id_,text) then
...
DevAbs:sadd(muslim..'Abs:ListRolet'..msg.chat_id_,text)
local CountAdd = DevAbs:get(muslim.."Abs:NumRolet"..msg.chat_id_..msg.sender_user_id_)
local CountAll = DevAbs:scard(muslim..'Abs:ListRolet'..msg.chat_id_)
local CountUser = CountAdd - CountAll
if tonumber(CountAll) == tonumber(CountAdd) then 
DevAbs:del(muslim.."Abs:NumRolet"..msg.chat_id_..msg.sender_user_id_) 
DevAbs:setex(muslim.."Abs:WittingStartRolet"..msg.chat_id_..msg.sender_user_id_,1400,true) 
...
if not DevAbs:get(muslim..'Abs:Lock:Gamesinline'..msg.chat_id_) then
Text =[[
...
DevAbs0 = (DevAbs:get(muslim..'Abs:GamesNumber'..msg.chat_id_..msg.sender_user_id_) * 50)
DevAbs:incrby(muslim..'Abs:UsersMsgs'..msg.chat_id_..':'..msg.sender_user_id_,DevAbs0)
Dev_Abs(msg.chat_id_, msg.id_, 1,'🇸🇦︙تم بيع '..(DevAbs:get(muslim..'Abs:GamesNumber'..msg.chat_id_..msg.sender_user_id_))..' من نقاطك\n🇸🇦︙كل نقطه تساوي 50 رساله', 'md')
DevAbs:del(muslim..'Abs:GamesNumber'..msg.chat_id_..msg.sender_user_id_)
end
end
--     Source dev yousef     --
...
DevAbs:sadd(muslim..'Abs:BasicConstructor:'..msg.chat_id_,Manager_id)  
DevAbs:sadd(muslim..'Abs:AbsConstructor:'..msg.chat_id_,Manager_id)   
end  
end  
if num == 0 then
...
if DevAbs:get(muslim.."Abs:Left:Bot"..muslim) and not SecondSudo(msg) then
...
local msguser = tonumber(DevAbs:get(muslim..'Abs:UsersMsgs'..msg.chat_id_..':'..msg.sender_user_id_))
local user_msgs = DevAbs:get(muslim..'Abs:UsersMsgs'..msg.chat_id_..':'..msg.sender_user_id_)
local cont = (tonumber(DevAbs:get(muslim..'Abs:ContactNumber'..msg.chat_id_..':'..msg.sender_user_id_)) or 0)
local user_nkt = tonumber(DevAbs:get(muslim..'Abs:GamesNumber'..msg.chat_id_..msg.sender_user_id_) or 0)
if result.username_ then username = '@'..result.username_ else username = 'لا يوجد' end
...
DevAbs:setex(muslim..'DevAbs4'..msg.sender_user_id_,360,true)
...
function devyousef(extra,result,success)
if DevAbs:get(muslim.."Abs:Send:Bot"..muslim) and not SecondSudo(msg) then 
send(msg.chat_id_, msg.id_,"🇸🇦︙الاذاعه معطله من قبل المطور الاساسي")
return false
end
local GpList = DevAbs:smembers(muslim.."Abs:Groups")
for k,v in pairs(GpList) do
tdcli_function({ID="ForwardMessages", chat_id_ = v, from_chat_id_ = msg.chat_id_, message_ids_ = {[0] = result.id_}, disable_notification_ = 0, from_background_ = 1},function(a,t) end,nil) 
end
local PvList = DevAbs:smembers(muslim.."Abs:Users")
...
DevAbs:set(muslim..'Abs:viewget'..msg.sender_user_id_,true)
...
if DevAbs:get(muslim.."Abs:Kick:Me"..msg.chat_id_) then
Dev_Abs(msg.chat_id_, msg.id_, 1, '🇸🇦︙عذرا هذه الخاصيه معطله ', 1, 'md')
return false
end
DevAbs:set(muslim..'yes'..msg.sender_user_id_, 'delyes')
DevAbs:set(muslim..'no'..msg.sender_user_id_, 'delno')
...
DevAbs:set(muslim.."Abs:Kick:Me"..msg.chat_id_, true)
local devyousef = '🇸🇦︙اهلا عزيزي -› '..AbsRank(msg)..' \n🇸🇦︙تم تعطيل امر اطردني'
absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, devyousef, 14, string.len(msg.sender_user_id_))
end
if text == 'تفعيل اطردني' and Manager(msg) and ChCheck(msg) then
DevAbs:del(muslim.."Abs:Kick:Me"..msg.chat_id_)
local devyousef = '🇸🇦︙اهلا عزيزي -› '..AbsRank(msg)..' \n🇸🇦︙تم تفعيل امر اطردني'
absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, devyousef, 14, string.len(msg.sender_user_id_))
end
--     Source dev yousef     --
if text == "نزلني" and ChCheck(msg) then
if DevAbs:get(muslim.."Abs:Del:Me"..msg.chat_id_) then
Dev_Abs(msg.chat_id_, msg.id_, 1, '🇸🇦︙عذرا هذه الخاصيه معطله ', 1, 'md')
return false
end
DevAbs:set(muslim..'yesdel'..msg.sender_user_id_, 'delyes')
DevAbs:set(muslim..'nodel'..msg.sender_user_id_, 'delno')
...
DevAbs:set(muslim.."Abs:Del:Me"..msg.chat_id_, true)
local devyousef = '🇸🇦︙اهلا عزيزي -› '..AbsRank(msg)..' \n🇸🇦︙تم تعطيل امر نزلني'
absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, devyousef, 14, string.len(msg.sender_user_id_))
end
if text == 'تفعيل نزلني' and BasicConstructor(msg) and ChCheck(msg) then
DevAbs:del(muslim.."Abs:Del:Me"..msg.chat_id_)
local devyousef = '🇸🇦︙اهلا عزيزي -› '..AbsRank(msg)..' \n🇸🇦︙تم تفعيل امر نزلني'
absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, devyousef, 14, string.len(msg.sender_user_id_))
end
--     Source dev yousef     --
...
absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, devyousef, 14, string.len(msg.sender_user_id_))
DevAbs:del(muslim..'Abs:Lock:TagAll'..msg.chat_id_)
end
...
absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, devyousef, 14, string.len(msg.sender_user_id_))
DevAbs:set(muslim..'Abs:Lock:TagAll'..msg.chat_id_,true)
end
if Admin(msg) then
if text == "تاك للكل" and ChCheck(msg) then
if not DevAbs:get(muslim..'Abs:Lock:TagAll'..msg.chat_id_) then
function TagAll(dp1,dp2)
local text = "🇸🇦︙وينكم يالربع \n------------ Y●L ------------\n"
i = 0
for k, v in pairs(dp2.members_) do
i = i + 1
if DevAbs:get(muslim..'Save:UserName'..v.user_id_) then
text = text..i.."~ : [@"..DevAbs:get(muslim..'Save:UserName'..v.user_id_).."]\n"
else
...
local EntryNumber = (DevAbs:get(muslim..'Abs:EntryNumber'..msg.chat_id_..':'..os.date('%d')) or 0)
local ExitNumber = (DevAbs:get(muslim..'Abs:ExitNumber'..msg.chat_id_..':'..os.date('%d')) or 0)
local MsgNumberDay = (DevAbs:get(muslim..'Abs:MsgNumberDay'..msg.chat_id_..':'..os.date('%d')) or 0)
...
if text == "جهاتي" and ChCheck(msg) or text == "اضافاتي" and ChCheck(msg) then add = (tonumber(DevAbs:get(muslim..'Abs:ContactNumber'..msg.chat_id_..':'..msg.sender_user_id_)) or 0) Dev_Abs(msg.chat_id_, msg.id_, 1, "🇸🇦︙عدد جهاتك المضافة -› *❨ "..add.." ❩* ", 1, 'md') end
if text == "تعديلاتي" or text == "تعديلاتي" and ChCheck(msg) then local edit_msg = DevAbs:get(muslim..'Abs:EditMsg'..msg.chat_id_..msg.sender_user_id_) or 0  Dev_Abs(msg.chat_id_, msg.id_, 1, "🇸🇦︙عدد تعديلاتك -› *❨ "..edit_msg.." ❩* ", 1, 'md') end
...
if text == 'مسح تعديلاتي' or text == 'مسح تعديلاتي' or text == 'حذف تعديلاتي' or text == 'حذف تعديلاتي' then DevAbs:del(muslim..'Abs:EditMsg'..msg.chat_id_..msg.sender_user_id_) Dev_Abs(msg.chat_id_, msg.id_, 1, '🇸🇦︙تم حذف جميع تعديلاتك بنجاح' , 1, 'md') end
if text == 'مسح جهاتي' or text == 'مسح اضافاتي' or text == 'حذف جهاتي' or text == 'حذف اضافاتي' then DevAbs:del(muslim..'Abs:ContactNumber'..msg.chat_id_..':'..msg.sender_user_id_) Dev_Abs(msg.chat_id_, msg.id_, 1, '🇸🇦︙تم حذف جميع جهاتك المضافة' , 1, 'md') end
--     Source dev yousef     --
if text == "المطور" then 
local DevText = DevAbs:get(muslim.."DevText")
if DevAbs:get(muslim.."Abs:ChId") then local Check = https.request('https://api.telegram.org/bot'..TokenBot..'/getChat?chat_id='..DevAbs:get(muslim.."Abs:ChId")) local GetInfo = JSON.decode(Check) local DevCh1 = GetInfo.result.username DevCh = '\n🇸🇦︙*Dev Ch* ↬ [@'..DevCh1..']' else DevCh = '' end
...
if not DevAbs:get(muslim..'Abs:Lock:Stupid'..msg.chat_id_) then
local username = text:match('^هينه @(.*)') or text:match('^هينها @(.*)') 
function devyousef(extra,result,success)
if result.id_ then  
if tonumber(result.id_) == tonumber(muslim) then  
...
if DevAbs:sismember(muslim.."Abs:AbsConstructor:"..msg.chat_id_,result.id_) then
Dev_Abs(msg.chat_id_, msg.id_, 1, 'دي لكك تريد اهينن تاج راسكك؟😏🖕🏿', 1, 'md')
return false
end 
...
if tonumber(result.sender_user_id_) == tonumber(muslim) then 
Dev_Abs(msg.chat_id_, msg.id_, 1, 'شو تمضرط اكو واحد يهين نفسه؟🤔👌🏿', 1, 'md') 
return false  
end  
...
if DevAbs:sismember(muslim.."Abs:AbsConstructor:"..msg.chat_id_,result.sender_user_id_) then
Dev_Abs(msg.chat_id_, msg.id_, 1, 'دي لكك تريد اهينن تاج راسكك؟😏🖕🏿', 1, 'md')
return false
end 
local devyousef = "صارر ستاذيي 🏃🏻‍♂️♥️" 
...
if DevAbs:sismember(muslim..'Abs:SecondSudo:',result.sender_user_id_) then
secondsudo = 'المطورين الثانويين • ' else secondsudo = '' end
if DevAbs:sismember(muslim..'Abs:SudoBot:',result.sender_user_id_) then
sudobot = 'المطورين • ' else sudobot = '' end
if DevAbs:sismember(muslim..'Abs:ManagerAll:',result.sender_user_id_) then
managerall = 'المدراء العامين • ' else managerall = '' end
if DevAbs:sismember(muslim..'Abs:AdminAll:',result.sender_user_id_) then
adminall = 'الادامن العامين • ' else adminall = '' end
if DevAbs:sismember(muslim..'Abs:VipAll:',result.sender_user_id_) then
vpall = 'المميزين العامين • ' else vpall = '' end
if DevAbs:sismember(muslim..'Abs:BasicConstructor:'..msg.chat_id_, result.sender_user_id_) then
basicconstructor = 'المنشئين الاساسيين • ' else basicconstructor = '' end
if DevAbs:sismember(muslim..'Abs:Constructor:'..msg.chat_id_, result.sender_user_id_) then
constructor = 'المنشئين • ' else constructor = '' end 
if DevAbs:sismember(muslim..'Abs:Managers:'..msg.chat_id_, result.sender_user_id_) then
manager = 'المدراء • ' else manager = '' end
if DevAbs:sismember(muslim..'Abs:Admins:'..msg.chat_id_, result.sender_user_id_) then
admins = 'الادامن • ' else admins = '' end
if DevAbs:sismember(muslim..'Abs:VipMem:'..msg.chat_id_, result.sender_user_id_) then
vipmem = 'المميزين • ' else vipmem = '' end
if DevAbs:sismember(muslim..'Abs:Cleaner:'..msg.chat_id_, result.sender_user_id_) then
...
DevAbs:srem(muslim..'Abs:SecondSudo:', result.sender_user_id_)
DevAbs:srem(muslim..'Abs:SudoBot:', result.sender_user_id_)
DevAbs:srem(muslim..'Abs:ManagerAll:', result.sender_user_id_)
DevAbs:srem(muslim..'Abs:AdminAll:', result.sender_user_id_)
DevAbs:srem(muslim..'Abs:VipAll:', result.sender_user_id_)
DevAbs:srem(muslim..'Abs:BasicConstructor:'..msg.chat_id_,result.sender_user_id_)
DevAbs:srem(muslim..'Abs:Constructor:'..msg.chat_id_,result.sender_user_id_)
DevAbs:srem(muslim..'Abs:Managers:'..msg.chat_id_, result.sender_user_id_)
DevAbs:srem(muslim..'Abs:Admins:'..msg.chat_id_, result.sender_user_id_)
DevAbs:srem(muslim..'Abs:VipMem:'..msg.chat_id_, result.sender_user_id_)
DevAbs:srem(muslim..'Abs:Cleaner:'..msg.chat_id_, result.sender_user_id_)
...
if DevAbs:sismember(muslim..'Abs:SecondSudo:',result.id_) then
secondsudo = 'المطورين الثانويين • ' else secondsudo = '' end
if DevAbs:sismember(muslim..'Abs:SudoBot:',result.id_) then
sudobot = 'المطورين • ' else sudobot = '' end
if DevAbs:sismember(muslim..'Abs:ManagerAll:',result.id_) then
managerall = 'المدراء العامين • ' else managerall = '' end
if DevAbs:sismember(muslim..'Abs:AdminAll:',result.id_) then
adminall = 'الادامن العامين • ' else adminall = '' end
if DevAbs:sismember(muslim..'Abs:VipAll:',result.id_) then
vpall = 'المميزين العامين • ' else vpall = '' end
if DevAbs:sismember(muslim..'Abs:BasicConstructor:'..msg.chat_id_, result.id_) then
basicconstructor = 'المنشئين الاساسيين • ' else basicconstructor = '' end
if DevAbs:sismember(muslim..'Abs:Constructor:'..msg.chat_id_, result.id_) then
constructor = 'المنشئين • ' else constructor = '' end 
if DevAbs:sismember(muslim..'Abs:Managers:'..msg.chat_id_, result.id_) then
manager = 'المدراء • ' else manager = '' end
if DevAbs:sismember(muslim..'Abs:Admins:'..msg.chat_id_, result.id_) then
admins = 'الادامن • ' else admins = '' end
if DevAbs:sismember(muslim..'Abs:VipMem:'..msg.chat_id_, result.id_) then
vipmem = 'المميزين • ' else vipmem = '' end
if DevAbs:sismember(muslim..'Abs:Cleaner:'..msg.chat_id_, result.id_) then
cleaner = 'المنظفين • ' else cleaner = ''
end
...
if AbsDelAll(msg.sender_user_id_,msg.chat_id_) == 'sudoid' then
DevAbs:srem(muslim..'Abs:SecondSudo:', result.id_)
DevAbs:srem(muslim..'Abs:SudoBot:', result.id_)
DevAbs:srem(muslim..'Abs:ManagerAll:', result.id_)
DevAbs:srem(muslim..'Abs:AdminAll:', result.id_)
DevAbs:srem(muslim..'Abs:VipAll:', result.id_)
DevAbs:srem(muslim..'Abs:BasicConstructor:'..msg.chat_id_,result.id_)
DevAbs:srem(muslim..'Abs:Constructor:'..msg.chat_id_,result.id_)
DevAbs:srem(muslim..'Abs:Managers:'..msg.chat_id_, result.id_)
DevAbs:srem(muslim..'Abs:Admins:'..msg.chat_id_, result.id_)
DevAbs:srem(muslim..'Abs:VipMem:'..msg.chat_id_, result.id_)
DevAbs:srem(muslim..'Abs:Cleaner:'..msg.chat_id_, result.id_)
elseif AbsDelAll(msg.sender_user_id_,msg.chat_id_) == 'secondsudo' then
DevAbs:srem(muslim..'Abs:SudoBot:', result.id_)
DevAbs:srem(muslim..'Abs:ManagerAll:', result.id_)
DevAbs:srem(muslim..'Abs:AdminAll:', result.id_)
...
DevAbs:sadd(muslim..'Abs:SecondSudo:',result.sender_user_id_)
...
if result.id_ then
DevAbs:sadd(muslim..'Abs:SecondSudo:',result.id_)
...
DevAbs:sadd(muslim..'Abs:SecondSudo:',user)
...
DevAbs:srem(muslim..'Abs:SecondSudo:',user)
...
function sudo_reply(extra, result, success)
DevAbs:sadd(muslim..'Abs:SudoBot:',result.sender_user_id_)
...
function promreply(extra,result,success)
if result.id_ then
DevAbs:sadd(muslim..'Abs:SudoBot:',result.id_)
...
DevAbs:sadd(muslim..'Abs:SudoBot:',user)
...
DevAbs:srem(muslim..'Abs:SudoBot:',user)
...
DevAbs:sadd(muslim..'Abs:ManagerAll:',result.sender_user_id_)
...
function promreply(extra,result,success)
if result.id_ then
DevAbs:sadd(muslim..'Abs:ManagerAll:',result.id_)
...
DevAbs:sadd(muslim..'Abs:ManagerAll:',user)
...
DevAbs:srem(muslim..'Abs:ManagerAll:',user)
...
function raf_reply(extra, result, success)
DevAbs:sadd(muslim..'Abs:AdminAll:',result.sender_user_id_)
...
function promreply(extra,result,success)
if result.id_ then
DevAbs:sadd(muslim..'Abs:AdminAll:',result.id_)
...
DevAbs:sadd(muslim..'Abs:AdminAll:',user)
...
DevAbs:srem(muslim..'Abs:AdminAll:',user)
...
function raf_reply(extra, result, success)
DevAbs:sadd(muslim..'Abs:VipAll:',result.sender_user_id_)
...
function promreply(extra,result,success)
if result.id_ then
DevAbs:sadd(muslim..'Abs:VipAll:',result.id_)
...
DevAbs:sadd(muslim..'Abs:VipAll:',user)
...
DevAbs:srem(muslim..'Abs:VipAll:',user)
...
function raf_reply(extra, result, success)
DevAbs:sadd(muslim..'Abs:AbsConstructor:'..msg.chat_id_,result.sender_user_id_)
...
function promreply(extra,result,success)
if result.id_ then
DevAbs:sadd(muslim..'Abs:AbsConstructor:'..msg.chat_id_,result.id_)
...
DevAbs:sadd(muslim..'Abs:AbsConstructor:'..msg.chat_id_,user)
...
DevAbs:srem(muslim..'Abs:AbsConstructor:'..msg.chat_id_,result.sender_user_id_)
...
DevAbs:srem(muslim..'Abs:AbsConstructor:'..msg.chat_id_,result.id_)
...
DevAbs:srem(muslim..'Abs:AbsConstructor:'..msg.chat_id_,user)
...
function raf_reply(extra, result, success)
DevAbs:sadd(muslim..'Abs:BasicConstructor:'..msg.chat_id_,result.sender_user_id_)
...
function promreply(extra,result,success)
if result.id_ then
DevAbs:sadd(muslim..'Abs:BasicConstructor:'..msg.chat_id_,result.id_)
...
DevAbs:sadd(muslim..'Abs:BasicConstructor:'..msg.chat_id_,user)
...
DevAbs:srem(muslim..'Abs:BasicConstructor:'..msg.chat_id_,user)
...
function raf_reply(extra, result, success)
DevAbs:sadd(muslim..'Abs:Constructor:'..msg.chat_id_,result.sender_user_id_)
...
function promreply(extra,result,success)
if result.id_ then
DevAbs:sadd(muslim..'Abs:Constructor:'..msg.chat_id_,result.id_)
...
DevAbs:sadd(muslim..'Abs:Constructor:'..msg.chat_id_,user)
...
DevAbs:srem(muslim..'Abs:Constructor:'..msg.chat_id_,user)
...
DevAbs:sadd(muslim..'Abs:Managers:'..msg.chat_id_,user)
...
DevAbs:srem(muslim..'Abs:Managers:'..msg.chat_id_,user)
...
function prom_reply(extra, result, success)
DevAbs:sadd(muslim..'Abs:Cleaner:'..msg.chat_id_,result.sender_user_id_)
...
function promreply(extra,result,success)
if result.id_ then
DevAbs:sadd(muslim..'Abs:Cleaner:'..msg.chat_id_,result.id_)
...
DevAbs:sadd(muslim..'Abs:Cleaner:'..msg.chat_id_,user)
...
DevAbs:srem(muslim..'Abs:Cleaner:'..msg.chat_id_,user)
...
function prom_reply(extra, result, success)
if not BasicConstructor(msg) and DevAbs:get(muslim.."Abs:Lock:ProSet"..msg.chat_id_) then 
...
DevAbs:sadd(muslim..'Abs:Admins:'..msg.chat_id_,user)
...
DevAbs:srem(muslim..'Abs:Admins:'..msg.chat_id_,user)
...
DevAbs:sadd(muslim..'Abs:VipMem:'..msg.chat_id_,user)
...
DevAbs:srem(muslim..'Abs:VipMem:'..msg.chat_id_,user)
...
function promote_by_reply(extra, result, success)
local Check = https.request('https://api.telegram.org/bot'..TokenBot..'/getChatMember?chat_id='..msg.chat_id_..'&user_id='..muslim)
local GetInfo = JSON.decode(Check)
...
DevAbs:del(muslim.."Abs:Lock:ProSet"..msg.chat_id_)
...
DevAbs:set(muslim.."Abs:Lock:ProSet"..msg.chat_id_,"true")
...
if not Constructor(msg) and DevAbs:get(muslim.."Abs:Lock:KickBan"..msg.chat_id_) then 
...
ChatKick(result.chat_id_, result.sender_user_id_)
DevAbs:sadd(muslim..'Abs:Ban:'..msg.chat_id_, result.sender_user_id_)
...
ChatKick(msg.chat_id_, result.id_)
DevAbs:sadd(muslim..'Abs:Ban:'..msg.chat_id_, result.id_)
...
ChatKick(msg.chat_id_, user)
DevAbs:sadd(muslim..'Abs:Ban:'..msg.chat_id_, user)
...
DevAbs:srem(muslim..'Abs:Ban:'..msg.chat_id_, result.sender_user_id_)
...
if result.id_ then
DevAbs:srem(muslim..'Abs:Ban:'..msg.chat_id_, result.id_)
...
DevAbs:srem(muslim..'Abs:Ban:'..msg.chat_id_, user)
...
if not Constructor(msg) and DevAbs:get(muslim.."Abs:Lock:MuteTked"..msg.chat_id_) then 
...
if DevAbs:sismember(muslim..'Abs:Muted:'..msg.chat_id_, result.sender_user_id_) then
ReplyStatus(msg,result.sender_user_id_,"Reply","🇸🇦︙هو بالفعل مكتوم من المجموعة")  
else
DevAbs:sadd(muslim..'Abs:Muted:'..msg.chat_id_, result.sender_user_id_)
...
if DevAbs:sismember(muslim..'Abs:Muted:'..msg.chat_id_, result.id_) then
ReplyStatus(msg,result.id_,"Reply","🇸🇦︙هو بالفعل مكتوم من المجموعة")  
else
DevAbs:sadd(muslim..'Abs:Muted:'..msg.chat_id_, result.id_)
...
if DevAbs:sismember(muslim..'Abs:Muted:'..msg.chat_id_, user) then
ReplyStatus(msg,user,"Reply","🇸🇦︙هو بالفعل مكتوم من المجموعة")  
else
DevAbs:sadd(muslim..'Abs:Muted:'..msg.chat_id_, user)
...
if not DevAbs:sismember(muslim..'Abs:Muted:'..msg.chat_id_, result.sender_user_id_) then
ReplyStatus(msg,result.sender_user_id_,"Reply","🇸🇦︙هو ليس مكتوم لالغاء كتمه")  
else
DevAbs:srem(muslim..'Abs:Muted:'..msg.chat_id_, result.sender_user_id_)
...
if result.id_ then
if not DevAbs:sismember(muslim..'Abs:Muted:'..msg.chat_id_, result.id_) then
ReplyStatus(msg,result.id_,"Reply","🇸🇦︙هو ليس مكتوم لالغاء كتمه")  
else
DevAbs:srem(muslim..'Abs:Muted:'..msg.chat_id_, result.id_)
...
if not DevAbs:sismember(muslim..'Abs:Muted:'..msg.chat_id_, user) then
ReplyStatus(msg,user,"Reply","🇸🇦︙هو ليس مكتوم لالغاء كتمه")  
else
DevAbs:srem(muslim..'Abs:Muted:'..msg.chat_id_, user)
...
DevAbs:sadd(muslim..'Abs:Tkeed:'..msg.chat_id_, result.sender_user_id_)
...
DevAbs:sadd(muslim..'Abs:Tkeed:'..msg.chat_id_, result.id_)
...
DevAbs:sadd(muslim..'Abs:Tkeed:'..msg.chat_id_, user)
...
DevAbs:srem(muslim..'Abs:Tkeed:'..msg.chat_id_, result.sender_user_id_)
...
DevAbs:srem(muslim..'Abs:Tkeed:'..msg.chat_id_, result.id_)
...
DevAbs:srem(muslim..'Abs:Tkeed:'..msg.chat_id_, user)
...
if DevAbs:sismember(muslim..'Abs:SecondSudo:',result.sender_user_id_) and not Sudo(msg) then
Dev_Abs(msg.chat_id_, msg.id_, 1, "🇸🇦︙*لاتستطيع حظر المطور الثانوي*", 1, 'md')
return false 
end
ChatKick(result.chat_id_, result.sender_user_id_)
DevAbs:sadd(muslim..'Abs:BanAll:', result.sender_user_id_)
...
if DevAbs:sismember(muslim..'Abs:SecondSudo:',result.id_) and not Sudo(msg) then
Dev_Abs(msg.chat_id_, msg.id_, 1, "🇸🇦︙*لاتستطيع حظر المطور الثانوي*", 1, 'md')
return false 
end
if result.id_ then
ChatKick(msg.chat_id_, result.id_)
DevAbs:sadd(muslim..'Abs:BanAll:', result.id_)
...
if tonumber(user) == tonumber(muslim) then  
Dev_Abs(msg.chat_id_, msg.id_, 1, "🇸🇦︙*لاتستطيع حظر البوت عام*", 1, 'md')
return false 
end
...
if DevAbs:sismember(muslim..'Abs:SecondSudo:',user) and not Sudo(msg) then
Dev_Abs(msg.chat_id_, msg.id_, 1, "🇸🇦︙*لاتستطيع حظر المطور الثانوي*", 1, 'md')
return false 
end
ChatKick(msg.chat_id_, user)
DevAbs:sadd(muslim..'Abs:BanAll:', user)
...
DevAbs:sadd(muslim..'Abs:MuteAll:', result.sender_user_id_)
...
if result.id_ then
DevAbs:sadd(muslim..'Abs:MuteAll:', result.id_)
...
DevAbs:sadd(muslim..'Abs:MuteAll:', user)
...
DevAbs:srem(muslim..'Abs:BanAll:', result.sender_user_id_)
DevAbs:srem(muslim..'Abs:MuteAll:', result.sender_user_id_)
...
if result.id_ then
DevAbs:srem(muslim..'Abs:BanAll:', result.id_)
DevAbs:srem(muslim..'Abs:MuteAll:', result.id_)
...
DevAbs:srem(muslim..'Abs:BanAll:', user)
DevAbs:srem(muslim..'Abs:MuteAll:', user)
...
DevAbs:setex(muslim.."Abs:EditDev"..msg.sender_user_id_,300,true)
end
if DevAbs:get(muslim.."Abs:EditDev"..msg.sender_user_id_) then
if text and text:match("^الغاء$") then 
send(msg.chat_id_, msg.id_,'🇸🇦︙تم الغاء امر تغير المطور الاساسي')
DevAbs:del(muslim.."Abs:EditDev"..msg.sender_user_id_)
return false
end
if text and text:match("^(%d+)$") then 
...
if dp.first_name_ ~= false then
DevAbs:del(muslim.."Abs:EditDev"..msg.sender_user_id_)
DevAbs:set(muslim.."Abs:NewDev"..msg.sender_user_id_,dp.id_)
...
--     Source dev yousef     --
if msg.reply_to_message_id_ ~= 0 then
if text and text:match("^رفع مطي$") and not DevAbs:get(muslim..'Abs:Lock:Stupid'..msg.chat_id_) and ChCheck(msg) then
function donky_by_reply(extra, result, success)
if DevAbs:sismember(muslim..'User:Donky:'..msg.chat_id_, result.sender_user_id_) then
ReplyStatus(msg,result.sender_user_id_,"Reply","🇸🇦︙هو مطي شرفع منه بعد😹💔") 
else
ReplyStatus(msg,result.sender_user_id_,"Reply","🇸🇦︙تم رفعه في قائمة المطايا") 
DevAbs:sadd(muslim..'User:Donky:'..msg.chat_id_, result.sender_user_id_)
end end
getMessage(msg.chat_id_, msg.reply_to_message_id_,donky_by_reply)
end end
--     Source dev yousef     --
if msg.reply_to_message_id_ ~= 0  then
if text and text:match("^تنزيل مطي$") and not DevAbs:get(muslim..'Abs:Lock:Stupid'..msg.chat_id_) and ChCheck(msg) then
function donky_by_reply(extra, result, success)
if not DevAbs:sismember(muslim..'User:Donky:'..msg.chat_id_, result.sender_user_id_) then
ReplyStatus(msg,result.sender_user_id_,"Reply","🇸🇦︙هو ليس مطي ليتم تنزيله") 
else
DevAbs:srem(muslim..'User:Donky:'..msg.chat_id_, result.sender_user_id_)
...
DevAbs:set('devyousef:'..muslim..'id:user'..msg.chat_id_,TXT)  
DevAbs:setex('devyousef:'..muslim.."numadd:user"..msg.chat_id_.."" .. msg.sender_user_id_, 300, true)  
...
DevAbs:del(muslim..'Abs:UsersMsgs'..msg.chat_id_..':'..result.sender_user_id_) 
DevAbs:incrby(muslim..'Abs:UsersMsgs'..msg.chat_id_..':'..result.sender_user_id_,Num) 
...
DevAbs:set('devyousef:'..muslim..'ids:user'..msg.chat_id_,TXT)  
DevAbs:setex('devyousef:'..muslim.."nmadd:user"..msg.chat_id_.."" .. msg.sender_user_id_, 300, true)  
...
function Reply(extra, result, success)
DevAbs:incrby(muslim..'Abs:GamesNumber'..msg.chat_id_..result.sender_user_id_,Num) 
Dev_Abs(msg.chat_id_, msg.id_, 1, "🇸🇦︙تم اضافة "..Num..' نقطه', 1, 'md') 
end
tdcli_function ({ID = "GetMessage",chat_id_=msg.chat_id_,message_id_=tonumber(msg.reply_to_message_id_)},Reply, nil)
return false
end
if DevAbs:get(muslim..'Abs:Lock:Clean'..msg.chat_id_) then if msg.content_.video_ or msg.content_.document_ or msg.content_.sticker_ or msg.content_.photo_ or msg.content_.animation_ then if msg.reply_to_message_id_ ~= 0 then DevAbs:sadd(muslim.."Abs:cleaner"..msg.chat_id_, msg.id_) else DevAbs:sadd(muslim.."Abs:cleaner"..msg.chat_id_, msg.id_) end end end
if Manager(msg) and msg.reply_to_message_id_ ~= 0 then
if text and text:match("^تثبيت$") and ChCheck(msg) then 
if DevAbs:sismember(muslim.."Abs:Lock:Pinpin",msg.chat_id_) and not BasicConstructor(msg) then
...
if data.ID == "Ok" then
DevAbs:set(muslim..'Abs:PinnedMsg'..msg.chat_id_,msg.reply_to_message_id_)
...
local List = DevAbs:smembers(muslim..'Abs:VipMem:'..msg.chat_id_)
text = "🇸🇦︙قائمة المميزين -› ⤈ \n------------ Y●L ------------\n"
for k,v in pairs(List) do
local username = DevAbs:get(muslim..'Save:UserName'..v)
if username then
...
local List = DevAbs:smembers(muslim..abs)
text = "🇸🇦︙قائمة الادامن -› ⤈ \n------------ Y●L ------------\n"
for k,v in pairs(List) do
local username = DevAbs:get(muslim..'Save:UserName'..v)
if username then
...
local List = DevAbs:smembers(muslim..'Abs:Managers:'..msg.chat_id_)
text = "🇸🇦︙قائمة المدراء -› ⤈ \n------------ Y●L ------------\n"
for k,v in pairs(List) do
local username = DevAbs:get(muslim..'Save:UserName'..v)
if username then
...
local List = DevAbs:smembers(muslim..'Abs:Cleaner:'..msg.chat_id_)
text = "🇸🇦︙قائمة المنظفين -› ⤈ \n------------ Y●L ------------\n"
for k,v in pairs(List) do
local username = DevAbs:get(muslim..'Save:UserName'..v)
if username then
...
local List = DevAbs:smembers(muslim..'Abs:Constructor:'..msg.chat_id_)
text = "🇸🇦︙قائمة المنشئين -› ⤈ \n------------ Y●L ------------\n"
for k,v in pairs(List) do
local username = DevAbs:get(muslim..'Save:UserName'..v)
if username then
...
local List = DevAbs:smembers(muslim..'Abs:AbsConstructor:'..msg.chat_id_)
text = "🇸🇦︙قائمة المالكين -› ⤈ \n------------ Y●L ------------\n"
for k,v in pairs(List) do
local username = DevAbs:get(muslim..'Save:UserName'..v)
if username then
...
local List = DevAbs:smembers(muslim..'Abs:BasicConstructor:'..msg.chat_id_)
...
local List = DevAbs:smembers(muslim..'Abs:Muted:'..msg.chat_id_)
...
local List = DevAbs:smembers(muslim..'Abs:Tkeed:'..msg.chat_id_)
text = "🇸🇦︙قائمة المقيدين -› ⤈ \n------------ Y●L ------------\n"
for k,v in pairs(List) do
local username = DevAbs:get(muslim..'Save:UserName'..v)
if username then
...
local List = DevAbs:smembers(muslim..'Abs:Ban:'..msg.chat_id_)
...
local List = DevAbs:hkeys(muslim..'Abs:Filters:'..msg.chat_id_)
text = "🇸🇦︙قائمة المنع -› ⤈ \n------------ Y●L ------------\n"
for k, v in pairs(List) do
text = text..k..'~ ❨ '..v..' ❩\n'
end
...
local List = DevAbs:smembers(muslim..'User:Donky:'..msg.chat_id_)
...
local List = DevAbs:smembers(muslim..'Abs:SecondSudo:')
...
local BanAll = DevAbs:smembers(muslim..'Abs:BanAll:')
local MuteAll = DevAbs:smembers(muslim..'Abs:MuteAll:')
if #BanAll ~= 0 then 
...
local List = DevAbs:smembers(muslim..'Abs:SudoBot:')
text = "🇸🇦︙قائمة المطورين -› ⤈ \n------------ Y●L ------------\n"
for k,v in pairs(List) do
local sudouser = DevAbs:get(muslim..'Abs:Sudos'..v) 
local username = DevAbs:get(muslim..'Save:UserName'..v)
if username then
...
local List = DevAbs:smembers(muslim..'Abs:ManagerAll:')
...
local List = DevAbs:smembers(muslim..'Abs:VipAll:')
...
DevAbs:sadd(muslim.."Abs:AbsConstructor:"..msg.chat_id_,dp.id_)
end,nil)   
end,nil)   
end
end 
...
if result.content_.sticker_ then
local idsticker = result.content_.sticker_.sticker_.persistent_id_
DevAbs:srem(muslim.."Abs:FilterSteckr"..msg.chat_id_,idsticker)
Dev_Abs(msg.chat_id_, msg.id_, 1, '🇸🇦︙تم الغاء منع الملصق يمكنهم ارساله الان', 1, 'md')
return false
end
if result.content_.ID == "MessagePhoto" then
local photo = result.content_.photo_.id_
DevAbs:srem(muslim.."Abs:FilterPhoto"..msg.chat_id_,photo)
Dev_Abs(msg.chat_id_, msg.id_, 1, '🇸🇦︙تم الغاء منع الصورة يمكنهم ارسالها الان', 1, 'md')
return false
end
if result.content_.animation_.animation_ then
local idanimation = result.content_.animation_.animation_.persistent_id_
DevAbs:srem(muslim.."Abs:FilterAnimation"..msg.chat_id_,idanimation)
...
absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, devyousef, 14, string.len(msg.sender_user_id_))
DevAbs:del(muslim..'Abs:Thwel:Abs'..msg.chat_id_) 
end
if text and (text == "تعطيل تحويل الصيغ" or text == "تعطيل التحويل") and Manager(msg) and ChCheck(msg) then
local devyousef = '🇸🇦︙اهلا عزيزي -› '..AbsRank(msg)..' \n🇸🇦︙تم تعطيل تحويل الصيغ'
absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, devyousef, 14, string.len(msg.sender_user_id_))
DevAbs:set(muslim..'Abs:Thwel:Abs'..msg.chat_id_,true)  
end
if text == 'تحويل' and not DevAbs:get(muslim..'Abs:Thwel:Abs'..msg.chat_id_) and ChCheck(msg) then  
if tonumber(msg.reply_to_message_id_) > 0 then 
...
tdcli_function ({ID = "GetUser",user_id_ = result.sender_user_id_},function(arg,data) 
local user_msgs = DevAbs:get(muslim..'Abs:UsersMsgs'..msg.chat_id_..':'..data.id_) or 0
local user_nkt = tonumber(DevAbs:get(muslim..'Abs:GamesNumber'..msg.chat_id_..data.id_) or 0)
if DevAbs:sismember(muslim..'Abs:BanAll:',result.sender_user_id_) then
Tkeed = 'محظور عام'
elseif DevAbs:sismember(muslim..'Abs:MuteAll:',result.sender_user_id_) then
Tkeed = 'مكتوم عام'
elseif DevAbs:sismember(muslim..'Abs:Ban:'..msg.chat_id_,result.sender_user_id_) then
Tkeed = 'محظور'
elseif DevAbs:sismember(muslim..'Abs:Muted:'..msg.chat_id_,result.sender_user_id_) then
Tkeed = 'مكتوم'
elseif DevAbs:sismember(muslim..'Abs:Tkeed:'..msg.chat_id_,result.sender_user_id_) then
Tkeed = 'مقيد'
else
Tkeed = false
end
...
if DevAbs:sismember(muslim..'Abs:SudoBot:',result.sender_user_id_) and SudoBot(msg) then
sudobot = '\n🇸🇦︙عدد القروبات -› '..(DevAbs:get(muslim..'Abs:Sudos'..result.sender_user_id_) or 0)..'' 
else 
sudobot = '' 
end
...
local user_msgs = DevAbs:get(muslim..'Abs:UsersMsgs'..msg.chat_id_..':'..res.id_) or 0
local user_nkt = tonumber(DevAbs:get(muslim..'Abs:GamesNumber'..msg.chat_id_..res.id_) or 0)
if DevAbs:sismember(muslim..'Abs:BanAll:',res.id_) then
Tkeed = 'محظور عام'
elseif DevAbs:sismember(muslim..'Abs:MuteAll:',res.id_) then
Tkeed = 'مكتوم عام'
elseif DevAbs:sismember(muslim..'Abs:Ban:'..msg.chat_id_,res.id_) then
Tkeed = 'محظور'
elseif DevAbs:sismember(muslim..'Abs:Muted:'..msg.chat_id_,res.id_) then
Tkeed = 'مكتوم'
elseif DevAbs:sismember(muslim..'Abs:Tkeed:'..msg.chat_id_,res.id_) then
Tkeed = 'مقيد'
else
Tkeed = false
end
...
if DevAbs:sismember(muslim..'Abs:SudoBot:',res.id_) and SudoBot(msg) then
sudobot = '\n🇸🇦︙عدد القروبات -› '..(DevAbs:get(muslim..'Abs:Sudos'..res.id_) or 0)..'' 
else 
sudobot = '' 
end
...
local user_msgs = DevAbs:get(muslim..'Abs:UsersMsgs'..msg.chat_id_..':'..iduser) or 0
local user_nkt = tonumber(DevAbs:get(muslim..'Abs:GamesNumber'..msg.chat_id_..iduser) or 0)
if DevAbs:sismember(muslim..'Abs:BanAll:',iduser) then
Tkeed = 'محظور عام'
elseif DevAbs:sismember(muslim..'Abs:MuteAll:',iduser) then
Tkeed = 'مكتوم عام'
elseif DevAbs:sismember(muslim..'Abs:Ban:'..msg.chat_id_,iduser) then
Tkeed = 'محظور'
elseif DevAbs:sismember(muslim..'Abs:Muted:'..msg.chat_id_,iduser) then
Tkeed = 'مكتوم'
elseif DevAbs:sismember(muslim..'Abs:Tkeed:'..msg.chat_id_,iduser) then
Tkeed = 'مقيد'
else
Tkeed = false
end
...
if DevAbs:sismember(muslim..'Abs:SudoBot:',iduser) and SudoBot(msg) then
sudobot = '\n🇸🇦︙عدد القروبات -› '..(DevAbs:get(muslim..'Abs:Sudos'..iduser) or 0)..'' 
else 
sudobot = '' 
end
...
if DevAbs:sismember(muslim..'Abs:Muted:'..msg.chat_id_,result.sender_user_id_) then muted = 'مكتوم' else muted = 'غير مكتوم' end
if DevAbs:sismember(muslim..'Abs:Ban:'..msg.chat_id_,result.sender_user_id_) then banned = 'محظور' else banned = 'غير محظور' end
if DevAbs:sismember(muslim..'Abs:BanAll:',result.sender_user_id_) then banall = 'محظور عام' else banall = 'غير محظور عام' end
if DevAbs:sismember(muslim..'Abs:MuteAll:',result.sender_user_id_) then muteall = 'مكتوم عام' else muteall = 'غير مكتوم عام' end
if DevAbs:sismember(muslim..'Abs:Tkeed:',result.sender_user_id_) then tkeed = 'مقيد' else tkeed = 'غير مقيد' end
...
if result.id_ then
if DevAbs:sismember(muslim..'Abs:Muted:'..msg.chat_id_,result.id_) then muted = 'مكتوم' else muted = 'غير مكتوم' end
if DevAbs:sismember(muslim..'Abs:Ban:'..msg.chat_id_,result.id_) then banned = 'محظور' else banned = 'غير محظور' end
if DevAbs:sismember(muslim..'Abs:BanAll:',result.id_) then banall = 'محظور عام' else banall = 'غير محظور عام' end
if DevAbs:sismember(muslim..'Abs:MuteAll:',result.id_) then muteall = 'مكتوم عام' else muteall = 'غير مكتوم عام' end
if DevAbs:sismember(muslim..'Abs:Tkeed:',result.id_) then tkeed = 'مقيد' else tkeed = 'غير مقيد' end
Dev_Abs(msg.chat_id_, msg.id_, 1, '🇸🇦︙الحظر العام -› '..banall..'\n🇸🇦︙الكتم العام -› '..muteall..'\n🇸🇦︙الحظر -› '..banned..'\n🇸🇦︙الكتم -› '..muted..'\n🇸🇦︙التقيد -› '..tkeed, 1, 'md')  
else
Dev_Abs(msg.chat_id_, msg.id_, 1, '🇸🇦︙*المعرف غير صحيح*', 1, 'md')  
end
...
if SecondSudo(msg) then
HTTPS.request("https://api.telegram.org/bot"..TokenBot.."/restrictChatMember?chat_id="..msg.chat_id_.."&user_id=" ..result.sender_user_id_.. "&can_send_messages=True&can_send_media_messages=True&can_send_other_messages=True&can_add_web_page_previews=True")  
DevAbs:srem(muslim..'Abs:Tkeed:'..msg.chat_id_,result.sender_user_id_) DevAbs:srem(muslim..'Abs:Ban:'..msg.chat_id_,result.sender_user_id_) DevAbs:srem(muslim..'Abs:Muted:'..msg.chat_id_,result.sender_user_id_) DevAbs:srem(muslim..'Abs:BanAll:',result.sender_user_id_) DevAbs:srem(muslim..'Abs:MuteAll:',result.sender_user_id_)
else
HTTPS.request("https://api.telegram.org/bot"..TokenBot.."/restrictChatMember?chat_id="..msg.chat_id_.."&user_id=" ..result.sender_user_id_.. "&can_send_messages=True&can_send_media_messages=True&can_send_other_messages=True&can_add_web_page_previews=True")  
DevAbs:srem(muslim..'Abs:Tkeed:'..msg.chat_id_,result.sender_user_id_) DevAbs:srem(muslim..'Abs:Ban:'..msg.chat_id_,result.sender_user_id_) DevAbs:srem(muslim..'Abs:Muted:'..msg.chat_id_,result.sender_user_id_) 
end
end
getMessage(msg.chat_id_, tonumber(msg.reply_to_message_id_),unbanreply) 
end
...
if SecondSudo(msg) then
HTTPS.request("https://api.telegram.org/bot"..TokenBot.."/restrictChatMember?chat_id="..msg.chat_id_.."&user_id=" ..user.. "&can_send_messages=True&can_send_media_messages=True&can_send_other_messages=True&can_add_web_page_previews=True")  
DevAbs:srem(muslim..'Abs:Tkeed:'..msg.chat_id_,user) DevAbs:srem(muslim..'Abs:Ban:'..msg.chat_id_,user) DevAbs:srem(muslim..'Abs:Muted:'..msg.chat_id_,user) DevAbs:srem(muslim..'Abs:BanAll:',user) DevAbs:srem(muslim..'Abs:MuteAll:',user)
else
HTTPS.request("https://api.telegram.org/bot"..TokenBot.."/restrictChatMember?chat_id="..msg.chat_id_.."&user_id=" ..user.. "&can_send_messages=True&can_send_media_messages=True&can_send_other_messages=True&can_add_web_page_previews=True")  
DevAbs:srem(muslim..'Abs:Tkeed:'..msg.chat_id_,user) DevAbs:srem(muslim..'Abs:Ban:'..msg.chat_id_,user) DevAbs:srem(muslim..'Abs:Muted:'..msg.chat_id_,user) 
end  
end,nil)  
end
...
if SecondSudo(msg) then
HTTPS.request("https://api.telegram.org/bot"..TokenBot.."/restrictChatMember?chat_id="..msg.chat_id_.."&user_id=" ..result.id_.. "&can_send_messages=True&can_send_media_messages=True&can_send_other_messages=True&can_add_web_page_previews=True")  
DevAbs:srem(muslim..'Abs:Tkeed:'..msg.chat_id_,result.id_) DevAbs:srem(muslim..'Abs:Ban:'..msg.chat_id_,result.id_) DevAbs:srem(muslim..'Abs:Muted:'..msg.chat_id_,result.id_) DevAbs:srem(muslim..'Abs:BanAll:',result.id_) DevAbs:srem(muslim..'Abs:MuteAll:',result.id_)
else
HTTPS.request("https://api.telegram.org/bot"..TokenBot.."/restrictChatMember?chat_id="..msg.chat_id_.."&user_id=" ..result.id_.. "&can_send_messages=True&can_send_media_messages=True&can_send_other_messages=True&can_add_web_page_previews=True")  
DevAbs:srem(muslim..'Abs:Tkeed:'..msg.chat_id_,result.id_) DevAbs:srem(muslim..'Abs:Ban:'..msg.chat_id_,result.id_) DevAbs:srem(muslim..'Abs:Muted:'..msg.chat_id_,result.id_) 
end
end,nil)   
end  
...
DevAbs:set(muslim.."Abs:GpIds:Text"..msg.chat_id_,Text_Rand)
...
DevAbs:set("muslim:New:id:"..muslim..msg.sender_user_id_,'devyousef')
return "devyousef"
end
if text and DevAbs:get("muslim:New:id:"..muslim..msg.sender_user_id_) then 
if text == 'الغاء' then   
Dev_Abs(msg.chat_id_, msg.id_, 1, '🇸🇦︙تم الغاء حفظ كليشة الايدي', 1, 'md')
DevAbs:del("muslim:New:id:"..muslim..msg.sender_user_id_)
return false
end
DevAbs:del("muslim:New:id:"..muslim..msg.sender_user_id_)
Dev_Abs(msg.chat_id_, msg.id_, 1, '🇸🇦︙تم حفظ كليشة الايدي العامه', 1, 'md')
DevAbs:set(muslim.."Abs:AllIds:Text",text)
return false
end
...
DevAbs:del(muslim.."Abs:AllIds:Text")
end
end
--     Source dev yousef     --
if text and text:match("^تعيين الايدي$") and ChCheck(msg) or text and text:match("^تعين الايدي$") and ChCheck(msg) then
Dev_Abs(msg.chat_id_, msg.id_, 1, '🇸🇦︙رجائا اتبع التعليمات للتعيين \n🇸🇦︙لطبع كليشة الايدي ارسل كليشه تحتوي على النصوص التي باللغه الانجليزيه ادناه -› ⤈\n------------ Y●L ------------\n `#username` ↬ لطبع المعرف\n `#id` ↬ لطبع الايدي \n `#photos` ↬ لطبع عدد الصور \n `#stast` ↬ لطبع الرتب \n `#msgs` ↬ لطبع عدد الرسائل \n `#msgday` ↬ لطبع الرسائل اليوميه \n `#CustomTitle` ↬ لطبع اللقب \n `#bio` ↬ لطبع البايو \n `#auto` ↬ لطبع التفاعل \n `#game` ↬ لطبع عدد النقاط \n `#cont` ↬ لطبع عدد الجهات \n `#edit` ↬ لطبع عدد التعديلات \n `#Description` ↬ لطبع تعليق الصور\n------------ Y●L ------------', 1, 'md')
DevAbs:set("muslim:New:id:"..muslim..msg.chat_id_..msg.sender_user_id_,'devyousef')
return "devyousef"
end
if text and Manager(msg) and DevAbs:get("muslim:New:id:"..muslim..msg.chat_id_..msg.sender_user_id_) then 
if text == 'الغاء' then   
Dev_Abs(msg.chat_id_, msg.id_, 1, '🇸🇦︙تم الغاء حفظ كليشة الايدي', 1, 'md')
DevAbs:del("muslim:New:id:"..muslim..msg.chat_id_..msg.sender_user_id_)
return false
end
DevAbs:del("muslim:New:id:"..muslim..msg.chat_id_..msg.sender_user_id_)
Dev_Abs(msg.chat_id_, msg.id_, 1, '🇸🇦︙تم حفظ الكليشه الجديده', 1, 'md')
DevAbs:set(muslim.."Abs:GpIds:Text"..msg.chat_id_,text)
return false
end
if text and text:match("^حذف الايدي$") and ChCheck(msg) or text and text:match("^مسح الايدي$") and ChCheck(msg) then
ReplyStatus(msg,msg.sender_user_id_,"ReplyBy","🇸🇦︙تم حذف كليشة الايدي")  
DevAbs:del(muslim.."Abs:GpIds:Text"..msg.chat_id_)
end
end
--     Source dev yousef     --
if msg.reply_to_message_id_ ~= 0 then
...
local msgsday = DevAbs:get(muslim..'Abs:UsersMsgs'..muslim..os.date('%d')..':'..msg.chat_id_..':'..msg.sender_user_id_) or 0
local edit_msg = DevAbs:get(muslim..'Abs:EditMsg'..msg.chat_id_..msg.sender_user_id_) or 0
local user_msgs = DevAbs:get(muslim..'Abs:UsersMsgs'..msg.chat_id_..':'..msg.sender_user_id_)
local user_nkt = tonumber(DevAbs:get(muslim..'Abs:GamesNumber'..msg.chat_id_..msg.sender_user_id_) or 0)
local cont = (tonumber(DevAbs:get(muslim..'Abs:ContactNumber'..msg.chat_id_..':'..msg.sender_user_id_)) or 0)
...
if abbas.photos_[0] then
if not DevAbs:get(muslim..'Abs:Lock:Id'..msg.chat_id_) then 
if not DevAbs:get(muslim..'Abs:Lock:Id:Photo'..msg.chat_id_) then 
if DevAbs:get(muslim.."Abs:AllIds:Text") then
newpicid = DevAbs:get(muslim.."Abs:AllIds:Text")
...
if not DevAbs:get(muslim.."Abs:GpIds:Text"..msg.chat_id_) then 
sendPhoto(msg.chat_id_, msg.id_, 0, 1, nil, abbas.photos_[0].sizes_[1].photo_.persistent_id_,newpicid,msg.id_,msg.id_.."")
else 
local new_id = DevAbs:get(muslim.."Abs:GpIds:Text"..msg.chat_id_)
...
if DevAbs:get(muslim.."Abs:AllIds:Text") then
newallid = DevAbs:get(muslim.."Abs:AllIds:Text")
...
if DevAbs:get(muslim.."Abs:AllIds:Text") then
notpicid = DevAbs:get(muslim.."Abs:AllIds:Text")
...
DevAbs:set(muslim..'Abs:Lock:EditMsgs'..msg.chat_id_,true)
else
...
if not DevAbs:get(muslim..'Abs:Lock:Farsi'..msg.chat_id_) then
ReplyStatus(msg,msg.sender_user_id_,"ReplyBy","🇸🇦︙تم قفل الفارسية")  
DevAbs:set(muslim..'Abs:Lock:Farsi'..msg.chat_id_,true)
else
Dev_Abs(msg.chat_id_, msg.id_, 1, '🇸🇦︙الفارسية بالفعل مقفل ه في المجموعة', 1, 'md')
end
end
if LockText[2] == "السب" then
if DevAbs:get(muslim..'Abs:Lock:Fshar'..msg.chat_id_) then
ReplyStatus(msg,msg.sender_user_id_,"ReplyBy","🇸🇦︙تم قفل السب")  
DevAbs:del(muslim..'Abs:Lock:Fshar'..msg.chat_id_)
else
Dev_Abs(msg.chat_id_, msg.id_, 1, '🇸🇦︙السب بالفعل مقفل  في المجموعة', 1, 'md')
end
end
if LockText[2] == "الطائفيه" then
if DevAbs:get(muslim..'Abs:Lock:Taf'..msg.chat_id_) then
ReplyStatus(msg,msg.sender_user_id_,"ReplyBy","🇸🇦︙تم قفل الطائفيه")  
DevAbs:del(muslim..'Abs:Lock:Taf'..msg.chat_id_)
else
Dev_Abs(msg.chat_id_, msg.id_, 1, '🇸🇦︙الطائفيه بالفعل مقفل ه في المجموعة', 1, 'md')
end
end
if LockText[2] == "الكفر" then
if DevAbs:get(muslim..'Abs:Lock:Kfr'..msg.chat_id_) then
ReplyStatus(msg,msg.sender_user_id_,"ReplyBy","🇸🇦︙تم قفل الكفر")  
DevAbs:del(muslim..'Abs:Lock:Kfr'..msg.chat_id_)
else
Dev_Abs(msg.chat_id_, msg.id_, 1, '🇸🇦︙الكفر بالفعل مقفل  في المجموعة', 1, 'md')
end
end
if LockText[2] == "الفارسية بالطرد" then
if not DevAbs:get(muslim..'Abs:Lock:FarsiBan'..msg.chat_id_) then
ReplyStatus(msg,msg.sender_user_id_,"ReplyBy","🇸🇦︙تم قفل الفارسية بالطرد")  
DevAbs:set(muslim..'Abs:Lock:FarsiBan'..msg.chat_id_,true)
else
Dev_Abs(msg.chat_id_, msg.id_, 1, '🇸🇦︙الفارسية بالطرد بالفعل مقفل ه ', 1, 'md')
end
end
if LockText[2] == "البوتات" or LockText[2] == "البوتات بالحذف" then
ReplyStatus(msg,msg.sender_user_id_,"ReplyBy","🇸🇦︙تم قفل البوتات بالحذف")  
DevAbs:set(muslim.."Abs:Lock:Bots"..msg.chat_id_,"del")  
end
if LockText[2] == "البوتات بالطرد" then
ReplyStatus(msg,msg.sender_user_id_,"ReplyBy","🇸🇦︙تم قفل البوتات بالطرد")  
DevAbs:set(muslim.."Abs:Lock:Bots"..msg.chat_id_,"kick")  
end
...
DevAbs:set(muslim.."Abs:Lock:Bots"..msg.chat_id_,"ked")  
end
if LockText[2] == "التكرار" or LockText[2] == "التكرار بالحذف" then 
DevAbs:hset(muslim.."Abs:Spam:Group:User"..msg.chat_id_ ,"Spam:User","del")  
ReplyStatus(msg,msg.sender_user_id_,"ReplyBy","🇸🇦︙تم قفل التكرار بالحذف")  
end
if LockText[2] == "التكرار بالطرد" then 
DevAbs:hset(muslim.."Abs:Spam:Group:User"..msg.chat_id_ ,"Spam:User","kick")  
...
DevAbs:hset(muslim.."Abs:Spam:Group:User"..msg.chat_id_ ,"Spam:User","keed")  
ReplyStatus(msg,msg.sender_user_id_,"ReplyBy","🇸🇦︙تم قفل التكرار بالتقيد")  
end
if LockText[2] == "التكرار بالكتم" then 
DevAbs:hset(muslim.."Abs:Spam:Group:User"..msg.chat_id_ ,"Spam:User","mute")  
ReplyStatus(msg,msg.sender_user_id_,"ReplyBy","🇸🇦︙تم قفل التكرار بالكتم")  
end
if BasicConstructor(msg) then
if LockText[2] == "التثبيت" then
if not DevAbs:get(muslim..'Abs:Lock:Pin'..msg.chat_id_) then
tdcli_function ({ ID = "GetChannelFull",  channel_id_ = msg.chat_id_:gsub("-100","") }, function(arg,data)  DevAbs:set(muslim.."Abs:PinnedMsg"..msg.chat_id_,data.pinned_message_id_)  end,nil)
ReplyStatus(msg,msg.sender_user_id_,"ReplyBy","🇸🇦︙تم قفل التثبيت")  
DevAbs:set(muslim..'Abs:Lock:Pin'..msg.chat_id_,true)
DevAbs:sadd(muslim.."Abs:Lock:Pinpin",msg.chat_id_) 
else
...
DevAbs:hset(muslim.."Abs:Spam:Group:User"..msg.chat_id_ ,"Num:Spam" ,TextSpam) 
end
end
if text and (text:match("^ضع زمن التكرار (%d+)$") or text:match("^وضع زمن التكرار (%d+)$")) then  
local TextSpam = text:match("ضع زمن التكرار (%d+)$") or text:match("وضع زمن التكرار (%d+)$")
DevAbs:hset(muslim.."Abs:Spam:Group:User"..msg.chat_id_ ,"Num:Spam:Time" ,TextSpam) 
...
absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, devyousef, 14, string.len(msg.sender_user_id_))
DevAbs:del(muslim..'Abs:Lock:Id:Photo'..msg.chat_id_)
end end
if text and text == 'تعطيل الايدي بالصورة' and ChCheck(msg) then
if DevAbs:get(muslim..'Abs:Lock:Id:Photo'..msg.chat_id_) then
Dev_Abs(msg.chat_id_, msg.id_, 1, '🇸🇦︙الايدي بالصورة بالتاكيد معطل', 1, 'md')
else
local devyousef = '🇸🇦︙اهلا عزيزي -› '..AbsRank(msg)..' \n🇸🇦︙تم تعطيل الايدي بالصورة'
absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, devyousef, 14, string.len(msg.sender_user_id_))
DevAbs:set(muslim..'Abs:Lock:Id:Photo'..msg.chat_id_,true)
end end 
...
absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, devyousef, 14, string.len(msg.sender_user_id_))
DevAbs:del(muslim..'Abs:Lock:Id'..msg.chat_id_)
end end 
if text and text == 'تعطيل الايدي' and ChCheck(msg) then
if DevAbs:get(muslim..'Abs:Lock:Id'..msg.chat_id_) then
Dev_Abs(msg.chat_id_, msg.id_, 1, '🇸🇦︙الايدي بالتاكيد معطل ', 1, 'md')
else
local devyousef = '🇸🇦︙اهلا عزيزي -› '..AbsRank(msg)..' \n🇸🇦︙تم تعطيل الايدي بنجاح'
absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, devyousef, 14, string.len(msg.sender_user_id_))
DevAbs:set(muslim..'Abs:Lock:Id'..msg.chat_id_,true)
end end
end
--     Source dev yousef     --
...
DevAbs:setex(muslim.."Abs:Set:Groups:Links"..msg.chat_id_..msg.sender_user_id_,300,true) 
end
if text == 'انشاء رابط' or text == 'انشاء الرابط' then
local LinkGp = json:decode(https.request('https://api.telegram.org/bot'..TokenBot..'/exportChatInviteLink?chat_id='..msg.chat_id_))
if not DevAbs:get(muslim.."Abs:Groups:Links"..msg.chat_id_)  then 
if LinkGp.ok == true then 
LinkGroup = LinkGp.result
DevAbs:set(muslim.."Abs:Groups:Links"..msg.chat_id_,LinkGroup) 
...
absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, devyousef, 14, string.len(msg.sender_user_id_))
DevAbs:set(muslim.."Abs:Lock:Welcome"..msg.chat_id_,true)
end
if text and text:match("^تعطيل الترحيب$") and ChCheck(msg) then
local devyousef = '🇸🇦︙اهلا عزيزي -› '..AbsRank(msg)..' \n🇸🇦︙تم تعطيل الترحيب بنجاح'
absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, devyousef, 14, string.len(msg.sender_user_id_))
DevAbs:del(muslim.."Abs:Lock:Welcome"..msg.chat_id_)
end
if DevAbs:get(muslim..'Abs:setwelcome'..msg.chat_id_..':'..msg.sender_user_id_) then 
if text == 'الغاء' then 
Dev_Abs(msg.chat_id_, msg.id_, 1, '🇸🇦︙تم الغاء حفظ كليشة الترحيب', 1, 'md')
DevAbs:del(muslim..'Abs:setwelcome'..msg.chat_id_..':'..msg.sender_user_id_)
return false  
end 
DevAbs:del(muslim..'Abs:setwelcome'..msg.chat_id_..':'..msg.sender_user_id_)
Welcomes = text:gsub('"',"") Welcomes = text:gsub("'","") Welcomes = text:gsub(",","") Welcomes = text:gsub("*","") Welcomes = text:gsub(";","") Welcomes = text:gsub("`","") Welcomes = text:gsub("{","") Welcomes = text:gsub("}","") 
DevAbs:set(muslim..'Abs:Groups:Welcomes'..msg.chat_id_,Welcomes)
...
DevAbs:set(muslim..'Abs:setwelcome'..msg.chat_id_..':'..msg.sender_user_id_,true)
end
if text and text:match("^حذف الترحيب$") and ChCheck(msg) or text and text:match("^حذف ترحيب$") and ChCheck(msg) then
ReplyStatus(msg,msg.sender_user_id_,"ReplyBy","🇸🇦︙تم حذف الترحيب")  
DevAbs:del(muslim..'Abs:Groups:Welcomes'..msg.chat_id_)
end
if text and text:match("^جلب الترحيب$") and ChCheck(msg) or text and text:match("^جلب ترحيب$") and ChCheck(msg) or text and text:match("^الترحيب$") and ChCheck(msg) then
local Welcomes = DevAbs:get(muslim..'Abs:Groups:Welcomes'..msg.chat_id_)
if Welcomes then
Dev_Abs(msg.chat_id_, msg.id_, 1, Welcomes, 1, 'md')
else
...
--     Source dev yousef     --
if DevAbs:get(muslim..'Abs:SetDescription'..msg.chat_id_..':'..msg.sender_user_id_) then  
if text == 'الغاء' then 
Dev_Abs(msg.chat_id_, msg.id_, 1, "🇸🇦︙تم الغاء حفظ الوصف", 1, 'md')
DevAbs:del(muslim..'Abs:SetDescription'..msg.chat_id_..':'..msg.sender_user_id_)
return false  
end 
DevAbs:del(muslim..'Abs:SetDescription'..msg.chat_id_..':'..msg.sender_user_id_)
...
DevAbs:set(muslim..'Abs:SetDescription'..msg.chat_id_..':'..msg.sender_user_id_,true)
...
DevAbs:set(muslim.."Abs:SetFilters"..msg.sender_user_id_..msg.chat_id_,"add")  
return false  
end    
if DevAbs:get(muslim.."Abs:SetFilters"..msg.sender_user_id_..msg.chat_id_) == "add" then
if text == 'الغاء' then 
Dev_Abs(msg.chat_id_, msg.id_, 1, '🇸🇦︙تم الغاء امر المنع', 1, 'md')
DevAbs:del(muslim.."Abs:SetFilters"..msg.sender_user_id_..msg.chat_id_)  
return false  
end   
Dev_Abs(msg.chat_id_, msg.id_, 1, "🇸🇦︙تم منع الكلمه -› "..text, 1, 'html')
DevAbs:del(muslim.."Abs:SetFilters"..msg.sender_user_id_..msg.chat_id_)  
DevAbs:hset(muslim..'Abs:Filters:'..msg.chat_id_, text,'newword')
return false
end
if text and text == "الغاء منع" and msg.reply_to_message_id_ == 0 and ChCheck(msg) then       
Dev_Abs(msg.chat_id_, msg.id_, 1, "🇸🇦︙ارسل لي الكلمه الان", 1, 'md') 
DevAbs:set(muslim.."Abs:SetFilters"..msg.sender_user_id_..msg.chat_id_,"del")  
return false  
end    
if DevAbs:get(muslim.."Abs:SetFilters"..msg.sender_user_id_..msg.chat_id_) == "del" then   
if text == 'الغاء' then 
Dev_Abs(msg.chat_id_, msg.id_, 1, '🇸🇦︙تم الغاء امر الغاء المنع', 1, 'md')
DevAbs:del(muslim.."Abs:SetFilters"..msg.sender_user_id_..msg.chat_id_)  
return false  
end   
if not DevAbs:hget(muslim..'Abs:Filters:'..msg.chat_id_, text) then  
Dev_Abs(msg.chat_id_, msg.id_, 1, "🇸🇦︙الكلمه -› "..text.." غير ممنوعه", 1, 'html')
DevAbs:del(muslim.."Abs:SetFilters"..msg.sender_user_id_..msg.chat_id_)  
else
DevAbs:hdel(muslim..'Abs:Filters:'..msg.chat_id_, text)
...
local gps = DevAbs:scard(muslim.."Abs:Groups") local users = DevAbs:scard(muslim.."Abs:Users") 
...
local users = DevAbs:scard(muslim.."Abs:Users")
...
local gps = DevAbs:scard(muslim.."Abs:Groups")
Dev_Abs(msg.chat_id_, msg.id_, 1, '🇸🇦︙عدد المجموعات -› ❨ '..gps..' ❩', 1, 'md')
end
end
--     Source dev yousef     --
if text and text:match('^تنظيف (%d+)$') and ChCheck(msg) then  
if not DevAbs:get(muslim..'Delete:Time'..msg.chat_id_..':'..msg.sender_user_id_) then  
...
DevAbs:setex(muslim..'Delete:Time'..msg.chat_id_..':'..msg.sender_user_id_,300,true)
end 
end
if text == "تنظيف المشتركين" and SecondSudo(msg) and ChCheck(msg) then 
local pv = DevAbs:smembers(muslim.."Abs:Users")
local sendok = 0
for i = 1, #pv do
...
DevAbs:srem(muslim.."Abs:Users",pv[i])
sendok = sendok + 1
end
if #pv == i then 
if sendok == 0 then
...
local group = DevAbs:smembers(muslim.."Abs:Groups")
local w = 0
local q = 0
for i = 1, #group do
tdcli_function({ID='GetChat',chat_id_ = group[i]},function(arg,data)
if data and data.type_ and data.type_.channel_ and data.type_.channel_.status_ and data.type_.channel_.status_.ID == "ChatMemberStatusMember" then
DevAbs:srem(muslim.."Abs:Groups",group[i]) 
tdcli_function ({ ID = "ChangeChatMemberStatus", chat_id_ = group[i], user_id_ = muslim, status_ = { ID = "ChatMemberStatusLeft" }, }, dl_cb, nil)
w = w + 1
end
...
local muslimgp2 = (w + q)
local muslimgp3 = #group - muslimgp2
if q == 0 then
muslimgp2 = ''
else
...
Dev_Abs(msg.chat_id_, msg.id_, 1,'🇸🇦︙عدد القروبات الان -› { '..#group..' }'..muslimgp1..muslimgp2..'\n🇸🇦︙العدد الحقيقي الان  -› ( '..muslimgp3..' ) \n ', 1, 'md')
end end
end,nil)
end
return false
...
absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, devyousef, 14, string.len(msg.sender_user_id_))
DevAbs:set(muslim..'Abs:Lock:Clean'..msg.chat_id_,true)  
end
...
absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, devyousef, 14, string.len(msg.sender_user_id_))
DevAbs:del(muslim..'Abs:Lock:Clean'..msg.chat_id_) 
end
...
DevAbs:set(muslim..'Abs:CleanNum'..msg.chat_id_,Num) 
end end 
if msg and DevAbs:get(muslim..'Abs:Lock:Clean'..msg.chat_id_) then
if DevAbs:get(muslim..'Abs:CleanNum'..msg.chat_id_) then CleanNum = DevAbs:get(muslim..'Abs:CleanNum'..msg.chat_id_) else CleanNum = 200 end
if DevAbs:scard(muslim.."Abs:cleaner"..msg.chat_id_) >= tonumber(CleanNum) then 
local List = DevAbs:smembers(muslim.."Abs:cleaner"..msg.chat_id_)
local Del = 0
for k,v in pairs(List) do
Del = (Del + 1)
...
DevAbs:del(muslim.."Abs:cleaner"..msg.chat_id_)
end 
end 
if Cleaner(msg) then
if DevAbs:get(muslim..'Abs:Lock:Clean'..msg.chat_id_) then 
if text == "الميديا" and ChCheck(msg) or text == "عدد الميديا" and ChCheck(msg) then 
local M = DevAbs:scard(muslim.."Abs:cleaner"..msg.chat_id_)
if M ~= 0 then
Dev_Abs(msg.chat_id_, msg.id_, 1, "🇸🇦︙عدد الميديا -› "..M.."\n🇸🇦︙الحذف التلقائي -› "..(DevAbs:get(muslim..'Abs:CleanNum'..msg.chat_id_) or 200), 1, 'md') 
else
...
if DevAbs:get(muslim..'Abs:Lock:EditMsgs'..msg.chat_id_) then
ReplyStatus(msg,msg.sender_user_id_,"ReplyBy","🇸🇦︙تم فتح التعديل")  
DevAbs:del(muslim..'Abs:Lock:EditMsgs'..msg.chat_id_)
else
...
DevAbs:del(muslim.."Abs:Lock:Bots"..msg.chat_id_)  
end
if UnLockText[2] == "التكرار" then 
DevAbs:hdel(muslim.."Abs:Spam:Group:User"..msg.chat_id_ ,"Spam:User")  
ReplyStatus(msg,msg.sender_user_id_,"ReplyBy","🇸🇦︙تم فتح التكرار")  
end
if BasicConstructor(msg) then
if UnLockText[2] == "التثبيت" then
if DevAbs:get(muslim..'Abs:Lock:Pin'..msg.chat_id_) then
ReplyStatus(msg,msg.sender_user_id_,"ReplyBy","🇸🇦︙تم فتح التثبيت")  
DevAbs:del(muslim..'Abs:Lock:Pin'..msg.chat_id_)
DevAbs:srem(muslim.."Abs:Lock:Pinpin",msg.chat_id_)
else
...
DevAbs:set(muslim..'Abs:Lock:Text'..msg.chat_id_,true)
else
Dev_Abs(msg.chat_id_, msg.id_, 1, '🇸🇦︙الدردشة بالفعل مقفل ه في المجموعة', 1, 'md')
end
end
if LockText[2] == "الاونلاين" then
if not DevAbs:get(muslim..'Abs:Lock:Inline'..msg.chat_id_) then
ReplyStatus(msg,msg.sender_user_id_,"ReplyBy","🇸🇦︙تم قفل الاونلاين")  
DevAbs:set(muslim..'Abs:Lock:Inline'..msg.chat_id_,true)
else
Dev_Abs(msg.chat_id_, msg.id_, 1, '🇸🇦︙الاونلاين بالفعل مقفل  في المجموعة', 1, 'md')
end
end
if LockText[2] == "الصور" then
if not DevAbs:get(muslim..'Abs:Lock:Photo'..msg.chat_id_) then
ReplyStatus(msg,msg.sender_user_id_,"ReplyBy","🇸🇦︙تم قفل الصور")  
DevAbs:set(muslim..'Abs:Lock:Photo'..msg.chat_id_,true)
else
Dev_Abs(msg.chat_id_, msg.id_, 1, '🇸🇦︙الصور بالفعل مقفل ه في المجموعة', 1, 'md')
end
end
if LockText[2] == "المنشورات" then
if not DevAbs:get(muslim..'Abs:Lock:Spam'..msg.chat_id_) then
ReplyStatus(msg,msg.sender_user_id_,"ReplyBy","🇸🇦︙تم قفل المنشورات")  
DevAbs:set(muslim..'Abs:Lock:Spam'..msg.chat_id_,true)
else
Dev_Abs(msg.chat_id_, msg.id_, 1, '🇸🇦︙المنشورات بالفعل مقفل ه في المجموعة', 1, 'md')
end
end
if LockText[2] == "الدخول" then
if not DevAbs:get(muslim..'Abs:Lock:Join'..msg.chat_id_) then
ReplyStatus(msg,msg.sender_user_id_,"ReplyBy","🇸🇦︙تم قفل الدخول")  
DevAbs:set(muslim..'Abs:Lock:Join'..msg.chat_id_,true)
else
Dev_Abs(msg.chat_id_, msg.id_, 1, '🇸🇦︙الدخول بالفعل مقفل  في المجموعة', 1, 'md')
end
end
if LockText[2] == "الفيديو" then
if not DevAbs:get(muslim..'Abs:Lock:Videos'..msg.chat_id_) then
ReplyStatus(msg,msg.sender_user_id_,"ReplyBy","🇸🇦︙تم قفل الفيديو")  
DevAbs:set(muslim..'Abs:Lock:Videos'..msg.chat_id_,true)
else
Dev_Abs(msg.chat_id_, msg.id_, 1, '🇸🇦︙الفيديو بالفعل مقفل  في المجموعة', 1, 'md')
end
end
if LockText[2] == "المتحركة" then
if not DevAbs:get(muslim..'Abs:Lock:Gifs'..msg.chat_id_) then
ReplyStatus(msg,msg.sender_user_id_,"ReplyBy","🇸🇦︙تم قفل المتحركة")  
DevAbs:set(muslim..'Abs:Lock:Gifs'..msg.chat_id_,true)
else
Dev_Abs(msg.chat_id_, msg.id_, 1, '🇸🇦︙المتحركة بالفعل مقفل ه في المجموعة', 1, 'md')
end
end
if LockText[2] == "الاغاني" then
if not DevAbs:get(muslim..'Abs:Lock:Music'..msg.chat_id_) then
ReplyStatus(msg,msg.sender_user_id_,"ReplyBy","🇸🇦︙تم قفل الاغاني")  
DevAbs:set(muslim..'Abs:Lock:Music'..msg.chat_id_,true)
else
Dev_Abs(msg.chat_id_, msg.id_, 1, '🇸🇦︙الاغاني بالفعل مقفل ه في المجموعة', 1, 'md')
end
end
if LockText[2] == "الصوت" then
if not DevAbs:get(muslim..'Abs:Lock:Voice'..msg.chat_id_) then
ReplyStatus(msg,msg.sender_user_id_,"ReplyBy","🇸🇦︙تم قفل الصوت")  
DevAbs:set(muslim..'Abs:Lock:Voice'..msg.chat_id_,true)
else
Dev_Abs(msg.chat_id_, msg.id_, 1, '🇸🇦︙الصوت بالفعل مقفل  في المجموعة', 1, 'md')
end
end
if LockText[2] == "الروابط" then
if not DevAbs:get(muslim..'Abs:Lock:Links'..msg.chat_id_) then
ReplyStatus(msg,msg.sender_user_id_,"ReplyBy","🇸🇦︙تم قفل الروابط")  
DevAbs:set(muslim..'Abs:Lock:Links'..msg.chat_id_,true)
else
Dev_Abs(msg.chat_id_, msg.id_, 1, '🇸🇦︙الروابط بالفعل مقفل ه في المجموعة', 1, 'md')
end
end
if LockText[2] == "المواقع" then
if not DevAbs:get(muslim..'Abs:Lock:Location'..msg.chat_id_) then
ReplyStatus(msg,msg.sender_user_id_,"ReplyBy","🇸🇦︙تم قفل المواقع")  
DevAbs:set(muslim..'Abs:Lock:Location'..msg.chat_id_,true)
else
Dev_Abs(msg.chat_id_, msg.id_, 1, '🇸🇦︙المواقع بالفعل مقفل ه في المجموعة', 1, 'md')
end
end
if LockText[2] == "المعرف" or LockText[2] == "المعرفات" then
if not DevAbs:get(muslim..'Abs:Lock:Tags'..msg.chat_id_) then
ReplyStatus(msg,msg.sender_user_id_,"ReplyBy","🇸🇦︙تم قفل المعرفات")  
DevAbs:set(muslim..'Abs:Lock:Tags'..msg.chat_id_,true)
else
Dev_Abs(msg.chat_id_, msg.id_, 1, '🇸🇦︙المعرفات بالفعل مقفل ه في المجموعة', 1, 'md')
end
end
if LockText[2] == "الملفات" then
if not DevAbs:get(muslim..'Abs:Lock:Document'..msg.chat_id_) then
ReplyStatus(msg,msg.sender_user_id_,"ReplyBy","🇸🇦︙تم قفل الملفات")  
DevAbs:set(muslim..'Abs:Lock:Document'..msg.chat_id_,true)
else
Dev_Abs(msg.chat_id_, msg.id_, 1, '🇸🇦︙الملفات بالفعل مقفل ه في المجموعة', 1, 'md')
end
end
if LockText[2] == "الهاشتاك" or LockText[2] == "التاك" then
if not DevAbs:get(muslim..'Abs:Lock:Hashtak'..msg.chat_id_) then
ReplyStatus(msg,msg.sender_user_id_,"ReplyBy","🇸🇦︙تم قفل الهاشتاك")  
DevAbs:set(muslim..'Abs:Lock:Hashtak'..msg.chat_id_,true)
else
Dev_Abs(msg.chat_id_, msg.id_, 1, '🇸🇦︙الهاشتاك بالفعل مقفل  في المجموعة', 1, 'md')
end
end
if LockText[2] == "الجهات" then
if not DevAbs:get(muslim..'Abs:Lock:Contact'..msg.chat_id_) then
ReplyStatus(msg,msg.sender_user_id_,"ReplyBy","🇸🇦︙تم قفل الجهات")  
DevAbs:set(muslim..'Abs:Lock:Contact'..msg.chat_id_,true)
else
Dev_Abs(msg.chat_id_, msg.id_, 1, '️🇸🇦︙الجهات بالفعل مقفل ه في المجموعة', 1, 'md')
end
end
if LockText[2] == "الشبكات" then
if not DevAbs:get(muslim..'Abs:Lock:WebLinks'..msg.chat_id_) then
ReplyStatus(msg,msg.sender_user_id_,"ReplyBy","🇸🇦︙تم قفل الشبكات")  
DevAbs:set(muslim..'Abs:Lock:WebLinks'..msg.chat_id_,true) 
else
Dev_Abs(msg.chat_id_, msg.id_, 1, '🇸🇦︙الشبكات بالفعل مقفل ه في المجموعة', 1, 'md')
end
end
if LockText[2] == "العربية" then
if not DevAbs:get(muslim..'Abs:Lock:Arabic'..msg.chat_id_) then
ReplyStatus(msg,msg.sender_user_id_,"ReplyBy","🇸🇦︙تم قفل العربية")  
DevAbs:set(muslim..'Abs:Lock:Arabic'..msg.chat_id_,true)
else
Dev_Abs(msg.chat_id_, msg.id_, 1, '🇸🇦︙العربية بالفعل مقفل ه في المجموعة', 1, 'md')
end
end
if LockText[2] == "الانجليزية" then
if not DevAbs:get(muslim..'Abs:Lock:English'..msg.chat_id_) then
ReplyStatus(msg,msg.sender_user_id_,"ReplyBy","🇸🇦︙تم قفل الانجليزية")  
DevAbs:set(muslim..'Abs:Lock:English'..msg.chat_id_,true)
else
Dev_Abs(msg.chat_id_, msg.id_, 1, '🇸🇦︙الانجليزية بالفعل مقفل ه في المجموعة', 1, 'md')
end
end
if LockText[2] == "الملصقات" then
if not DevAbs:get(muslim..'Abs:Lock:Stickers'..msg.chat_id_) then
ReplyStatus(msg,msg.sender_user_id_,"ReplyBy","🇸🇦︙تم قفل الملصقات")  
DevAbs:set(muslim..'Abs:Lock:Stickers'..msg.chat_id_,true)
else
Dev_Abs(msg.chat_id_, msg.id_, 1, '🇸🇦︙الملصقات بالفعل مقفل ه في المجموعة', 1, 'md')
end
end
if LockText[2] == "الماركداون" then
if not DevAbs:get(muslim..'Abs:Lock:Markdown'..msg.chat_id_) then
ReplyStatus(msg,msg.sender_user_id_,"ReplyBy","🇸🇦︙تم قفل الماركداون")  
DevAbs:set(muslim..'Abs:Lock:Markdown'..msg.chat_id_,true)
else
Dev_Abs(msg.chat_id_, msg.id_, 1, '🇸🇦︙الماركداون بالفعل مقفل  في المجموعة', 1, 'md')
end
end
if LockText[2] == "الاشعارات" then
if not DevAbs:get(muslim..'Abs:Lock:TagServr'..msg.chat_id_) then
ReplyStatus(msg,msg.sender_user_id_,"ReplyBy","🇸🇦︙تم قفل الاشعارات")  
DevAbs:set(muslim..'Abs:Lock:TagServr'..msg.chat_id_,true)
else
Dev_Abs(msg.chat_id_, msg.id_, 1, '🇸🇦︙الاشعارات بالفعل مقفل ه في المجموعة', 1, 'md')
end
end
if LockText[2] == "التوجيه" then
if not DevAbs:get(muslim..'Abs:Lock:Forwards'..msg.chat_id_) then
ReplyStatus(msg,msg.sender_user_id_,"ReplyBy","🇸🇦︙تم قفل التوجيه")  
DevAbs:set(muslim..'Abs:Lock:Forwards'..msg.chat_id_,true)
else
...
DevAbs:del(muslim..'Abs:Lock:Text'..msg.chat_id_)
else
...
DevAbs:del(muslim..'Abs:Lock:Join'..msg.chat_id_)
else
...
DevAbs:del(muslim..'Abs:Lock:Videos'..msg.chat_id_)
else
...
DevAbs:del(muslim..'Abs:Lock:Document'..msg.chat_id_)
else
...
DevAbs:del(muslim..'Abs:Lock:Inline'..msg.chat_id_)
else
...
DevAbs:del(muslim..'Abs:Lock:Markdown'..msg.chat_id_)
else
...
DevAbs:del(muslim..'Abs:Lock:Gifs'..msg.chat_id_)
else
...
DevAbs:del(muslim..'Abs:Lock:Music'..msg.chat_id_)
else
...
DevAbs:del(muslim..'Abs:Lock:Voice'..msg.chat_id_)
else
...
DevAbs:del(muslim..'Abs:Lock:Links'..msg.chat_id_)
else
...
DevAbs:del(muslim..'Abs:Lock:Location'..msg.chat_id_)
else
...
DevAbs:del(muslim..'Abs:Lock:Tags'..msg.chat_id_)
else
...
DevAbs:del(muslim..'Abs:Lock:Hashtak'..msg.chat_id_)
else
...
DevAbs:del(muslim..'Abs:Lock:Contact'..msg.chat_id_)
else
...
DevAbs:del(muslim..'Abs:Lock:WebLinks'..msg.chat_id_)
else
...
DevAbs:del(muslim..'Abs:Lock:Arabic'..msg.chat_id_)
else
...
DevAbs:del(muslim..'Abs:Lock:English'..msg.chat_id_)
else
...
DevAbs:del(muslim..'Abs:Lock:TagServr'..msg.chat_id_)
else
...
DevAbs:del(muslim..'Abs:Lock:Stickers'..msg.chat_id_)
else
...
DevAbs:del(muslim..'Abs:Lock:Forwards'..msg.chat_id_)
else
...
DevAbs:set(muslim.."Abs:Lock:Bots"..msg.chat_id_,"del") DevAbs:hset(muslim.."Abs:Spam:Group:User"..msg.chat_id_ ,"Spam:User","keed") 
LockList ={'Abs:Lock:Links','Abs:Lock:Contact','Abs:Lock:Forwards','Abs:Lock:Videos','Abs:Lock:Gifs','Abs:Lock:EditMsgs','Abs:Lock:Stickers','Abs:Lock:Farsi','Abs:Lock:Spam','Abs:Lock:WebLinks','Abs:Lock:Photo'}
for i,Lock in pairs(LockList) do
DevAbs:set(muslim..Lock..msg.chat_id_,true)
end
...
DevAbs:del(muslim..UnLock..msg.chat_id_)
end
...
DevAbs:del(muslim..'Abs:Lock:Fshar'..msg.chat_id_) DevAbs:del(muslim..'Abs:Lock:Taf'..msg.chat_id_) DevAbs:del(muslim..'Abs:Lock:Kfr'..msg.chat_id_) 
DevAbs:set(muslim.."Abs:Lock:Bots"..msg.chat_id_,"del") DevAbs:hset(muslim.."Abs:Spam:Group:User"..msg.chat_id_ ,"Spam:User","keed") 
LockList ={'Abs:Lock:EditMsgs','Abs:Lock:Farsi','Abs:Lock:TagServr','Abs:Lock:Inline','Abs:Lock:Photo','Abs:Lock:Spam','Abs:Lock:Videos','Abs:Lock:Gifs','Abs:Lock:Music','Abs:Lock:Voice','Abs:Lock:Links','Abs:Lock:Location','Abs:Lock:Tags','Abs:Lock:Stickers','Abs:Lock:Markdown','Abs:Lock:Forwards','Abs:Lock:Document','Abs:Lock:Contact','Abs:Lock:Hashtak','Abs:Lock:WebLinks'}
for i,Lock in pairs(LockList) do
DevAbs:set(muslim..Lock..msg.chat_id_,true)
...
DevAbs:set(muslim..'Abs:Lock:Fshar'..msg.chat_id_,true) DevAbs:set(muslim..'Abs:Lock:Taf'..msg.chat_id_,true) DevAbs:set(muslim..'Abs:Lock:Kfr'..msg.chat_id_,true) DevAbs:hdel(muslim.."Abs:Spam:Group:User"..msg.chat_id_ ,"Spam:User") 
UnLockList ={'Abs:Lock:EditMsgs','Abs:Lock:Text','Abs:Lock:Arabic','Abs:Lock:English','Abs:Lock:Join','Abs:Lock:Bots','Abs:Lock:Farsi','Abs:Lock:FarsiBan','Abs:Lock:TagServr','Abs:Lock:Inline','Abs:Lock:Photo','Abs:Lock:Spam','Abs:Lock:Videos','Abs:Lock:Gifs','Abs:Lock:Music','Abs:Lock:Voice','Abs:Lock:Links','Abs:Lock:Location','Abs:Lock:Tags','Abs:Lock:Stickers','Abs:Lock:Markdown','Abs:Lock:Forwards','Abs:Lock:Document','Abs:Lock:Contact','Abs:Lock:Hashtak','Abs:Lock:WebLinks'}
for i,UnLock in pairs(UnLockList) do
DevAbs:del(muslim..UnLock..msg.chat_id_)
end
...
DevAbs:set(muslim..'Abs:Spam:Text'..msg.chat_id_,SetSpam)
...
DevAbs:del(muslim.."Abs:mem:Rd"..msg.chat_id_)
DevAbs:del(muslim.."Abs:Cleaner:Rd"..msg.chat_id_)
DevAbs:del(muslim.."Abs:VipMem:Rd"..msg.chat_id_)
DevAbs:del(muslim.."Abs:Admins:Rd"..msg.chat_id_)
DevAbs:del(muslim.."Abs:Managers:Rd"..msg.chat_id_)
DevAbs:del(muslim.."Abs:Constructor:Rd"..msg.chat_id_)
DevAbs:del(muslim.."Abs:BasicConstructor:Rd"..msg.chat_id_)
DevAbs:del(muslim.."Abs:SudoBot:Rd"..msg.chat_id_)
end
end
--     Source dev yousef     --
...
if tonumber(admins[i].user_id_) ~= tonumber(muslim) then
...
DevAbs:del(muslim..'Abs:AdminAll:')
end
...
DevAbs:del(muslim..'Abs:ManagerAll:')
end
if txts[2] == 'المالكين' or txtss[2] == 'المالكين' then
DevAbs:del(muslim..'Abs:AbsConstructor:'..msg.chat_id_)
...
DevAbs:del(muslim..'Abs:BasicConstructor:'..msg.chat_id_)
end
end
if BasicConstructor(msg) then
if txts[2] == 'المنشئين' or txtss[2] == 'المنشئين' then
ReplyStatus(msg,msg.sender_user_id_,"ReplyBy","🇸🇦︙تم حذف المنشئين")  
DevAbs:del(muslim..'Abs:Constructor:'..msg.chat_id_)
end end
if Constructor(msg) then
if txts[2] == 'المدراء' or txtss[2] == 'المدراء' then
ReplyStatus(msg,msg.sender_user_id_,"ReplyBy","🇸🇦︙تم حذف المدراء")  
DevAbs:del(muslim..'Abs:Managers:'..msg.chat_id_)
end 
...
DevAbs:srem(muslim..'Abs:Tkeed:'..msg.chat_id_, v)
end 
...
DevAbs:del(muslim..'Abs:Filters:'..msg.chat_id_)
end
if txts[2] == 'قوائم المنع' or txtss[2] == 'قوائم المنع' then
ReplyStatus(msg,msg.sender_user_id_,"ReplyBy","🇸🇦︙تم حذف قوائم المنع")  
DevAbs:del(muslim..'Abs:Filters:'..msg.chat_id_)
DevAbs:del(muslim.."Abs:FilterAnimation"..msg.chat_id_)
DevAbs:del(muslim.."Abs:FilterPhoto"..msg.chat_id_)
DevAbs:del(muslim.."Abs:FilterSteckr"..msg.chat_id_)
end
...
DevAbs:del(muslim..'Abs:Ban:'..msg.chat_id_) DevAbs:del(muslim..'Abs:Admins:'..msg.chat_id_) DevAbs:del(muslim..'User:Donky:'..msg.chat_id_) DevAbs:del(muslim..'Abs:VipMem:'..msg.chat_id_) DevAbs:del(muslim..'Abs:Filters:'..msg.chat_id_) DevAbs:del(muslim..'Abs:Muted:'..msg.chat_id_)
...
local basicconstructor = DevAbs:smembers(muslim..'Abs:BasicConstructor:'..msg.chat_id_)
local constructor = DevAbs:smembers(muslim..'Abs:Constructor:'..msg.chat_id_)
local Managers = DevAbs:smembers(muslim..'Abs:Managers:'..msg.chat_id_)
local admins = DevAbs:smembers(muslim..'Abs:Admins:'..msg.chat_id_)
local vipmem = DevAbs:smembers(muslim..'Abs:VipMem:'..msg.chat_id_)
local donky = DevAbs:smembers(muslim..'User:Donky:'..msg.chat_id_)
...
spam_c = DevAbs:get(muslim..'Abs:Spam:Text'..msg.chat_id_)
end
--     Source dev yousef     --
if DevAbs:hget(muslim.."Abs:Spam:Group:User"..msg.chat_id_, "Spam:User") == "kick" then     
flood = "بالطرد"     
elseif DevAbs:hget(muslim.."Abs:Spam:Group:User"..msg.chat_id_,"Spam:User") == "keed" then     
flood = "بالتقيد"     
elseif DevAbs:hget(muslim.."Abs:Spam:Group:User"..msg.chat_id_,"Spam:User") == "mute" then     
flood = "بالكتم"           
elseif DevAbs:hget(muslim.."Abs:Spam:Group:User"..msg.chat_id_,"Spam:User") == "del" then     
flood = "بالحذف"
else     
flood = "مفتوح"     
end
--     Source dev yousef     --
if DevAbs:get(muslim.."Abs:Lock:Bots"..msg.chat_id_) == "del" then
lock_bots = "بالحذف"
elseif DevAbs:get(muslim.."Abs:Lock:Bots"..msg.chat_id_) == "ked" then
lock_bots = "بالتقيد"   
elseif DevAbs:get(muslim.."Abs:Lock:Bots"..msg.chat_id_) == "kick" then
lock_bots = "بالطرد"    
else
lock_bots = "مفتوحة"    
end
--     Source dev yousef     --
if DevAbs:get(muslim..'Abs:Lock:Text'..msg.chat_id_) then mute_text = 'مقفل ه' else mute_text = 'مفتوحة'end
if DevAbs:get(muslim..'Abs:Lock:Photo'..msg.chat_id_) then mute_photo = 'مقفل ه' else mute_photo = 'مفتوحة' end
if DevAbs:get(muslim..'Abs:Lock:Videos'..msg.chat_id_) then mute_video = 'مقفل ه' else mute_video = 'مفتوحة' end
if DevAbs:get(muslim..'Abs:Lock:Gifs'..msg.chat_id_) then mute_gifs = 'مقفل ه' else mute_gifs = 'مفتوحة' end
if DevAbs:get(muslim..'Abs:Lock:Music'..msg.chat_id_) then mute_music = 'مقفل ه' else mute_music = 'مفتوحة' end
if DevAbs:get(muslim..'Abs:Lock:Inline'..msg.chat_id_) then mute_in = 'مقفل ه' else mute_in = 'مفتوحة' end
if DevAbs:get(muslim..'Abs:Lock:Voice'..msg.chat_id_) then mute_voice = 'مقفل ه' else mute_voice = 'مفتوحة' end
if DevAbs:get(muslim..'Abs:Lock:EditMsgs'..msg.chat_id_) then mute_edit = 'مقفل ه' else mute_edit = 'مفتوحة' end
if DevAbs:get(muslim..'Abs:Lock:Links'..msg.chat_id_) then mute_links = 'مقفل ه' else mute_links = 'مفتوحة' end
if DevAbs:get(muslim..'Abs:Lock:Pin'..msg.chat_id_) then lock_pin = 'مقفل ه' else lock_pin = 'مفتوحة' end
if DevAbs:get(muslim..'Abs:Lock:Stickers'..msg.chat_id_) then lock_sticker = 'مقفل ه' else lock_sticker = 'مفتوحة' end
if DevAbs:get(muslim..'Abs:Lock:TagServr'..msg.chat_id_) then lock_tgservice = 'مقفل ه' else lock_tgservice = 'مفتوحة' end
if DevAbs:get(muslim..'Abs:Lock:WebLinks'..msg.chat_id_) then lock_wp = 'مقفل ه' else lock_wp = 'مفتوحة' end
if DevAbs:get(muslim..'Abs:Lock:Hashtak'..msg.chat_id_) then lock_htag = 'مقفل ه' else lock_htag = 'مفتوحة' end
if DevAbs:get(muslim..'Abs:Lock:Tags'..msg.chat_id_) then lock_tag = 'مقفل ه' else lock_tag = 'مفتوحة' end
if DevAbs:get(muslim..'Abs:Lock:Location'..msg.chat_id_) then lock_location = 'مقفل ه' else lock_location = 'مفتوحة' end
if DevAbs:get(muslim..'Abs:Lock:Contact'..msg.chat_id_) then lock_contact = 'مقفل ه' else lock_contact = 'مفتوحة' end
if DevAbs:get(muslim..'Abs:Lock:English'..msg.chat_id_) then lock_english = 'مقفل ه' else lock_english = 'مفتوحة' end
if DevAbs:get(muslim..'Abs:Lock:Arabic'..msg.chat_id_) then lock_arabic = 'مقفل ه' else lock_arabic = 'مفتوحة' end
if DevAbs:get(muslim..'Abs:Lock:Forwards'..msg.chat_id_) then lock_forward = 'مقفل ه' else lock_forward = 'مفتوحة' end
if DevAbs:get(muslim..'Abs:Lock:Document'..msg.chat_id_) then lock_file = 'مقفل ه' else lock_file = 'مفتوحة' end
if DevAbs:get(muslim..'Abs:Lock:Markdown'..msg.chat_id_) then markdown = 'مقفل ه' else markdown = 'مفتوحة' end
if DevAbs:get(muslim..'Abs:Lock:Spam'..msg.chat_id_) then lock_spam = 'مقفل ه' else lock_spam = 'مفتوحة' end
if DevAbs:get(muslim..'Abs:Lock:Join'..msg.chat_id_) then lock_Join = 'مقفل ' else lock_Join = 'مفتوح' end
if DevAbs:get(muslim.."Abs:Lock:Welcome"..msg.chat_id_) then send_welcome = 'مقفل ه' else send_welcome = 'مفتوحة' end
if DevAbs:get(muslim..'Abs:Lock:Fshar'..msg.chat_id_) then lock_fshar = 'مفتوح' else lock_fshar = 'مقفل ' end
if DevAbs:get(muslim..'Abs:Lock:Kfr'..msg.chat_id_) then lock_kaf = 'مفتوح' else lock_kaf = 'مقفل ' end
if DevAbs:get(muslim..'Abs:Lock:Taf'..msg.chat_id_) then lock_taf = 'مفتوحة' else lock_taf = 'مقفل ه' end
if DevAbs:get(muslim..'Abs:Lock:Farsi'..msg.chat_id_) then lock_farsi = 'مقفل ه' else lock_farsi = 'مفتوحة' end
local Flood_Num = DevAbs:hget(muslim.."Abs:Spam:Group:User"..msg.chat_id_,"Num:Spam") or 5
--     Source dev yousef     --
...
--     Source dev yousef     --
if DevAbs:get(muslim..'Abs:setrules'..msg.chat_id_..':'..msg.sender_user_id_) then 
if text == 'الغاء' then 
Dev_Abs(msg.chat_id_, msg.id_, 1, '🇸🇦︙تم الغاء حفظ قوانين المجموعة', 1, 'md')
DevAbs:del(muslim..'Abs:setrules'..msg.chat_id_..':'..msg.sender_user_id_)
return false  
end 
DevAbs:del(muslim..'Abs:setrules'..msg.chat_id_..':'..msg.sender_user_id_)
DevAbs:set(muslim..'Abs:rules'..msg.chat_id_,text)
...
DevAbs:set(muslim..'Abs:setrules'..msg.chat_id_..':'..msg.sender_user_id_,true)
end
end
if text and text:match("^القوانين$") then
local rules = DevAbs:get(muslim..'Abs:rules'..msg.chat_id_)
...
local devyousef = '🇸🇦︙اهلا عزيزي -› '..AbsRank(msg)..' \n🇸🇦︙تم تفعيل ميزة انطق'
absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, devyousef, 14, string.len(msg.sender_user_id_))
DevAbs:del(muslim..'Abs:Antk:Abs'..msg.chat_id_) 
end
if text == "تعطيل انطق" and Manager(msg) and ChCheck(msg) then
local devyousef = '🇸🇦︙اهلا عزيزي -› '..AbsRank(msg)..' \n🇸🇦︙تم تعطيل ميزة انطق'
absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, devyousef, 14, string.len(msg.sender_user_id_))
DevAbs:set(muslim..'Abs:Antk:Abs'..msg.chat_id_,true)  
end
if text and text:match("^انطق (.*)$") and not DevAbs:get(muslim..'Abs:Antk:Abs'..msg.chat_id_) and ChCheck(msg) then
...
absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, devyousef, 14, string.len(msg.sender_user_id_))
DevAbs:del(muslim..'Abs:Zrf:Abs'..msg.chat_id_) 
end
if text == "تعطيل الزخرفه" and Manager(msg) and ChCheck(msg) then
local devyousef = '🇸🇦︙اهلا عزيزي -› '..AbsRank(msg)..' \n🇸🇦︙تم تعطيل الزخرفه بنجاح'
absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, devyousef, 14, string.len(msg.sender_user_id_))
DevAbs:set(muslim..'Abs:Zrf:Abs'..msg.chat_id_,true)  
end
if DevAbs:get(muslim..'Zrf:Abs'..msg.chat_id_..msg.sender_user_id_) then 
if text and text == 'الغاء' then 
Dev_Abs(msg.chat_id_, msg.id_, 1, '🇸🇦︙تم الغاء امر الزخرفه', 1, 'md')
DevAbs:del(muslim..'Zrf:Abs'..msg.chat_id_..msg.sender_user_id_)
return false  
end 
...
if not DevAbs:get(muslim..'Abs:Zrf:Abs'..msg.chat_id_) then
if text == 'زخرفه' and ChCheck(msg) or text == 'الزخرفه' and ChCheck(msg) then  
DevAbs:setex(muslim.."Zrf:Abs"..msg.chat_id_..msg.sender_user_id_,300,true)
...
absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, devyousef, 14, string.len(msg.sender_user_id_))
DevAbs:del(muslim..'Abs:Brg:Abs'..msg.chat_id_) 
end
if text == "تعطيل الابراج" and Manager(msg) and ChCheck(msg) then
local devyousef = '🇸🇦︙اهلا عزيزي -› '..AbsRank(msg)..' \n🇸🇦︙تم تعطيل الابراج بنجاح'
absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, devyousef, 14, string.len(msg.sender_user_id_))
DevAbs:set(muslim..'Abs:Brg:Abs'..msg.chat_id_,true)  
end
if not DevAbs:get(muslim..'Abs:Brg:Abs'..msg.chat_id_) then
...
absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, devyousef, 14, string.len(msg.sender_user_id_))
DevAbs:del(muslim..'Abs:Nsba:Abs'..msg.chat_id_) 
end
...
absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, devyousef, 14, string.len(msg.sender_user_id_))
DevAbs:set(muslim..'Abs:Nsba:Abs'..msg.chat_id_,true)  
end
if not DevAbs:get(muslim..'Abs:Nsba:Abs'..msg.chat_id_) then
if text == "نسبة الحب" and ChCheck(msg) or text == "نسبة الحب" and ChCheck(msg) then
DevAbs:set(muslim..'LoveNsba:Abs'..msg.chat_id_..msg.sender_user_id_,true) 
Dev_Abs(msg.chat_id_, msg.id_, 1, '🇸🇦︙قم بارسل اسمين لحساب نسبة الحب بينهما كمثال -› جاك وروز', 1, 'md')
end
end
if text and text ~= "نسبة الحب" and text ~= "نسبة الحب" and DevAbs:get(muslim..'LoveNsba:Abs'..msg.chat_id_..msg.sender_user_id_) then
if text and text == 'الغاء' then 
Dev_Abs(msg.chat_id_, msg.id_, 1, '🇸🇦︙تم الغاء امر نسبة الحب ', 1, 'md')
DevAbs:del(muslim..'LoveNsba:Abs'..msg.chat_id_..msg.sender_user_id_) 
return false 
end 
...
DevAbs:set(muslim..'HataNsba:Abs'..msg.chat_id_..msg.sender_user_id_,true) 
Dev_Abs(msg.chat_id_, msg.id_, 1, '🇸🇦︙قم بارسل اسمين لحساب نسبة الكره بينهما كمثال -› جاك وروز', 1, 'md')
end
end
if text and text ~= "نسبة الكره" and text ~= "نسبة الكره" and DevAbs:get(muslim..'HataNsba:Abs'..msg.chat_id_..msg.sender_user_id_) then
if text and text == 'الغاء' then 
Dev_Abs(msg.chat_id_, msg.id_, 1, '🇸🇦︙تم الغاء امر نسبة الكره ', 1, 'md')
DevAbs:del(muslim..'HataNsba:Abs'..msg.chat_id_..msg.sender_user_id_) 
return false 
end 
Abs = math.random(0,100);
...
DevAbs:set(muslim..'RjolaNsba:Abs'..msg.chat_id_..msg.sender_user_id_,true) 
Dev_Abs(msg.chat_id_, msg.id_, 1, '🇸🇦︙قم بارسل اسم الشخص لقياس نسبة رجولته كمثال -› جاك', 1, 'md')
end
end
if text and text ~= "نسبة الرجوله" and text ~= "نسبة الرجوله" and text ~= "نسبة رجوله" and text ~= "نسبة رجوله" and DevAbs:get(muslim..'RjolaNsba:Abs'..msg.chat_id_..msg.sender_user_id_) then
if text and text == 'الغاء' then 
Dev_Abs(msg.chat_id_, msg.id_, 1, '🇸🇦︙تم الغاء امر نسبة الرجوله ', 1, 'md')
DevAbs:del(muslim..'RjolaNsba:Abs'..msg.chat_id_..msg.sender_user_id_) 
return false 
end 
Abs = math.random(0,100);
...
DevAbs:set(muslim..'AnothaNsba:Abs'..msg.chat_id_..msg.sender_user_id_,true) 
Dev_Abs(msg.chat_id_, msg.id_, 1, '🇸🇦︙قم بارسل اسم الشخص لقياس نسبة انوثته كمثال -› روز', 1, 'md')
end
end
if text and text ~= "نسبة الانوثة" and text ~= "نسبة الانوثة" and text ~= "نسبة انوثة" and text ~= "نسبة انوثة" and DevAbs:get(muslim..'AnothaNsba:Abs'..msg.chat_id_..msg.sender_user_id_) then
if text and text == 'الغاء' then 
Dev_Abs(msg.chat_id_, msg.id_, 1, '🇸🇦︙تم الغاء امر نسبة الانوثة ', 1, 'md')
DevAbs:del(muslim..'AnothaNsba:Abs'..msg.chat_id_..msg.sender_user_id_) 
return false 
end 
Abs = math.random(0,100);
...
DevAbs:set(muslim..'StupidNsba:Abs'..msg.chat_id_..msg.sender_user_id_,true) 
Dev_Abs(msg.chat_id_, msg.id_, 1, '🇸🇦︙قم بارسل اسم الشخص لقياس نسبة غبائه كمثال -› جاك او روز', 1, 'md')
end
end
if text and text ~= "نسبة الغباء" and text ~= "نسبة الغباء" and DevAbs:get(muslim..'StupidNsba:Abs'..msg.chat_id_..msg.sender_user_id_) then
if text and text == 'الغاء' then 
Dev_Abs(msg.chat_id_, msg.id_, 1, '🇸🇦︙تم الغاء امر نسبة الغباء ', 1, 'md')
DevAbs:del(muslim..'StupidNsba:Abs'..msg.chat_id_..msg.sender_user_id_) 
return false 
end 
Abs = math.random(0,100);
...
local devyousef = '🇸🇦︙اهلا عزيزي -› '..AbsRank(msg)..' \n🇸🇦︙تم تفعيل حساب العمر'
absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, devyousef, 14, string.len(msg.sender_user_id_))
DevAbs:del(muslim..'Abs:Age:Abs'..msg.chat_id_) 
end
if text == "تعطيل حساب العمر" and Manager(msg) and ChCheck(msg) then
local devyousef = '🇸🇦︙اهلا عزيزي -› '..AbsRank(msg)..' \n🇸🇦︙تم تعطيل حساب العمر'
absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, devyousef, 14, string.len(msg.sender_user_id_))
DevAbs:set(muslim..'Abs:Age:Abs'..msg.chat_id_,true)  
end
if not DevAbs:get(muslim..'Abs:Age:Abs'..msg.chat_id_) then
...
absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, devyousef, 14, string.len(msg.sender_user_id_))
DevAbs:del(muslim..'Abs:Mean:Abs'..msg.chat_id_) 
end
if text == "تعطيل معاني الاسماء" and Manager(msg) and ChCheck(msg) then
local devyousef = '🇸🇦︙اهلا عزيزي -› '..AbsRank(msg)..' \n🇸🇦︙تم تعطيل معاني الاسماء'
absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, devyousef, 14, string.len(msg.sender_user_id_))
DevAbs:set(muslim..'Abs:Mean:Abs'..msg.chat_id_,true)  
end
if not DevAbs:get(muslim..'Abs:Mean:Abs'..msg.chat_id_) then
...
absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, devyousef, 14, string.len(msg.sender_user_id_))
DevAbs:del(muslim..'Abs:Audios:Abs'..msg.chat_id_) 
end
if text == "تعطيل غنيلي" and Manager(msg) and ChCheck(msg) then
local devyousef = '🇸🇦︙اهلا عزيزي -› '..AbsRank(msg)..' \n🇸🇦︙تم تعطيل غنيلي بنجاح'
absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, devyousef, 14, string.len(msg.sender_user_id_))
DevAbs:set(muslim..'Abs:Audios:Abs'..msg.chat_id_,true)  
end
if text == "غنيلي" and not DevAbs:get(muslim..'Abs:Audios:Abs'..msg.chat_id_) and ChCheck(msg) then
...
if text and (text == "متحركة" or text == "-› متحركة ᥀") and not DevAbs:get(muslim..'Abs:gif:Abs'..msg.chat_id_) and SourceCh(msg) then
...
local msg_id = msg.id_/2097152/0.5 
https.request("https://api.telegram.org/bot"..TokenBot..'/sendanimation?chat_id=' .. msg.chat_id_ .. '&animation=https://t.me/Gifmuslim/'..Abs..'&caption=' .. URL.escape(Text).."&reply_to_message_id="..msg_id.."&parse_mode=markdown&disable_web_page_preview=true&reply_markup="..JSON.encode(keyboard)) 
end
if text and (text == "ميمز" or text == "-› ميمز ᥀") and not DevAbs:get(muslim..'Abs:memz:Abs'..msg.chat_id_) and SourceCh(msg) then
...
local msg_id = msg.id_/2097152/0.5 
https.request("https://api.telegram.org/bot"..TokenBot..'/sendVoice?chat_id=' .. msg.chat_id_ .. '&voice=https://t.me/Memzmuslim/'..Abs..'&caption=' .. URL.escape(Text).."&reply_to_message_id="..msg_id.."&parse_mode=markdown&disable_web_page_preview=true&reply_markup="..JSON.encode(keyboard)) 
end
--     Source dev yousef     --
if Admin(msg) then
if DevAbs:get(muslim..'Abs:LockSettings'..msg.chat_id_) then 
if text == "الروابط" then if DevAbs:get(muslim..'Abs:Lock:Links'..msg.chat_id_) then mute_links = 'مقفل ه' else mute_links = 'مفتوحة' end local devyousef = "\n" .."🇸🇦︙الروابط -› "..mute_links.."\n" Dev_Abs(msg.chat_id_, msg.id_, 1, devyousef, 1, 'md') end
if text == "المعرف" or text == "المعرفات" then if DevAbs:get(muslim..'Abs:Lock:Tags'..msg.chat_id_) then lock_tag = 'مقفوله' else lock_tag = 'مفتوحة' end local devyousef = "\n" .."🇸🇦︙المعرف -› "..lock_tag.."\n" Dev_Abs(msg.chat_id_, msg.id_, 1, devyousef, 1, 'md') end
if text == "المتحركة" or text == "الملصقات المتحركة" then if DevAbs:get(muslim..'Abs:Lock:Gifs'..msg.chat_id_) then mute_gifs = 'مقفوله' else mute_gifs = 'مفتوحة' end local devyousef = "\n" .."🇸🇦︙المتحركة -› "..mute_gifs.."\n" Dev_Abs(msg.chat_id_, msg.id_, 1, devyousef, 1, 'md') end
if text == "الملصقات" then if DevAbs:get(muslim..'Abs:Lock:Stickers'..msg.chat_id_) then lock_sticker = 'مقفوله' else lock_sticker = 'مفتوحة' end local devyousef = "\n" .."🇸🇦︙الملصقات -› "..lock_sticker.."\n" Dev_Abs(msg.chat_id_, msg.id_, 1, devyousef, 1, 'md') end
if text == "الصور" then if DevAbs:get(muslim..'Abs:Lock:Photo'..msg.chat_id_) then mute_photo = 'مقفوله' else mute_photo = 'مفتوحة' end local devyousef = "\n" .."🇸🇦︙الصور -› "..mute_photo.."\n" Dev_Abs(msg.chat_id_, msg.id_, 1, devyousef, 1, 'md') end
if text == "الفيديو" or text == "الفيديوهات" then if DevAbs:get(muslim..'Abs:Lock:Videos'..msg.chat_id_) then mute_video = 'مقفوله' else mute_video = 'مفتوحة' end local devyousef = "\n" .."🇸🇦︙الفيديو -› "..mute_video.."\n" Dev_Abs(msg.chat_id_, msg.id_, 1, devyousef, 1, 'md') end
if text == "الاونلاين" then if DevAbs:get(muslim..'Abs:Lock:Inline'..msg.chat_id_) then mute_in = 'مقفل ' else mute_in = 'مفتوح' end local devyousef = "\n" .."🇸🇦︙الاونلاين -› "..mute_in.."\n" Dev_Abs(msg.chat_id_, msg.id_, 1, devyousef, 1, 'md') end
if text == "الدردشة" then if DevAbs:get(muslim..'Abs:Lock:Text'..msg.chat_id_) then mute_text = 'مقفل ه' else mute_text = 'مفتوحة' end local devyousef = "\n" .."🇸🇦︙الدردشة -› "..mute_text.."\n" Dev_Abs(msg.chat_id_, msg.id_, 1, devyousef, 1, 'md') end
if text == "التوجيه" or text == "اعاده التوجيه" then if DevAbs:get(muslim..'Abs:Lock:Forwards'..msg.chat_id_) then lock_forward = 'مقفل ' else lock_forward = 'مفتوح' end local devyousef = "\n" .."🇸🇦︙التوجيه -› "..lock_forward.."\n" Dev_Abs(msg.chat_id_, msg.id_, 1, devyousef, 1, 'md') end
if text == "الاغاني" then if DevAbs:get(muslim..'Abs:Lock:Music'..msg.chat_id_) then mute_music = 'مقفوله' else mute_music = 'مفتوحة' end local devyousef = "\n" .."🇸🇦︙الاغاني -› "..mute_music.."\n" Dev_Abs(msg.chat_id_, msg.id_, 1, devyousef, 1, 'md') end
if text == "الصوت" or text == "الصوتيات" then if DevAbs:get(muslim..'Abs:Lock:Voice'..msg.chat_id_) then mute_voice = 'مقفول' else mute_voice = 'مفتوح' end local devyousef = "\n" .."🇸🇦︙الصوت -› "..mute_voice.."\n" Dev_Abs(msg.chat_id_, msg.id_, 1, devyousef, 1, 'md') end
if text == "الجهات" or text == "جهات الاتصال" then if DevAbs:get(muslim..'Abs:Lock:Contact'..msg.chat_id_) then lock_contact = 'مقفوله' else lock_contact = 'مفتوحة' end local devyousef = "\n" .."🇸🇦︙الجهات -› "..lock_contact.."\n" Dev_Abs(msg.chat_id_, msg.id_, 1, devyousef, 1, 'md') end
if text == "الماركداون" then if DevAbs:get(muslim..'Abs:Lock:Markdown'..msg.chat_id_) then markdown = 'مقفل ' else markdown = 'مفتوح' end local devyousef = "\n" .."🇸🇦︙الماركداون -› "..markdown.."\n" Dev_Abs(msg.chat_id_, msg.id_, 1, devyousef, 1, 'md') end
if text == "الهاشتاك" then if DevAbs:get(muslim..'Abs:Lock:Hashtak'..msg.chat_id_) then lock_htag = 'مقفل ' else lock_htag = 'مفتوح' end local devyousef = "\n" .."🇸🇦︙الهاشتاك -› "..lock_htag.."\n"Dev_Abs(msg.chat_id_, msg.id_, 1, devyousef, 1, 'md') end
if text == "التعديل" then if DevAbs:get(muslim..'Abs:Lock:EditMsgs'..msg.chat_id_) then mute_edit = 'مقفل ' else mute_edit = 'مفتوح' end local devyousef = "\n" .."🇸🇦︙التعديل -› "..mute_edit.."\n" Dev_Abs(msg.chat_id_, msg.id_, 1, devyousef, 1, 'md') end
if text == "التثبيت" then if DevAbs:get(muslim..'Abs:Lock:Pin'..msg.chat_id_) then lock_pin = 'مقفل ' else lock_pin = 'مفتوح' end local devyousef = "\n" .."🇸🇦︙التثبيت -› "..lock_pin.."\n" Dev_Abs(msg.chat_id_, msg.id_, 1, devyousef, 1, 'md') end
if text == "الاشعارات" then if DevAbs:get(muslim..'Abs:Lock:TagServr'..msg.chat_id_) then lock_tgservice = 'مقفوله' else lock_tgservice = 'مفتوحة' end local devyousef = "\n" .."🇸🇦︙الاشعارات -› "..lock_tgservice.."\n" Dev_Abs(msg.chat_id_, msg.id_, 1, devyousef, 1, 'md') end
if text == "المنشورات" then if DevAbs:get(muslim..'Abs:Lock:Spam'..msg.chat_id_) then lock_spam = 'مقفوله' else lock_spam = 'مفتوحة' end local devyousef = "\n" .."🇸🇦︙المنشورات -› "..lock_spam.."\n" Dev_Abs(msg.chat_id_, msg.id_, 1, devyousef, 1, 'md') end
if text == "الدخول" then if DevAbs:get(muslim..'Abs:Lock:Join'..msg.chat_id_) then lock_Join = 'مقفول' else lock_Join = 'مفتوح' end local devyousef = "\n" .."🇸🇦︙الدخول -› "..lock_Join.."\n" Dev_Abs(msg.chat_id_, msg.id_, 1, devyousef, 1, 'md') end
if text == "الشبكات" then if DevAbs:get(muslim..'Abs:Lock:WebLinks'..msg.chat_id_) then lock_wp = 'مقفوله' else lock_wp = 'مفتوحة' end local devyousef = "\n" .."🇸🇦︙الشبكات -› "..lock_wp.."\n" Dev_Abs(msg.chat_id_, msg.id_, 1, devyousef, 1, 'md') end
if text == "المواقع" then if DevAbs:get(muslim..'Abs:Lock:Location'..msg.chat_id_) then lock_location = 'مقفوله' else lock_location = 'مفتوحة' end local devyousef = "\n" .."🇸🇦︙المواقع -› "..lock_location.."\n" Dev_Abs(msg.chat_id_, msg.id_, 1, devyousef, 1, 'md') end
if text == "العربية" then if DevAbs:get(muslim..'Abs:Lock:Arabic'..msg.chat_id_) then lock_arabic = 'مقفوله' else lock_arabic = 'مفتوحة' end local devyousef = "\n" .."🇸🇦︙العربية -› "..lock_arabic.."\n" Dev_Abs(msg.chat_id_, msg.id_, 1, devyousef, 1, 'md') end
if text == "الانجليزية" then if DevAbs:get(muslim..'Abs:Lock:English'..msg.chat_id_) then lock_english = 'مقفوله' else lock_english = 'مفتوحة' end local devyousef = "\n" .."🇸🇦︙الانجليزية -› "..lock_english.."\n" Dev_Abs(msg.chat_id_, msg.id_, 1, devyousef, 1, 'md') end
if text == "الكفر" then if DevAbs:get(muslim..'Abs:Lock:Kfr'..msg.chat_id_) then lock_kaf = 'مفتوح' else lock_kaf = 'مقفل ' end local devyousef = "\n" .."🇸🇦︙الكفر -› "..lock_kaf.."\n" Dev_Abs(msg.chat_id_, msg.id_, 1, devyousef, 1, 'md') end
if text == "السب" then if DevAbs:get(muslim..'Abs:Lock:Fshar'..msg.chat_id_) then lock_fshar = 'مفتوح' else lock_fshar = 'مقفل ' end local devyousef = "\n" .."🇸🇦︙السب -› "..lock_fshar.."\n" Dev_Abs(msg.chat_id_, msg.id_, 1, devyousef, 1, 'md') end
if text == "الطائفيه" then if DevAbs:get(muslim..'Abs:Lock:Taf'..msg.chat_id_) then lock_taf = 'مفتوحة' else lock_taf = 'مقفل ه' end local devyousef = "\n" .."🇸🇦︙الطائفيه -› "..lock_taf.."\n" Dev_Abs(msg.chat_id_, msg.id_, 1, devyousef, 1, 'md') end
end
--     Source dev yousef     --
if text == 'تفعيل كشف الاعدادات' and ChCheck(msg) then 
local devyousef = '🇸🇦︙اهلا عزيزي -› '..AbsRank(msg)..' \n🇸🇦︙تم تفعيل كشف الاعدادات'
absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, devyousef, 14, string.len(msg.sender_user_id_))
DevAbs:set(muslim..'Abs:LockSettings'..msg.chat_id_,true)  
end
if text == 'تعطيل كشف الاعدادات' and ChCheck(msg) then 
local devyousef = '🇸🇦︙اهلا عزيزي -› '..AbsRank(msg)..' \n🇸🇦︙تم تعطيل كشف الاعدادات'
absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, devyousef, 14, string.len(msg.sender_user_id_))
DevAbs:del(muslim..'Abs:LockSettings'..msg.chat_id_) 
end
--     Source dev yousef     --
if text == 'تفعيل اوامر التحشيش' and Manager(msg) and ChCheck(msg) then 
local devyousef = '🇸🇦︙اهلا عزيزي -› '..AbsRank(msg)..' \n🇸🇦︙تم تفعيل اوامر التحشيش'
absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, devyousef, 14, string.len(msg.sender_user_id_))
DevAbs:del(muslim..'Abs:Lock:Stupid'..msg.chat_id_)
end
if text == 'تعطيل اوامر التحشيش' and Manager(msg) and ChCheck(msg) then 
local devyousef = '🇸🇦︙اهلا عزيزي -› '..AbsRank(msg)..' \n🇸🇦︙تم تعطيل اوامر التحشيش'
absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, devyousef, 14, string.len(msg.sender_user_id_))
DevAbs:set(muslim..'Abs:Lock:Stupid'..msg.chat_id_,true)
end
--     Source dev yousef     --
...
absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, devyousef, 14, string.len(msg.sender_user_id_))
DevAbs:del(muslim..'Abs:Lock:Robot'..msg.chat_id_)
end
...
absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, devyousef, 14, string.len(msg.sender_user_id_))
DevAbs:set(muslim..'Abs:Lock:Robot'..msg.chat_id_,true)
end
--     Source dev yousef     --
if text == 'تفعيل ردود المدير' and Manager(msg) and ChCheck(msg) then 
local devyousef = '🇸🇦︙اهلا عزيزي -› '..AbsRank(msg)..' \n🇸🇦︙تم تفعيل ردود المدير'
absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, devyousef, 14, string.len(msg.sender_user_id_))
DevAbs:del(muslim..'Abs:Lock:GpRed'..msg.chat_id_)
end
if text == 'تعطيل ردود المدير' and Manager(msg) and ChCheck(msg) then 
local devyousef = '🇸🇦︙اهلا عزيزي -› '..AbsRank(msg)..' \n🇸🇦︙تم تعطيل ردود المدير'
absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, devyousef, 14, string.len(msg.sender_user_id_))
DevAbs:set(muslim..'Abs:Lock:GpRed'..msg.chat_id_,true)
end
--     Source dev yousef     --
if text == 'تفعيل ردود المطور' and Manager(msg) and ChCheck(msg) then 
local devyousef = '🇸🇦︙اهلا عزيزي -› '..AbsRank(msg)..' \n🇸🇦︙تم تفعيل ردود المطور'
absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, devyousef, 14, string.len(msg.sender_user_id_))
DevAbs:del(muslim..'Abs:Lock:AllRed'..msg.chat_id_)
end
if text == 'تعطيل ردود المطور' and Manager(msg) and ChCheck(msg) then 
local devyousef = '🇸🇦︙اهلا عزيزي -› '..AbsRank(msg)..' \n🇸🇦︙تم تعطيل ردود المطور'
absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, devyousef, 14, string.len(msg.sender_user_id_))
DevAbs:set(muslim..'Abs:Lock:AllRed'..msg.chat_id_,true)
end
--     Source dev yousef     --
if SecondSudo(msg) then
...
absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, devyousef, 14, string.len(msg.sender_user_id_))
DevAbs:del(muslim.."Abs:Left:Bot"..muslim)
end
if text == 'تعطيل المغادره' or text == '-› تعطيل المغادره ⌁' then 
local devyousef = '🇸🇦︙اهلا عزيزي -› '..AbsRank(msg)..' \n🇸🇦︙تم تعطيل المغادره بنجاح'
absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, devyousef, 14, string.len(msg.sender_user_id_))
DevAbs:set(muslim.."Abs:Left:Bot"..muslim,true) 
end 
if text == 'تفعيل الاذاعه' or text == '-› تفعيل الاذاعه ⌁' then 
local devyousef = '🇸🇦︙اهلا عزيزي -› '..AbsRank(msg)..' \n🇸🇦︙تم تفعيل الاذاعه بنجاح'
absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, devyousef, 14, string.len(msg.sender_user_id_))
DevAbs:del(muslim.."Abs:Send:Bot"..muslim)
end
if text == 'تعطيل الاذاعه' or text == '-› تعطيل الاذاعه ⌁' then 
local devyousef = '🇸🇦︙اهلا عزيزي -› '..AbsRank(msg)..' \n🇸🇦︙تم تعطيل الاذاعه بنجاح'
absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, devyousef, 14, string.len(msg.sender_user_id_))
DevAbs:set(muslim.."Abs:Send:Bot"..muslim,true) 
end
end
--     Source dev yousef     --
...
--     Source dev yousef     --
if msg.content_.photo_ then
if DevAbs:get(muslim..'Abs:SetPhoto'..msg.chat_id_..':'..msg.sender_user_id_) then
if msg.content_.photo_.sizes_[3] then
...
DevAbs:del(muslim..'Abs:SetPhoto'..msg.chat_id_..':'..msg.sender_user_id_)
return false  end
if data.message_ == "CHAT_ADMIN_REQUIRED" then 
send(msg.chat_id_, msg.id_,"🇸🇦︙ليست لدي صلاحية تغير معلومات المجموعة يرجى التحقق من الصلاحيات") 
DevAbs:del(muslim..'Abs:SetPhoto'..msg.chat_id_..':'..msg.sender_user_id_)
...
DevAbs:set(muslim..'Abs:SetPhoto'..msg.chat_id_..':'..msg.sender_user_id_,true)
end
--     Source dev yousef     --
...
if data.ID == "Ok" then
DevAbs:del(muslim..'Abs:PinnedMsg'..msg.chat_id_)
...
local PinId = DevAbs:get(muslim..'Abs:PinnedMsg'..msg.chat_id_)
if PinId then
...
DevAbs:del(muslim..'Abs:Ban:'..msg.chat_id_)
else
local x = 0
...
local Muted = DevAbs:scard(muslim.."Abs:Muted:"..msg.chat_id_) or "0"
tdcli_function({ID ="GetChat",chat_id_=msg.chat_id_},function(arg,dp) 
tdcli_function({ID="GetChannelFull",channel_id_ = msg.chat_id_:gsub("-100","")},function(arg,data) 
...
local ConstructorList = DevAbs:scard(muslim.."Abs:Constructor:"..ChatId) or 0
local BanedList = DevAbs:scard(muslim.."Abs:Ban:"..ChatId) or 0
local ManagerList = DevAbs:scard(muslim.."Abs:Managers:"..ChatId) or 0
local MutedList = DevAbs:scard(muslim.."Abs:Muted:"..ChatId) or 0
local TkeedList = DevAbs:scard(muslim.."Abs:Abs:Tkeed:"..ChatId) or 0
local AdminsList = DevAbs:scard(muslim.."Abs:Admins:"..ChatId) or 0
local VipList = DevAbs:scard(muslim.."Abs:VipMem:"..ChatId) or 0
...
ChatLeave(dp.id_, muslim)
DevAbs:srem(muslim.."Abs:Groups", dp.id_)
else
...
DevAbs:set(muslim..'Abs:Num:Add:Bot',Num) 
...
absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, devyousef, 14, string.len(msg.sender_user_id_))
DevAbs:del(muslim..'Abs:Lock:FreeBot'..muslim) 
end 
end
...
absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, devyousef, 14, string.len(msg.sender_user_id_))
DevAbs:set(muslim..'Abs:Lock:FreeBot'..muslim,true) 
end 
end
if ChatType == 'sp' or ChatType == 'gp'  then
...
absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, devyousef, 14, string.len(msg.sender_user_id_))
DevAbs:del(muslim..'Abs:Lock:Games'..msg.chat_id_) 
end
if text == 'تعطيل الالعاب' and Manager(msg) and ChCheck(msg) or text == 'تعطيل اللعبه' and Manager(msg) and ChCheck(msg) then  
local devyousef = '🇸🇦︙اهلا عزيزي -› '..AbsRank(msg)..' \n🇸🇦︙تم تعطيل الالعاب بنجاح'
absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, devyousef, 14, string.len(msg.sender_user_id_))
DevAbs:set(muslim..'Abs:Lock:Games'..msg.chat_id_,true)  
end
if text == "تفعيل الرابط" or text == "تفعيل جلب الرابط" then 
if Admin(msg) then
local devyousef = '🇸🇦︙اهلا عزيزي -› '..AbsRank(msg)..' \n🇸🇦︙تم تفعيل جلب رابط المجموعة'
absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, devyousef, 14, string.len(msg.sender_user_id_))
DevAbs:del(muslim.."Abs:Lock:GpLinks"..msg.chat_id_)
return false  
end
end
...
absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, devyousef, 14, string.len(msg.sender_user_id_))
DevAbs:set(muslim.."Abs:Lock:GpLinks"..msg.chat_id_,"ok")
return false  
end
end
--     Source dev yousef     --
...
ReplyStatus(msg,result.id_,"ReplyBy","🇸🇦︙تم تفعيل المجموعة "..dp.title_)  
DevAbs:sadd(muslim.."Abs:Groups",msg.chat_id_)
if not DevAbs:get(muslim..'Abs:SudosGp'..msg.sender_user_id_..msg.chat_id_) and not SecondSudo(msg) then 
DevAbs:incrby(muslim..'Abs:Sudos'..msg.sender_user_id_,1)
DevAbs:set(muslim..'Abs:SudosGp'..msg.sender_user_id_..msg.chat_id_,"abs")
end
local Name1 = result.first_name_
local Name1 = Name1:gsub('"',"") 
local Name1 = Name1:gsub("'","") 
...
tdcli_function ({ID = "GetUser",user_id_ = msg.sender_user_id_},function(extra,result,success)
tdcli_function({ID ="GetChat",chat_id_=msg.chat_id_},function(arg,dp) 
if not DevAbs:sismember(muslim..'Abs:Groups',msg.chat_id_) then
...
sendDocument(msg.chat_id_, msg.id_, 0, 1, nil, './muslim.lua', '🇸🇦︙نسخة ملف سورس dev-yousef 🇸🇦',dl_cb, nil)
end end
--     Source dev yousef     --
...
for k,v in pairs(List) do
local GroupsManagers = DevAbs:scard(muslim.."Abs:Managers:"..v) or 0
local GroupsAdmins = DevAbs:scard(muslim.."Abs:Admins:"..v) or 0
local Groupslink = DevAbs:get(muslim.."Abs:Groups:Links" ..v)
...
DevAbs:setex(muslim.."Abs:Send:Pv"..msg.chat_id_..":" .. msg.sender_user_id_, 600, true) 
local devyousef = '🇸🇦︙اهلا عزيزي -› '..AbsRank(msg)..' \n🇸🇦︙ارسل لي سواء -› ⤈ \n❨ ملف • ملصق • متحركة • صورة\n • فيديو • بصمه • صوت • رساله ❩\n🇸🇦︙للخروج ارسل -› ( الغاء ) \n ✓'
absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, devyousef, 14, string.len(msg.sender_user_id_))
return false
end 
if DevAbs:get(muslim.."Abs:Send:Pv"..msg.chat_id_..":" .. msg.sender_user_id_) then 
if text == 'الغاء' then   
Dev_Abs(msg.chat_id_, msg.id_, 1, "🇸🇦︙تم الغاء امر الاذاعه بنجاح", 1, 'md')
DevAbs:del(muslim.."Abs:Send:Pv"..msg.chat_id_..":" .. msg.sender_user_id_) 
return false
end 
List = DevAbs:smembers(muslim..'Abs:Users') 
if msg.content_.text_ then
for k,v in pairs(List) do 
AbsText = "الرساله"
...
DevAbs:setex(muslim.."Abs:Send:Gp"..msg.chat_id_..":" .. msg.sender_user_id_, 600, true) 
local devyousef = '🇸🇦︙اهلا عزيزي -› '..AbsRank(msg)..' \n🇸🇦︙ارسل لي سواء -› ⤈ \n❨ ملف • ملصق • متحركة • صورة\n • فيديو • بصمه • صوت • رساله ❩\n🇸🇦︙للخروج ارسل -› ( الغاء ) \n ✓'
absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, devyousef, 14, string.len(msg.sender_user_id_))
return false
end 
if DevAbs:get(muslim.."Abs:Send:Gp"..msg.chat_id_..":" .. msg.sender_user_id_) then 
if text == 'الغاء' then   
Dev_Abs(msg.chat_id_, msg.id_, 1, "🇸🇦︙تم الغاء امر الاذاعه بنجاح", 1, 'md')
DevAbs:del(muslim.."Abs:Send:Gp"..msg.chat_id_..":" .. msg.sender_user_id_) 
return false
end 
List = DevAbs:smembers(muslim..'Abs:Groups') 
if msg.content_.text_ then
for k,v in pairs(List) do 
AbsText = "الرساله"
send(v, 0,"["..msg.content_.text_.."]") 
...
DevAbs:setex(muslim.."Abs:Send:FwdGp"..msg.chat_id_..":" .. msg.sender_user_id_, 600, true) 
local devyousef = '🇸🇦︙اهلا عزيزي -› '..AbsRank(msg)..' \n🇸🇦︙ارسل الرساله الان لتوجيها \n🇸🇦︙للخروج ارسل -› ( الغاء ) \n ✓'
absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, devyousef, 14, string.len(msg.sender_user_id_))
return false
end 
if DevAbs:get(muslim.."Abs:Send:FwdGp"..msg.chat_id_..":" .. msg.sender_user_id_) then 
if text == 'الغاء' then   
Dev_Abs(msg.chat_id_, msg.id_, 1, "🇸🇦︙تم الغاء امر الاذاعه بنجاح", 1, 'md')
DevAbs:del(muslim.."Abs:Send:FwdGp"..msg.chat_id_..":" .. msg.sender_user_id_) 
return false  
end 
local List = DevAbs:smembers(muslim..'Abs:Groups')   
for k,v in pairs(List) do  
...
DevAbs:setex(muslim.."Abs:Send:FwdPv"..msg.chat_id_..":" .. msg.sender_user_id_, 600, true) 
local devyousef = '🇸🇦︙اهلا عزيزي -› '..AbsRank(msg)..' \n🇸🇦︙ارسل الرساله الان لتوجيها \n🇸🇦︙للخروج ارسل -› ( الغاء ) \n ✓'
absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, devyousef, 14, string.len(msg.sender_user_id_))
return false
end 
if DevAbs:get(muslim.."Abs:Send:FwdPv"..msg.chat_id_..":" .. msg.sender_user_id_) then 
if text == 'الغاء' then   
Dev_Abs(msg.chat_id_, msg.id_, 1, "🇸🇦︙تم الغاء امر الاذاعه بنجاح", 1, 'md')
DevAbs:del(muslim.."Abs:Send:FwdPv"..msg.chat_id_..":" .. msg.sender_user_id_) 
return false  
end 
local List = DevAbs:smembers(muslim..'Abs:Users')   
for k,v in pairs(List) do  
tdcli_function({ID="ForwardMessages", chat_id_ = v, from_chat_id_ = msg.chat_id_, message_ids_ = {[0] = msg.id_}, disable_notification_ = 0, from_background_ = 1},function(a,t) end,nil) 
end   
Dev_Abs(msg.chat_id_, msg.id_, 1, "🇸🇦︙تم اذاعة رسالتك بالتوجيه \n🇸🇦︙‏الى -› ❨ "..#List.." ❩ مشترك \n ✓", 1, 'md')
...
DevAbs:setex(muslim.."Abs:Send:Gp:Pin"..msg.chat_id_..":" .. msg.sender_user_id_, 600, true) 
local devyousef = '🇸🇦︙اهلا عزيزي -› '..AbsRank(msg)..' \n🇸🇦︙ارسل لي سواء -› ⤈ \n❨ ملف • ملصق • متحركة • صورة\n • فيديو • بصمه • صوت • رساله ❩\n🇸🇦︙للخروج ارسل -› ( الغاء ) \n ✓'
absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, devyousef, 14, string.len(msg.sender_user_id_))
return false
end 
if DevAbs:get(muslim.."Abs:Send:Gp:Pin"..msg.chat_id_..":" .. msg.sender_user_id_) then 
if text == "الغاء" then   
Dev_Abs(msg.chat_id_, msg.id_, 1, "🇸🇦︙تم الغاء امر الاذاعه بنجاح", 1, 'md')
DevAbs:del(muslim.."Abs:Send:Gp:Pin"..msg.chat_id_..":" .. msg.sender_user_id_) 
return false
end 
local List = DevAbs:smembers(muslim.."Abs:Groups") 
if msg.content_.text_ then
...
AbsText = "الرساله"
send(v, 0,"["..msg.content_.text_.."]") 
DevAbs:set(muslim..'Abs:PinnedMsgs'..v,msg.content_.text_) 
end
elseif msg.content_.photo_ then
if msg.content_.photo_.sizes_[0] then
photo = msg.content_.photo_.sizes_[0].photo_.persistent_id_
...
AbsText = "الصورة"
sendPhoto(v, 0, 0, 1, nil, photo,(msg.content_.caption_ or ''))
DevAbs:set(muslim..'Abs:PinnedMsgs'..v,photo) 
end 
elseif msg.content_.animation_ then
for k,v in pairs(List) do 
AbsText = "المتحركة"
sendDocument(v, 0, 0, 1,nil, msg.content_.animation_.animation_.persistent_id_,(msg.content_.caption_ or ''))    
DevAbs:set(muslim..'Abs:PinnedMsgs'..v,msg.content_.animation_.animation_.persistent_id_)
end 
elseif msg.content_.video_ then
for k,v in pairs(List) do 
AbsText = "الفيديو"
sendVideo(v, 0, 0, 1, nil, msg.content_.video_.video_.persistent_id_,(msg.content_.caption_ or '')) 
DevAbs:set(muslim..'Abs:PinnedMsgs'..v,msg.content_.video_.video_.persistent_id_)
end 
elseif msg.content_.voice_ then
for k,v in pairs(List) do 
AbsText = "البصمه"
sendVoice(v, 0, 0, 1, nil, msg.content_.voice_.voice_.persistent_id_,(msg.content_.caption_ or '')) 
DevAbs:set(muslim..'Abs:PinnedMsgs'..v,msg.content_.voice_.voice_.persistent_id_)
end 
elseif msg.content_.audio_ then
for k,v in pairs(List) do 
AbsText = "الصوت"
sendAudio(v, 0, 0, 1, nil, msg.content_.audio_.audio_.persistent_id_,(msg.content_.caption_ or '')) 
DevAbs:set(muslim..'Abs:PinnedMsgs'..v,msg.content_.audio_.audio_.persistent_id_)
end 
elseif msg.content_.document_ then
for k,v in pairs(List) do 
AbsText = "الملف"
sendDocument(v, 0, 0, 1,nil, msg.content_.document_.document_.persistent_id_,(msg.content_.caption_ or ''))    
DevAbs:set(muslim..'Abs:PinnedMsgs'..v,msg.content_.document_.document_.persistent_id_)
end 
elseif msg.content_.sticker_ then
for k,v in pairs(List) do 
AbsText = "الملصق"
sendSticker(v, 0, 0, 1, nil, msg.content_.sticker_.sticker_.persistent_id_)   
DevAbs:set(muslim..'Abs:PinnedMsgs'..v,msg.content_.sticker_.sticker_.persistent_id_) 
end 
end
...
local List = DevAbs:smembers(muslim..'Abs:Manager:GpRedod'..msg.chat_id_)
if #List == 0 then
Dev_Abs(msg.chat_id_, msg.id_, 1, "🇸🇦︙لا توجد ردود متعددة مضافة" ,  1, "md")
return false
end
DevAbs:set(muslim..'Abs:Add:GpRedod'..msg.sender_user_id_..msg.chat_id_,'DelGpRedRedod')
Dev_Abs(msg.chat_id_, msg.id_, 1, "🇸🇦︙حسنا ارسل كلمة الرد اولا" ,  1, "md")
return false
end
if text and text:match("^(.*)$") then
local DelGpRedRedod = DevAbs:get(muslim..'Abs:Add:GpRedod'..msg.sender_user_id_..msg.chat_id_)
...
if not DevAbs:sismember(muslim..'Abs:Manager:GpRedod'..msg.chat_id_,text) then
...
DevAbs:set(muslim..'Abs:Add:GpRedod'..msg.sender_user_id_..msg.chat_id_,'DelGpRedRedods')
DevAbs:set(muslim..'Abs:Add:GpTexts'..msg.sender_user_id_..msg.chat_id_,text)
return false
end end
...
DevAbs:set(muslim..'Abs:Add:GpRedod'..msg.sender_user_id_..msg.chat_id_,'DelGpRedod')
Dev_Abs(msg.chat_id_, msg.id_, 1, "🇸🇦︙حسنا ارسل الكلمه لحذفها" ,  1, "md")
return false
end
if text == 'اضف رد متعدد' and Manager(msg) and ChCheck(msg) then
DevAbs:set(muslim..'Abs:Add:GpRedod'..msg.sender_user_id_..msg.chat_id_,'SetGpRedod')
Dev_Abs(msg.chat_id_, msg.id_, 1, "🇸🇦︙حسنا ارسل الكلمه الان" ,  1, "md")
return false
end
if text and text:match("^(.*)$") then
local SetGpRedod = DevAbs:get(muslim..'Abs:Add:GpRedod'..msg.sender_user_id_..msg.chat_id_)
...
DevAbs:set(muslim..'Abs:Add:GpRedod'..msg.sender_user_id_..msg.chat_id_,'SaveGpRedod')
DevAbs:set(muslim..'Abs:Add:GpTexts'..msg.sender_user_id_..msg.chat_id_,text)
DevAbs:sadd(muslim..'Abs:Manager:GpRedod'..msg.chat_id_,text)
return false
end end
--     Source dev yousef     --
if text == 'حذف رد' and Manager(msg) and ChCheck(msg) or text == 'مسح رد' and  Manager(msg) and ChCheck(msg) then
local List = DevAbs:smembers(muslim..'Abs:Manager:GpRed'..msg.chat_id_)
if #List == 0 then
Dev_Abs(msg.chat_id_, msg.id_, 1, "🇸🇦︙لا توجد ردود مضافة" ,  1, "md")
return false
end
DevAbs:set(muslim..'Abs:Add:GpRed'..msg.sender_user_id_..msg.chat_id_,'DelGpRed')
Dev_Abs(msg.chat_id_, msg.id_, 1, "🇸🇦︙حسنا ارسل الكلمه لحذفها " ,  1, "md")
return false
end
if text == 'اضف رد' and Manager(msg) and ChCheck(msg) then
DevAbs:set(muslim..'Abs:Add:GpRed'..msg.sender_user_id_..msg.chat_id_,'SetGpRed')
Dev_Abs(msg.chat_id_, msg.id_, 1, "🇸🇦︙حسنا ارسل الكلمه الان " ,  1, "md")
return false
end
if text and text:match("^(.*)$") then
local SetGpRed = DevAbs:get(muslim..'Abs:Add:GpRed'..msg.sender_user_id_..msg.chat_id_)
...
DevAbs:set(muslim..'Abs:Add:GpRed'..msg.sender_user_id_..msg.chat_id_,'SaveGpRed')
DevAbs:set(muslim..'Abs:Add:GpText'..msg.sender_user_id_..msg.chat_id_,text)
DevAbs:sadd(muslim..'Abs:Manager:GpRed'..msg.chat_id_,text)
DevAbs:set(muslim..'DelManagerRep'..msg.chat_id_,text)
return false
end end
--     Source dev yousef     --
if text == 'حذف رد عام' and SecondSudo(msg) or text == '-› حذف رد عام ⌁' and SecondSudo(msg) or text == 'مسح رد عام' and SecondSudo(msg) then
local List = DevAbs:smembers(muslim.."Abs:Sudo:AllRed")
if #List == 0 then
Dev_Abs(msg.chat_id_, msg.id_, 1, "🇸🇦︙لا توجد ردود مضافة" ,  1, "md")
return false
end
DevAbs:set(muslim.."Abs:Add:AllRed"..msg.sender_user_id_,'DelAllRed')
Dev_Abs(msg.chat_id_, msg.id_, 1, "🇸🇦︙حسنا ارسل الكلمه لحذفها " ,  1, "md")
return false
end
if text == 'اضف رد عام' and SecondSudo(msg) or text == '-› اضف رد عام ⌁' and SecondSudo(msg) then
DevAbs:set(muslim.."Abs:Add:AllRed"..msg.sender_user_id_,'SetAllRed')
Dev_Abs(msg.chat_id_, msg.id_, 1, "🇸🇦︙حسنا ارسل الكلمه الان " ,  1, "md")
return false
end
if text and text:match("^(.*)$") then
local SetAllRed = DevAbs:get(muslim.."Abs:Add:AllRed"..msg.sender_user_id_)
...
Dev_Abs(msg.chat_id_, msg.id_, 1, "🇸🇦︙ارسل لي الرد سواء كان -› ⤈\n❨ ملف • ملصق • متحركة • صورة\n • فيديو • بصمه • صوت • رساله ❩\n🇸🇦︙يمكنك اضافة الى النص -› ⤈\n------------ Y●L ------------\n `#username` ↬ معرف المستخدم\n `#msgs` ↬ عدد الرسائل\n `#name` ↬ اسم المستخدم\n `#id` ↬ ايدي المستخدم\n `#stast` ↬ رتبة المستخدم\n `#edit` ↬ عدد التعديلات\n------------ Y●L ------------\n🇸🇦︙للخروج ارسل -› ( الغاء )\n ✓" ,  1, "md")
DevAbs:set(muslim.."Abs:Add:AllRed"..msg.sender_user_id_,'SaveAllRed')
DevAbs:set(muslim.."Abs:Add:AllText"..msg.sender_user_id_, text)
DevAbs:sadd(muslim.."Abs:Sudo:AllRed",text)
DevAbs:set(muslim.."DelSudoRep",text)
return false 
end end
--     Source dev yousef     --
if text == 'الردود المتعددة' and Manager(msg) and ChCheck(msg) then
local redod = DevAbs:smembers(muslim..'Abs:Manager:GpRedod'..msg.chat_id_)
MsgRep = '🇸🇦︙قائمة الردود المتعددة -› ⤈ \n------------ Y●L ------------\n'
for k,v in pairs(redod) do
MsgRep = MsgRep..k..'~ (`'..v..'`) • {*العدد -› '..#DevAbs:smembers(muslim..'Abs:Text:GpTexts'..v..msg.chat_id_)..'*}\n' 
end
if #redod == 0 then
...
for k,v in pairs(redod) do
DevAbs:del(muslim..'Abs:Text:GpTexts'..v..msg.chat_id_)
DevAbs:del(muslim..'Abs:Manager:GpRedod'..msg.chat_id_)
end
...
local redod = DevAbs:smembers(muslim..'Abs:Manager:GpRed'..msg.chat_id_)
MsgRep = '🇸🇦︙ردود المدير -› ⤈ \n------------ Y●L ------------\n'
for k,v in pairs(redod) do
if DevAbs:get(muslim.."Abs:Gif:GpRed"..v..msg.chat_id_) then
dp = 'متحركة 🎭'
elseif DevAbs:get(muslim.."Abs:Voice:GpRed"..v..msg.chat_id_) then
dp = 'بصمه 🎙'
elseif DevAbs:get(muslim.."Abs:Stecker:GpRed"..v..msg.chat_id_) then
dp = 'ملصق 🃏'
elseif DevAbs:get(muslim.."Abs:Text:GpRed"..v..msg.chat_id_) then
dp = 'رساله ✉'
elseif DevAbs:get(muslim.."Abs:Photo:GpRed"..v..msg.chat_id_) then
dp = 'صورة 🎇'
elseif DevAbs:get(muslim.."Abs:Video:GpRed"..v..msg.chat_id_) then
dp = 'فيديو 📽'
elseif DevAbs:get(muslim.."Abs:File:GpRed"..v..msg.chat_id_) then
dp = 'ملف 📁'
elseif DevAbs:get(muslim.."Abs:Audio:GpRed"..v..msg.chat_id_) then
dp = 'اغنيه 🎶'
end
...
for k,v in pairs(redod) do
DevAbs:del(muslim..'Abs:Gif:GpRed'..v..msg.chat_id_)
DevAbs:del(muslim..'Abs:Voice:GpRed'..v..msg.chat_id_)
DevAbs:del(muslim..'Abs:Audio:GpRed'..v..msg.chat_id_)
DevAbs:del(muslim..'Abs:Photo:GpRed'..v..msg.chat_id_)
DevAbs:del(muslim..'Abs:Stecker:GpRed'..v..msg.chat_id_)
DevAbs:del(muslim..'Abs:Video:GpRed'..v..msg.chat_id_)
DevAbs:del(muslim..'Abs:File:GpRed'..v..msg.chat_id_)
DevAbs:del(muslim..'Abs:Text:GpRed'..v..msg.chat_id_)
DevAbs:del(muslim..'Abs:Manager:GpRed'..msg.chat_id_)
end
...
local redod = DevAbs:smembers(muslim.."Abs:Sudo:AllRed")
MsgRep = '🇸🇦︙ردود المطور -› ⤈ \n------------ Y●L ------------\n'
for k,v in pairs(redod) do
if DevAbs:get(muslim.."Abs:Gif:AllRed"..v) then
dp = 'متحركة 🎭'
elseif DevAbs:get(muslim.."Abs:Voice:AllRed"..v) then
dp = 'بصمه 🎙'
elseif DevAbs:get(muslim.."Abs:Stecker:AllRed"..v) then
dp = 'ملصق 🃏'
elseif DevAbs:get(muslim.."Abs:Text:AllRed"..v) then
dp = 'رساله ✉'
elseif DevAbs:get(muslim.."Abs:Photo:AllRed"..v) then
dp = 'صورة 🎇'
elseif DevAbs:get(muslim.."Abs:Video:AllRed"..v) then
dp = 'فيديو 📽'
elseif DevAbs:get(muslim.."Abs:File:AllRed"..v) then
dp = 'ملف 📁'
elseif DevAbs:get(muslim.."Abs:Audio:AllRed"..v) then
dp = 'اغنيه 🎶'
end
MsgRep = MsgRep..k..'~ (`'..v..'`) -› {*'..dp..'*}\n' 
end
...
for k,v in pairs(redod) do
DevAbs:del(muslim.."Abs:Add:AllRed"..v)
DevAbs:del(muslim.."Abs:Gif:AllRed"..v)
DevAbs:del(muslim.."Abs:Voice:AllRed"..v)
DevAbs:del(muslim.."Abs:Audio:AllRed"..v)
DevAbs:del(muslim.."Abs:Photo:AllRed"..v)
DevAbs:del(muslim.."Abs:Stecker:AllRed"..v)
DevAbs:del(muslim.."Abs:Video:AllRed"..v)
DevAbs:del(muslim.."Abs:File:AllRed"..v)
DevAbs:del(muslim.."Abs:Text:AllRed"..v)
DevAbs:del(muslim.."Abs:Sudo:AllRed")
end
...
DevAbs:set(muslim..'Abs:NameBot'..msg.sender_user_id_, 'msg')
return false 
end
end
...
DevAbs:set(muslim..'Abs:Help0'..msg.sender_user_id_, 'msg')
return false end
if text and text:match("^(.*)$") then
local devyousef =  DevAbs:get(muslim..'Abs:Help0'..msg.sender_user_id_)
if devyousef == 'msg' then
Dev_Abs(msg.chat_id_, msg.id_, 1, text , 1, 'md')
DevAbs:del(muslim..'Abs:Help0'..msg.sender_user_id_)
DevAbs:set(muslim..'Abs:Help', text)
Dev_Abs(msg.chat_id_, msg.id_, 1, "🇸🇦︙تم حفظ الكليشه الجديده " ,  1, "md")
return false end
end
...
DevAbs:set(muslim..'Abs:Help01'..msg.sender_user_id_, 'msg')
return false end
if text and text:match("^(.*)$") then
local devyousef =  DevAbs:get(muslim..'Abs:Help01'..msg.sender_user_id_)
if devyousef == 'msg' then 
Dev_Abs(msg.chat_id_, msg.id_, 1, text , 1, 'md')
DevAbs:del(muslim..'Abs:Help01'..msg.sender_user_id_)
DevAbs:set(muslim..'Abs:Help1', text)
Dev_Abs(msg.chat_id_, msg.id_, 1, "🇸🇦︙تم حفظ الكليشه الجديده " ,  1, "md")
return false end
end
...
DevAbs:set(muslim..'Abs:Help21'..msg.sender_user_id_, 'msg')
return false end
if text and text:match("^(.*)$") then
local devyousef =  DevAbs:get(muslim..'Abs:Help21'..msg.sender_user_id_)
if devyousef == 'msg' then
Dev_Abs(msg.chat_id_, msg.id_, 1, text , 1, 'md')
DevAbs:del(muslim..'Abs:Help21'..msg.sender_user_id_)
DevAbs:set(muslim..'Abs:Help2', text)
Dev_Abs(msg.chat_id_, msg.id_, 1, "🇸🇦︙تم حفظ الكليشه الجديده " ,  1, "md")
return false end
end
...
DevAbs:set(muslim..'Abs:Help31'..msg.sender_user_id_, 'msg')
return false end
if text and text:match("^(.*)$") then
local devyousef =  DevAbs:get(muslim..'Abs:Help31'..msg.sender_user_id_)
if devyousef == 'msg' then
Dev_Abs(msg.chat_id_, msg.id_, 1, text , 1, 'md')
DevAbs:del(muslim..'Abs:Help31'..msg.sender_user_id_)
DevAbs:set(muslim..'Abs:Help3', text)
Dev_Abs(msg.chat_id_, msg.id_, 1, "🇸🇦︙تم حفظ الكليشه الجديده " ,  1, "md")
return false end
end
...
DevAbs:set(muslim..'Abs:Help41'..msg.sender_user_id_, 'msg')
return false end
if text and text:match("^(.*)$") then
local devyousef =  DevAbs:get(muslim..'Abs:Help41'..msg.sender_user_id_)
if devyousef == 'msg' then
Dev_Abs(msg.chat_id_, msg.id_, 1, text , 1, 'md')
DevAbs:del(muslim..'Abs:Help41'..msg.sender_user_id_)
DevAbs:set(muslim..'Abs:Help4', text)
Dev_Abs(msg.chat_id_, msg.id_, 1, "🇸🇦︙تم حفظ الكليشه الجديده" ,  1, "md")
return false end
end
...
DevAbs:set(muslim..'Abs:Help51'..msg.sender_user_id_, 'msg')
return false end
if text and text:match("^(.*)$") then
local devyousef =  DevAbs:get(muslim..'Abs:Help51'..msg.sender_user_id_)
if devyousef == 'msg' then
Dev_Abs(msg.chat_id_, msg.id_, 1, text , 1, 'md')
DevAbs:del(muslim..'Abs:Help51'..msg.sender_user_id_)
DevAbs:set(muslim..'Abs:Help5', text)
Dev_Abs(msg.chat_id_, msg.id_, 1, "🇸🇦︙تم حفظ الكليشه الجديده " ,  1, "md")
return false end
end
...
DevAbs:set(muslim..'Abs:Help61'..msg.sender_user_id_, 'msg')
return false end
if text and text:match("^(.*)$") then
local devyousef =  DevAbs:get(muslim..'Abs:Help61'..msg.sender_user_id_)
if devyousef == 'msg' then
Dev_Abs(msg.chat_id_, msg.id_, 1, text , 1, 'md')
DevAbs:del(muslim..'Abs:Help61'..msg.sender_user_id_)
DevAbs:set(muslim..'Abs:Help6', text)
Dev_Abs(msg.chat_id_, msg.id_, 1, "🇸🇦︙تم حفظ الكليشه الجديده" ,  1, "md")
return false end
end
...
os.execute('rm -rf muslim.lua') 
os.execute('wget https://raw.githubusercontent.com/Yosef-lbban/muslim/master/muslim.lua') 
dofile('muslim.lua') 
...
local Get_Files, res = https.request("https://raw.githubusercontent.com/Yosef-lbban/muslimFiles/master/getfile.json")
if res == 200 then
...
local GetJson, Res = https.request("https://raw.githubusercontent.com/Yosef-lbban/muslimFiles/master/muslimFiles/"..FileName)
if Res == 200 then
...
DevAbs:del(muslim..'Abs:Text:BotWelcome')
DevAbs:del(muslim..'Abs:Photo:BotWelcome')
return false
end 
...
DevAbs:del(muslim..'Abs:Lock:BotWelcome')
return false
end 
...
DevAbs:set(muslim..'Abs:Lock:BotWelcome',true)
return false
end 
...
DevAbs:del(muslim..'Abs:Text:BotWelcome')
DevAbs:del(muslim..'Abs:Photo:BotWelcome')
DevAbs:set(muslim.."Abs:Set:BotWelcome"..msg.sender_user_id_,"Text") 
return false
end 
if text and DevAbs:get(muslim.."Abs:Set:BotWelcome"..msg.sender_user_id_) == 'Text' then 
if text and text:match("^الغاء$") then 
Dev_Abs(msg.chat_id_, msg.id_, 1, "🇸🇦︙تم الغاء الامر", 1, "md") 
DevAbs:del(muslim.."Abs:Set:BotWelcome"..msg.sender_user_id_)   
return false
end 
Dev_Abs(msg.chat_id_, msg.id_, 1, "🇸🇦︙تم حفظ النص ارسل لي صورة الترحيب\n🇸🇦︙ارسل -› الغاء لحفظ النص فقط", 1, 'md')   
DevAbs:set(muslim.."Abs:Text:BotWelcome",text) 
DevAbs:set(muslim.."Abs:Set:BotWelcome"..msg.sender_user_id_,"Photo") 
return false 
end 
if DevAbs:get(muslim.."Abs:Set:BotWelcome"..msg.sender_user_id_) == 'Photo' then 
if text and text:match("^الغاء$") then 
...
DevAbs:set(muslim.."Abs:Photo:BotWelcome",msg.content_.photo_.sizes_[1].photo_.persistent_id_)
...
DevAbs:setex(muslim.."Abs:DevText"..msg.chat_id_..":" .. msg.sender_user_id_, 300, true)
end end
...
DevAbs:del(muslim.."DevText")
end end
--     Source dev yousef     --
if DevAbs:get(muslim.."textch:user"..msg.chat_id_.."" .. msg.sender_user_id_) then 
if text and text:match("^الغاء$") then 
Dev_Abs(msg.chat_id_, msg.id_, 1, "🇸🇦︙تم الغاء الامر", 1, "md") 
DevAbs:del(muslim.."textch:user"..msg.chat_id_.."" .. msg.sender_user_id_)  
return false  end 
DevAbs:del(muslim.."textch:user"..msg.chat_id_.."" .. msg.sender_user_id_)  
local texxt = string.match(text, "(.*)") 
DevAbs:set(muslim..'Abs:ChText',texxt)
...
DevAbs:setex(muslim.."textch:user"..msg.chat_id_.."" .. msg.sender_user_id_, 300, true)  
...
DevAbs:incr(muslim..'Abs:EditMsg'..result.chat_id_..result.sender_user_id_)
local text = result.content_.text_ or result.content_.caption_
local Text = result.content_.text_
if DevAbs:get(muslim..'Abs:Lock:EditMsgs'..msg.chat_id_) and not Text and not BasicConstructor(result) then
...
local GetMsgPin = DevAbs:get(muslim..'Abs:PinnedMsgs'..msg.chat_id_)
if GetMsgPin ~= nil then
if text == GetMsgPin then
tdcli_function ({ID = "PinChannelMessage",channel_id_ = msg.chat_id_:gsub('-100',''),message_id_ = msg.id_,disable_notification_ = 0},function(arg,dp) if dp.ID == 'Ok' then;DevAbs:del(muslim..'Abs:PinnedMsgs'..msg.chat_id_);end;end,nil)   
elseif (msg.content_.sticker_) then 
if GetMsgPin == msg.content_.sticker_.sticker_.persistent_id_ then
tdcli_function ({ID = "PinChannelMessage",channel_id_ = msg.chat_id_:gsub('-100',''),message_id_ = msg.id_,disable_notification_ = 0},function(arg,dp) DevAbs:del(muslim..'Abs:PinnedMsgs'..msg.chat_id_) end,nil)   
end
end
if (msg.content_.animation_) then 
...
if data and data.type_ and data.type_.channel_ and data.type_.channel_.status_ and data.type_.channel_.status_.ID == "ChatMemberStatusMember" then
tdcli_function({ID = "ChangeChatMemberStatus",chat_id_=v,user_id_=muslim,status_={ID = "ChatMemberStatusLeft"},},function(e,g) end, nil) 
DevAbs:srem(muslim..'Abs:Groups',v)  
end
if data and data.type_ and data.type_.channel_ and data.type_.channel_.status_ and data.type_.channel_.status_.ID == "ChatMemberStatusLeft" then
DevAbs:srem(muslim..'Abs:Groups',v)  
end
...
DevAbs:sadd(muslim..'Abs:Groups',v)  
end end,nil) end
end
--     Source dev yousef     --
end 
